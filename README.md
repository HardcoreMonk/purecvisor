...
PureCVisor는 단일 노드에서 KVM VM, LXC 컨테이너, ZFS 스토리지, OVS/OVN 기반 네트워크, 백업, 인증, Web UI를 통합 관리하는 C23 기반 하이퍼바이저 오케스트레이터입니다.

이 공개 저장소의 범위는 Single Edge 기능과 그 실행에 필요한 공통 코어로 제한합니다.

라이선스는 비상업용 무료 사용을 허용하는 source-available 라이선스입니다. 자세한 조건은 [LICENSE](LICENSE)를 확인하세요.

## 저작 기록 (Authorship)

이 프로젝트의 공개 소스 코드와 문서는 CODEX가 100% 작성·유지보수합니다. 인간 협업자 `Hardcoremonk`는 요구사항 정의, 사양 결정, 사용자 테스트, 방향 검토를 담당했으며, 단 한 줄의 코드도 직접 손대지 않았습니다.
...
매 세션 시작 시 내부 작업 원칙이 절대 원칙(추측 금지, 자동 재시도 금지, 보안 규칙 등)으로 적용되며, 새 기능 추가나 버그 수정도 모두 같은 규칙 아래 CODEX가 수행합니다. 관련 커밋 메시지에는 `Co-Authored-By: CODEX` 트레일러로 일관 표기됩니다.
...
핵심 설계는 단일 프로세스 데몬과 `GMainLoop` 이벤트 루프입니다. 디스패처는 핸들러 진입 전에 메서드 role 정책과 operator VM owner-scope를 검사합니다. 장시간 작업은 먼저 응답을 반환한 뒤 `GTask` 워커에서 실행하며, 완료 결과는 Job ID, WebSocket push, polling 패턴으로 노출합니다.
...
Web UI 시각 규격은 루트 [DESIGN.md](DESIGN.md)를 기준으로 관리합니다. 운영 이벤트를 한 화면에서 triage하는 `이벤트 센터`는 `운영 > 이벤트 센터` 또는 `/ui#/ops-triage`에서 확인합니다. UI 모듈, Service Worker, vendor 자산, `ui/samples/` 프리뷰를 바꾼 경우 공개 배포 전 외부 런타임 참조가 남지 않았는지 확인합니다.
...
```bash
tests/integration/test_source_commenting_standard.sh
rg -n "\b(TODO|FIXME|HACK|XXX)\b" src include tests ui/modules scripts
git diff --check
```

첫 번째 명령은 소스코드 작성 및 주석 표준이 개발 문서 진입점과 검증 정책에 연결되어 있는지 확인합니다. 두 번째 명령은 일반 소스 영역에서 출력이 없어야 합니다.
