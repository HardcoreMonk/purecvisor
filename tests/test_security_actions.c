...
test_security_action_block_ip_builds_argv(void)
{
    ...
    g_assert_cmpstr(argv[2], ==, "element");                                     
    g_assert_cmpstr(argv[3], ==, "inet");
    g_assert_cmpstr(argv[4], ==, "purecvisor");
    g_assert_cmpstr(argv[5], ==, PCV_HIPS_BLOCK_SET);                             
    g_assert_cmpstr(argv[6], ==, "{");
    g_assert_cmpstr(argv[7], ==, "192.0.2.10");                  
    g_assert_cmpstr(argv[8], ==, "}");
    g_assert_null(argv[9]);                                                
}
...
static void
test_security_action_block_set_script_is_convergent(void)
{
    g_autofree gchar *script = pcv_hips_action_build_block_set_script();
    g_assert_nonnull(script);

                                     
    g_assert_nonnull(strstr(script, "add set inet purecvisor " PCV_HIPS_BLOCK_SET));
    const gchar *rule = strstr(script,
        "add rule inet purecvisor input ip saddr @" PCV_HIPS_BLOCK_SET " drop");
    g_assert_nonnull(rule);
    g_assert_nonnull(strstr(rule, PCV_HIPS_RULE_COMMENT));              

                                  
    guint rule_lines = 0;
    for (const gchar *p = script; (p = strstr(p, "add rule ")) != NULL; p += 9)
        rule_lines++;
    g_assert_cmpuint(rule_lines, ==, 1);

                                                   
    const gchar *flush = strstr(script, "flush chain inet purecvisor input");
    g_assert_nonnull(flush);
    g_assert_true(flush < rule);

                                                            
    g_assert_true(strstr(script, "add set inet purecvisor " PCV_HIPS_BLOCK_SET) < rule);
}
...
test_security_actions_register(void)
{
    ...
    g_test_add_func("/security/actions/block-set-script-convergent",
                    test_security_action_block_set_script_is_convergent);
