...
static void test_get_numa_info(CpuFixture *f, gconstpointer d) {
    ...
    cpu_allocator_add_core(f->alloc, 0, 0, 0, FALSE);            
    cpu_allocator_add_core(f->alloc, 1, 1, 0, TRUE);                 
    cpu_allocator_mark_used(f->alloc, 0, "sys-vm");                                  

    JsonObject *info = cpu_allocator_get_numa_info(f->alloc);
    g_assert_nonnull(info);
    JsonArray *nodes = json_object_get_array_member(info, "numa_nodes");
    g_assert_cmpuint(json_array_get_length(nodes), ==, 1);
    JsonObject *n0 = json_array_get_object_element(nodes, 0);
                                                     
    g_assert_cmpint(json_object_get_int_member(n0, "isolated"),  ==, 1);
    g_assert_cmpint(json_object_get_int_member(n0, "allocated"), ==, 0);
    g_assert_cmpint(json_object_get_int_member(n0, "free"),      ==, 1);
...
static void test_free_wrong_key_leaks(CpuFixture *f, gconstpointer d) {
    ...
    g_assert_cmpint(json_object_get_int_member(n1, "allocated"), ==, 2);              
    ...
    g_assert_cmpint(json_object_get_int_member(n2, "allocated"), ==, 0);             
...
static void test_parse_cpulist_empty(void) {
    GArray *a = cpu_allocator_parse_cpulist(NULL);
    g_assert_nonnull(a); g_assert_cmpuint(a->len, ==, 0); g_array_unref(a);

    a = cpu_allocator_parse_cpulist("");
    g_assert_nonnull(a); g_assert_cmpuint(a->len, ==, 0); g_array_unref(a);
...
static void test_parse_cpulist_single(void) {
    GArray *a = cpu_allocator_parse_cpulist("8");
    g_assert_nonnull(a); g_assert_cmpuint(a->len, ==, 1);
    g_assert_cmpuint(g_array_index(a, guint, 0), ==, 8);
    g_array_unref(a);
}

                           
static void test_parse_cpulist_range(void) {
    GArray *a = cpu_allocator_parse_cpulist("0-3");
    g_assert_nonnull(a); g_assert_cmpuint(a->len, ==, 4);
    for (guint i = 0; i < 4; i++)
        g_assert_cmpuint(g_array_index(a, guint, i), ==, i);
    g_array_unref(a);
}

                                                               
static void test_parse_cpulist_mixed(void) {
    GArray *a = cpu_allocator_parse_cpulist("0-3,8,10-11\n");
    g_assert_nonnull(a);
    guint expect[] = {0, 1, 2, 3, 8, 10, 11};
    g_assert_cmpuint(a->len, ==, G_N_ELEMENTS(expect));
    for (guint i = 0; i < G_N_ELEMENTS(expect); i++)
        g_assert_cmpuint(g_array_index(a, guint, i), ==, expect[i]);
    g_array_unref(a);
}

                                                   
static void test_parse_cpulist_full_host(void) {
    GArray *a = cpu_allocator_parse_cpulist("0-15");
    g_assert_nonnull(a); g_assert_cmpuint(a->len, ==, 16);
    g_assert_cmpuint(g_array_index(a, guint, 15), ==, 15);
    g_array_unref(a);
}

                                  
static void test_parse_cpulist_sort_dedup(void) {
    GArray *a = cpu_allocator_parse_cpulist("3,1,2,1,2-3");
    g_assert_nonnull(a);
    guint expect[] = {1, 2, 3};
    g_assert_cmpuint(a->len, ==, G_N_ELEMENTS(expect));
    for (guint i = 0; i < G_N_ELEMENTS(expect); i++)
...
static void test_parse_cpulist_malformed(void) {
    g_assert_null(cpu_allocator_parse_cpulist("abc"));
    g_assert_null(cpu_allocator_parse_cpulist("3-1"));                  
    g_assert_null(cpu_allocator_parse_cpulist("0-"));                
    g_assert_null(cpu_allocator_parse_cpulist("1,x"));
}

                                                  
static void test_cpulist_contains_intersection(void) {
    GArray *isolated = cpu_allocator_parse_cpulist("2,3");
    g_assert_nonnull(isolated);
                                
    g_assert_false(cpu_allocator_cpulist_contains(isolated, 0));
    g_assert_false(cpu_allocator_cpulist_contains(isolated, 1));
    g_assert_true(cpu_allocator_cpulist_contains(isolated, 2));
    g_assert_true(cpu_allocator_cpulist_contains(isolated, 3));
    g_array_unref(isolated);

                                    
    GArray *empty = cpu_allocator_parse_cpulist("");
    g_assert_false(cpu_allocator_cpulist_contains(empty, 0));
    g_array_unref(empty);

                                 
    g_assert_false(cpu_allocator_cpulist_contains(NULL, 0));
}

void test_cpu_allocator_register(void) {
    g_test_add_func("/cpu_allocator/parse_cpulist/empty",        test_parse_cpulist_empty);
    g_test_add_func("/cpu_allocator/parse_cpulist/single",       test_parse_cpulist_single);
    g_test_add_func("/cpu_allocator/parse_cpulist/range",        test_parse_cpulist_range);
    g_test_add_func("/cpu_allocator/parse_cpulist/mixed",        test_parse_cpulist_mixed);
    g_test_add_func("/cpu_allocator/parse_cpulist/full_host",    test_parse_cpulist_full_host);
    g_test_add_func("/cpu_allocator/parse_cpulist/sort_dedup",   test_parse_cpulist_sort_dedup);
    g_test_add_func("/cpu_allocator/parse_cpulist/malformed",    test_parse_cpulist_malformed);
    g_test_add_func("/cpu_allocator/cpulist_contains/intersect", test_cpulist_contains_intersection);
