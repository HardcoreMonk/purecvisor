...
LOG_FILE="/tmp/purecvisor_integ_${RUN_ID}.log"
DAEMON_PID=""
PASS=0; FAIL=0; WARN_CNT=0
ORIGINAL_SERVICE_ACTIVE=0
ORIGINAL_SERVICE_STATE=""
LOCAL_DAEMON_STARTED=0
CLEANUP_RUNNING=0
FIXTURES_ARMED=0
LOG_OWNED=0
SERVICE_RESTORE_ARMED=0
SOCKET_CLEANUP_ARMED=0

if [ "${PCV_DISPOSABLE_HOST:-0}" != "1" ]; then
    echo "REFUSED: set PCV_DISPOSABLE_HOST=1 only on a dedicated disposable host" >&2
    exit 2
fi
for required_tool in systemctl virsh zfs ip nft pgrep socat; do
    if ! command -v "$required_tool" >/dev/null 2>&1; then
        echo "REFUSED: required live-test tool not found: $required_tool" >&2
        exit 2
    fi
done
if [ ! -x "$DAEMON_BIN" ]; then
    echo "REFUSED: daemon binary is not executable: $DAEMON_BIN" >&2
    exit 2
fi
ORIGINAL_SERVICE_STATE=$(systemctl is-active "$DAEMON_SERVICE" 2>/dev/null || true)
case "$ORIGINAL_SERVICE_STATE" in
    active) ORIGINAL_SERVICE_ACTIVE=1 ;;
    inactive|failed) ;;
    *)
        echo "REFUSED: unsupported/transient service state '$ORIGINAL_SERVICE_STATE' for $DAEMON_SERVICE" >&2
        exit 2
        ;;
esac
...
start_daemon() {
    log "기존 daemon service 상태 캡처 및 정지..."
    if systemctl is-active --quiet "$DAEMON_SERVICE"; then
                                                            
        SERVICE_RESTORE_ARMED=1
        systemctl stop "$DAEMON_SERVICE" || {
            fail "기존 daemon service 정지 실패"
            exit 1
        }
    elif [ -S "$SOCKET_PATH" ]; then
        fail "systemd 밖에서 관리되는 daemon socket 발견 — 안전하게 교체할 수 없음"
        exit 1
    fi
    if systemctl is-active --quiet "$DAEMON_SERVICE"; then
        fail "기존 daemon service가 정지되지 않음"
        exit 1
    fi
    if [ -e "$SOCKET_PATH" ]; then
        fail "pre-existing daemon socket/path 발견 — 이 실행이 소유하지 않아 제거하지 않음"
        exit 1
    fi
    ...
    SOCKET_CLEANUP_ARMED=1
    "$DAEMON_BIN" > "$LOG_FILE" 2>&1 &
    DAEMON_PID=$!
    LOCAL_DAEMON_STARTED=1
...
TEST_VM="pcv-gio-${RUN_ID}"
REST_VM="pcv-rest-${RUN_ID}"
SNAP_VM="pcv-snap-${RUN_ID}"
SNAP_NAME="pcv-snap-${RUN_ID}"
NIC_TEST_VM="pcv-nic-${RUN_ID}"
G_BR="pcvg$(( $$ % 100000 ))"
G_OCTET=$(( $$ % 200 + 20 ))
G_CIDR="10.254.${G_OCTET}.1/24"
G_VLAN_VM="pcv-vlan-${RUN_ID}"
REST_R3_BODY="/tmp/pcv_rest_r3_${RUN_ID}.body"
...
cleanup() {
    local status=$? cleanup_failed=0 vm dataset network_state current_service_state
    local domains_after datasets_after links_after routes_after nft_after suffix
    local domains_ok=1 datasets_ok=1 links_ok=1 routes_ok=1 nft_ok=1
    [ "$CLEANUP_RUNNING" -eq 0 ] || return
    CLEANUP_RUNNING=1
    trap - EXIT

    if [ "$FIXTURES_ARMED" -eq 1 ]; then
                                                                       
        if [ "$LOCAL_DAEMON_STARTED" -eq 1 ]; then
            send_rpc "{\"jsonrpc\":\"2.0\",\"method\":\"network.delete\",\"params\":{\"bridge_name\":\"$G_BR\"},\"id\":\"cleanup-network\"}" 10 >/dev/null || true
            network_state=$(send_rpc '{"jsonrpc":"2.0","method":"network.list","params":{},"id":"cleanup-network-verify"}' 10)
            if [ -z "$network_state" ] || printf '%s' "$network_state" | grep -Fq "$G_BR"; then
                echo -e "${RED}[CLEANUP-FAIL]${NC} network registry 잔여 확인 실패: $G_BR" >&2
                cleanup_failed=1
            fi
        fi
        for vm in "$TEST_VM" "$REST_VM" "$SNAP_VM" "$NIC_TEST_VM" "$G_VLAN_VM"; do
            virsh destroy "$vm" >/dev/null 2>&1 || true
            virsh undefine "$vm" --nvram >/dev/null 2>&1 || virsh undefine "$vm" >/dev/null 2>&1 || true
            dataset="rpool/vms/$vm"
            zfs destroy -r "$dataset" >/dev/null 2>&1 || true
        done
        ip link delete "$G_BR" >/dev/null 2>&1 || true
    fi

    stop_daemon
    if [ "$SOCKET_CLEANUP_ARMED" -eq 1 ] && [ -e "$SOCKET_PATH" ]; then
        rm -f -- "$SOCKET_PATH" || {
            echo -e "${RED}[CLEANUP-FAIL]${NC} local daemon socket 제거 실패: $SOCKET_PATH" >&2
            cleanup_failed=1
        }
    fi

    if [ "$FIXTURES_ARMED" -eq 1 ]; then
        domains_after=$(virsh list --all --name 2>/dev/null) || domains_ok=0
        datasets_after=$(zfs list -H -o name 2>/dev/null) || datasets_ok=0
        links_after=$(ip -o link show 2>/dev/null) || links_ok=0
        routes_after=$(ip route show 2>/dev/null) || routes_ok=0
        nft_after=$(nft list ruleset 2>/dev/null) || nft_ok=0
        if [ "$domains_ok" -ne 1 ] || [ "$datasets_ok" -ne 1 ] || \
           [ "$links_ok" -ne 1 ] || [ "$routes_ok" -ne 1 ] || [ "$nft_ok" -ne 1 ]; then
            echo -e "${RED}[CLEANUP-FAIL]${NC} post-cleanup host inventory capture failed" >&2
            cleanup_failed=1
        fi
        for vm in "$TEST_VM" "$REST_VM" "$SNAP_VM" "$NIC_TEST_VM" "$G_VLAN_VM"; do
            if [ "$domains_ok" -eq 1 ] && printf '%s\n' "$domains_after" | grep -Fxq "$vm"; then
                echo -e "${RED}[CLEANUP-FAIL]${NC} libvirt domain 잔여: $vm" >&2
                cleanup_failed=1
            fi
            if [ "$datasets_ok" -eq 1 ] && printf '%s\n' "$datasets_after" | grep -Fxq "rpool/vms/$vm"; then
                echo -e "${RED}[CLEANUP-FAIL]${NC} ZFS dataset 잔여: rpool/vms/$vm" >&2
                cleanup_failed=1
            fi
        done
        if [ "$links_ok" -eq 1 ] && printf '%s\n' "$links_after" | grep -Eq "^[0-9]+: ${G_BR}(:|@)"; then
            echo -e "${RED}[CLEANUP-FAIL]${NC} network bridge 잔여: $G_BR" >&2
            cleanup_failed=1
        fi
        if [ "$routes_ok" -eq 1 ] && printf '%s\n' "$routes_after" | grep -Fq "${G_CIDR%.*}.0/24"; then
            echo -e "${RED}[CLEANUP-FAIL]${NC} network route 잔여: ${G_CIDR%.*}.0/24" >&2
            cleanup_failed=1
        fi
        if [ "$nft_ok" -eq 1 ] && printf '%s\n' "$nft_after" | grep -Fq "$G_BR"; then
            echo -e "${RED}[CLEANUP-FAIL]${NC} nftables rule 잔여: $G_BR" >&2
            cleanup_failed=1
        fi
        for suffix in conf pid leases meta; do
            if [ -e "/var/run/purecvisor/network/dnsmasq-${G_BR}.${suffix}" ]; then
                echo -e "${RED}[CLEANUP-FAIL]${NC} network runtime file 잔여: dnsmasq-${G_BR}.${suffix}" >&2
                cleanup_failed=1
            fi
        done
        if pgrep -af -- "$G_BR" >/dev/null 2>&1; then
            echo -e "${RED}[CLEANUP-FAIL]${NC} network helper process 잔여: $G_BR" >&2
            cleanup_failed=1
        fi
    fi

    if [ "$SERVICE_RESTORE_ARMED" -eq 1 ]; then
        systemctl start "$DAEMON_SERVICE" >/dev/null 2>&1 || cleanup_failed=1
        if ! systemctl is-active --quiet "$DAEMON_SERVICE"; then
            echo -e "${RED}[CLEANUP-FAIL]${NC} original daemon service active 상태 복원 실패" >&2
            cleanup_failed=1
        fi
        SERVICE_RESTORE_ARMED=0
    elif [ "$FIXTURES_ARMED" -eq 1 ]; then
        current_service_state=$(systemctl is-active "$DAEMON_SERVICE" 2>/dev/null || true)
        if [ "$current_service_state" != "$ORIGINAL_SERVICE_STATE" ]; then
            echo -e "${RED}[CLEANUP-FAIL]${NC} daemon service 상태 복원 실패 (expected=$ORIGINAL_SERVICE_STATE actual=$current_service_state)" >&2
            cleanup_failed=1
        fi
    fi

    if [ "$cleanup_failed" -ne 0 ]; then
        echo "cleanup diagnostics retained: $LOG_FILE" >&2
        exit 90
    fi
    if [ "$LOG_OWNED" -eq 1 ]; then
        rm -f -- "$LOG_FILE" "$REST_R3_BODY" || {
            echo -e "${RED}[CLEANUP-FAIL]${NC} integration temp file 제거 실패" >&2
            exit 90
        }
    fi
    exit "$status"
...
capture_fixture_baseline() {
    local vm domains datasets links routes nft_rules suffix
    if ! domains=$(virsh list --all --name 2>/dev/null); then
        echo -e "${RED}[ERROR]${NC} libvirt domain baseline capture failed" >&2
        return 1
    fi
    if ! datasets=$(zfs list -H -o name 2>/dev/null); then
        echo -e "${RED}[ERROR]${NC} ZFS dataset baseline capture failed" >&2
        return 1
    fi
    if ! links=$(ip -o link show 2>/dev/null) || \
       ! routes=$(ip route show 2>/dev/null); then
        echo -e "${RED}[ERROR]${NC} host network baseline capture failed" >&2
        return 1
    fi
    if ! nft_rules=$(nft list ruleset 2>/dev/null); then
        echo -e "${RED}[ERROR]${NC} nftables baseline capture failed" >&2
        return 1
    fi
    for vm in "$TEST_VM" "$REST_VM" "$SNAP_VM" "$NIC_TEST_VM" "$G_VLAN_VM"; do
        if printf '%s\n' "$domains" | grep -Fxq "$vm" || \
           printf '%s\n' "$datasets" | grep -Fxq "rpool/vms/$vm"; then
            echo -e "${RED}[ERROR]${NC} pre-existing VM fixture collision: $vm" >&2
            return 1
        fi
    done
    if printf '%s\n' "$links" | grep -Eq "^[0-9]+: ${G_BR}(:|@)"; then
        echo -e "${RED}[ERROR]${NC} pre-existing network fixture collision: $G_BR" >&2
        return 1
    fi
    if printf '%s\n' "$routes" | grep -Fq "${G_CIDR%.*}.0/24"; then
        echo -e "${RED}[ERROR]${NC} pre-existing route collision: ${G_CIDR%.*}.0/24" >&2
        return 1
    fi
    if printf '%s\n' "$nft_rules" | grep -Fq "$G_BR" || pgrep -af -- "$G_BR" >/dev/null 2>&1; then
        echo -e "${RED}[ERROR]${NC} pre-existing network rule/process collision: $G_BR" >&2
        return 1
    fi
    for suffix in conf pid leases meta; do
        if [ -e "/var/run/purecvisor/network/dnsmasq-${G_BR}.${suffix}" ]; then
            echo -e "${RED}[ERROR]${NC} pre-existing network runtime file collision: dnsmasq-${G_BR}.${suffix}" >&2
            return 1
        fi
    done
    if [ -e "$LOG_FILE" ] || [ -e "$REST_R3_BODY" ]; then
        echo -e "${RED}[ERROR]${NC} pre-existing integration temp file collision (run=$RUN_ID)" >&2
        return 1
    fi
    FIXTURES_ARMED=1
    LOG_OWNED=1
...
test_vm_create() {
    ...
    req=$(printf '{"jsonrpc":"2.0","method":"vm.create","params":{"name":"%s","vcpu":1,"ram_mb":512,"disk_size_gb":5%s,"network_bridge":"%s","qos_min_mbps":0,"qos_max_mbps":100},"id":1}' "$TEST_VM" "$iso_field" "$TEST_NET_BR")
    resp=$(send_rpc "$req")
...
capture_fixture_baseline || exit 1
test_daemon_starts
test_metrics_signal_no_vm

                    
if [ ! -f "$ISO_PATH" ] || [ ! -r "$ISO_PATH" ]; then
    warn "ISO 파일이 없거나 읽을 수 없음: $ISO_PATH"
...
code=$(curl -s -o "$REST_R3_BODY" -w "%{http_code}" -X POST --max-time 5 \
     -H "Content-Type: application/json" \
     -d "{\"username\":\"pcv-wrong-${RUN_ID}\",\"password\":\"wrong\"}" \
     "${REST_BASE}/auth/token" 2>/dev/null)
r=$(cat "$REST_R3_BODY" 2>/dev/null)
...
echo -e "${CYAN}════════════════════════════════════════════════════${NC}"
echo ""
echo "  임시 로그: $LOG_FILE (성공 시 EXIT cleanup, 실패 시 진단용 보존)"
echo ""
