...
BUNDLE=$(curl -s --connect-timeout 5 "${UI}/app.bundle.js" 2>/dev/null)
...
count_occ() { printf '%s' "$1" | grep -o "$2" 2>/dev/null | wc -l; }

                                                                                
aria_label=$(count_occ "$HTML" 'aria-label')
aria_live=$(count_occ "$HTML" 'aria-live')
roles=$(count_occ "$HTML" 'role=')

[ "$aria_label" -ge 15 ] && ok "index.html aria-label: ${aria_label}개" || fail "index.html aria-label: ${aria_label}개 (15+ expected)"
[ "$aria_live" -ge 4 ] && ok "index.html aria-live: ${aria_live}개" || fail "index.html aria-live: ${aria_live}개 (4+ expected)"
[ "$roles" -ge 8 ] && ok "index.html role=: ${roles}개" || fail "index.html role=: ${roles}개 (8+ expected)"

                                                                      
b_aria_label=$(count_occ "$BUNDLE" 'aria-label')
b_roles=$(count_occ "$BUNDLE" 'role:')
b_tabindexes=$(count_occ "$BUNDLE" 'tabindex:')

[ "$b_aria_label" -ge 58 ] && ok "bundle aria-label: ${b_aria_label}개" || fail "bundle aria-label: ${b_aria_label}개 (58+ expected)"
[ "$b_roles" -ge 26 ] && ok "bundle role: ${b_roles}개" || fail "bundle role: ${b_roles}개 (26+ expected)"
[ "$b_tabindexes" -ge 8 ] && ok "bundle tabindex: ${b_tabindexes}개" || fail "bundle tabindex: ${b_tabindexes}개 (8+ expected)"

                                       
check_contains "셸 사이드바 nav 항목 (data-nav)" "${UI}/app.bundle.js" 'data-nav'
check_contains "셸 크럼 컨테이너" "${UI}/app.bundle.js" 'shell-crumb'
...
check_contains "언어 선택기 존재 (Prefs 모달)" "${UI}/app.bundle.js" 'lang-select'
check_contains "테마 선택기 존재 (Prefs 모달)" "${UI}/app.bundle.js" 'theme-select'
