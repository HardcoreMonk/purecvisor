...
TEST_SG="pcv-sg-${RUN_ID}"
XSS_SG="pcv-xss-${RUN_ID}"
SQL_SG="pcv-sql-${RUN_ID};drop"
MUTATION_STARTED=0
...
cleanup() {
    local status=$? cleanup_failed=0 name response remaining
    trap - EXIT
    if [ "$MUTATION_STARTED" -eq 1 ]; then
        for name in "$TEST_SG" "$XSS_SG" "$SQL_SG"; do
            response=$(rpc "{\"jsonrpc\":\"2.0\",\"method\":\"security_group.delete\",\"params\":{\"name\":\"$name\"},\"id\":\"cleanup\"}")
            [ -n "$response" ] || {
                echo -e "${RED}[CLEANUP-FAIL]${NC} security group delete 응답 없음: $name" >&2
                cleanup_failed=1
            }
        done
        remaining=$(rpc '{"jsonrpc":"2.0","method":"security_group.list","params":{},"id":"cleanup-verify"}')
        for name in "$TEST_SG" "$XSS_SG" "$SQL_SG"; do
            if ! printf '%s' "$remaining" | grep -q '"result"' || printf '%s' "$remaining" | grep -Fq "$name"; then
                echo -e "${RED}[CLEANUP-FAIL]${NC} security group 잔여 확인 실패: $name" >&2
                cleanup_failed=1
            fi
        done
    fi
...
BASELINE=$(rpc '{"jsonrpc":"2.0","method":"security_group.list","params":{},"id":"baseline"}')
if ! printf '%s' "$BASELINE" | grep -q '"result"'; then
    echo -e "${RED}[ERROR]${NC} security group baseline capture failed" >&2
    exit 1
fi
for name in "$TEST_SG" "$XSS_SG" "$SQL_SG"; do
    if printf '%s' "$BASELINE" | grep -Fq "$name"; then
        echo -e "${RED}[ERROR]${NC} pre-existing security group collision: $name" >&2
        exit 1
    fi
done
MUTATION_STARTED=1
...
RESP=$(rpc "{\"jsonrpc\":\"2.0\",\"method\":\"security_group.create\",\"params\":{\"name\":\"$SQL_SG\"},\"id\":\"e1\"}")
assert_valid_jsonrpc "security_group.create: SQL injection in name returns valid JSON-RPC" "$RESP"

                    
RESP=$(rpc "{\"jsonrpc\":\"2.0\",\"method\":\"security_group.create\",\"params\":{\"name\":\"$XSS_SG\",\"description\":\"<script>alert(1)<\\/script>\"},\"id\":\"e2\"}")
