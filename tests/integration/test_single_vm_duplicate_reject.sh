...
first_resp="$(rpc "{\"jsonrpc\":\"2.0\",\"method\":\"vm.create\",\"params\":{\"name\":\"$VM\",\"vcpu\":1,\"memory_mb\":512,\"disk_size_gb\":5,\"network_bridge\":\"virbr0\",\"qos_min_mbps\":0,\"qos_max_mbps\":100},\"id\":7001}")"
printf '%s\n' "$first_resp" | jq -e '.result.accepted == true and .result.name == "'"$VM"'"' >/dev/null
sleep 2

second_resp="$(rpc "{\"jsonrpc\":\"2.0\",\"method\":\"vm.create\",\"params\":{\"name\":\"$VM\",\"vcpu\":1,\"memory_mb\":512,\"disk_size_gb\":5,\"network_bridge\":\"virbr0\",\"qos_min_mbps\":0,\"qos_max_mbps\":100},\"id\":7002}")"
