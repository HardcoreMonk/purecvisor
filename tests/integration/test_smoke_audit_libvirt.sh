#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
# B3 smoke test: audit DB ↔ libvirt 상태 일치성 검증 (ADR-0018)
#
# [목적] 배포 후 5초 안에 시스템이 거짓 success를 기록하지 않는지 확인.
#       audit DB의 vm.start result=ok 레코드가 실제 libvirt running 상태와
#       일치하는지, 그리고 reverse도 검증한다.
#
# [시나리오]
#   1. audit DB에서 최근 1시간 vm.start result=ok 레코드 추출
#   2. 각 VM이 libvirt에 실제로 존재하는지 확인 (정의 또는 running)
#   3. result=fail 레코드의 VM은 비/잘못 정의되어 있어야 함
#   4. dispatcher 자동 audit 회귀 (target='') 발생 0건 검증
#   5. 정상 vm.start 1회 후 ok 레코드 즉시 생성 검증
#
# [사용법]
#   bash tests/integration/test_smoke_audit_libvirt.sh [HOST]
# ═══════════════════════════════════════════════════════════════
set -uo pipefail
...
# ─────────────────────────────────────────────────────
# Test 1: vm.start 거짓 'ok' 회귀 방지 (최근 5분만)
# ADR-0018 Stage 1 — 등록된 fire-and-forget 메서드만 검증
# ─────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────
# Test 2: 최근 vm.start result=ok 레코드 ↔ libvirt 정의 일치성
# ─────────────────────────────────────────────────────
echo ""
...
# ─────────────────────────────────────────────────────
# Test 3: vm.start result=fail 레코드의 VM은 broken/비/정의되어야
# ─────────────────────────────────────────────────────
echo ""
...
if [ -z "$FAIL_VMS" ]; then
    ...
    for VM in $FAIL_VMS; do
        # fail이지만 현재 running인 VM은 의심스러움 (단, 후속 ok가 있을 수 있음)
        if STATE=$(sudo virsh domstate "$VM" 2>/dev/null) && [ "$STATE" = "running" ]; then
            # 같은 VM의 더 최근 ok 레코드가 있으면 정상
    ...
    if [ $UNEXPECTED -eq 0 ]; then
        ...
        # 경고만, fail 처리하지 않음 (재시도 시나리오 가능)
    fi
fi

# ─────────────────────────────────────────────────────
# Test 4: 정상 vm.start 1회 → 즉시 audit 기록 (E2E)
# ─────────────────────────────────────────────────────
echo ""
