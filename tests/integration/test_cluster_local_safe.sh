#!/bin/bash
# tests/integration/test_cluster_local_safe.sh
# SAFE
#
# Cluster Local — SAFE Integration Tests
# Tests: cluster.health, cluster.role, cluster.status,
#        cluster.maintenance.enter/exit, cluster.quota.get (quota.get),
#        cluster.upgrade.status, cluster.affinity.list
#
# Note: cluster.maintenance.enter is WRITE — test exits maintenance immediately
#       after entry to avoid leaving the node in maintenance mode.
#
# Prerequisites:
#   - purecvisormd running (cluster features enabled or graceful skip on -32601)
#   - /var/run/purecvisor/daemon.sock present
#   - nc (netcat) installed
#
# Run: sudo bash tests/integration/test_cluster_local_safe.sh
...
# Track maintenance state so we always exit if we entered
...
# Status should not contain an unexpected crash marker
...
# ══════════════════════════════════════════════════════════════════════
# [4] cluster.maintenance.enter / exit
# ══════════════════════════════════════════════════════════════════════
...
# Immediately exit maintenance to restore normal operation
...
# ══════════════════════════════════════════════════════════════════════
# [5] quota.get (cluster quota — note: registered as quota.get)
# ══════════════════════════════════════════════════════════════════════
log "--- [5/8] quota.get ---"
...
# cluster.health with unexpected param (daemon should ignore or error gracefully)
...
# cluster.status with unexpected param
...
# cluster.affinity.list — SQL injection in name
...
# cluster.upgrade.status — must not trigger an actual upgrade
