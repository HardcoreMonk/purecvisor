    ...
    if [ -n "$TOKEN" ]; then
        if [ "$SNAPSHOT_TOUCHED" -eq 1 ]; then
            curl -s -X DELETE -H "Authorization: Bearer $TOKEN" \
                "$API/containers/prometheus-ctr/snapshots/$SNAP_NAME" >/dev/null 2>&1 || true
        fi
        if [ "$USER_TOUCHED" -eq 1 ]; then
            curl -s -X DELETE -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
                -d "{\"username\":\"$TEST_USER\"}" "$API/auth/users" >/dev/null 2>&1 || true
        fi
    fi
    exit "$cleanup_status"
}
...
r=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    -d "{\"snap_name\":\"$SNAP_NAME\"}" "$API/containers/prometheus-ctr/snapshots" 2>/dev/null)
an "POST /snapshots not 404" "$r" "NOT_FOUND"
               
r=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    -d "{\"snap_name\":\"$SNAP_NAME\"}" "$API/containers/prometheus-ctr/snapshots/rollback" 2>/dev/null)
an "POST /snapshots/rollback not 404" "$r" "NOT_FOUND"
        
r=$(curl -s -X DELETE -H "Authorization: Bearer $TOKEN" "$API/containers/prometheus-ctr/snapshots/$SNAP_NAME" 2>/dev/null)
...
USER_TOUCHED=1
r=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    -d "{\"username\":\"$TEST_USER\",\"password\":\"pass123\",\"role\":\"viewer\"}" "$API/auth/users" 2>/dev/null)
an "POST create user (no error)" "$r" '"error"'

r=$(curl -s -H "Authorization: Bearer $TOKEN" "$API/auth/users" 2>/dev/null)
ac "User $TEST_USER exists" "$r" "$TEST_USER"

r=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    -d "{\"username\":\"$TEST_USER\",\"role\":\"operator\"}" "$API/auth/role" 2>/dev/null)
an "Set role (no error)" "$r" '"error"'

r=$(curl -s -X DELETE -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    -d "{\"username\":\"$TEST_USER\"}" "$API/auth/users" 2>/dev/null)
