...
static gboolean
nic_attach_model_is_valid(const gchar *model)
{
    return g_strcmp0(model, "virtio") == 0 || g_strcmp0(model, "e1000") == 0 ||
           g_strcmp0(model, "e1000e") == 0 || g_strcmp0(model, "rtl8139") == 0;
}

static void test_nic_attach_model_default_virtio(void) {
    JsonObject *params = json_object_new();
    const gchar *model = json_object_has_member(params, "model")
        ? json_object_get_string_member(params, "model") : "virtio";
    g_assert_cmpstr(model, ==, "virtio");
    g_assert_true(nic_attach_model_is_valid(model));
    json_object_unref(params);
}

static void test_nic_attach_model_allowlist_accepts_all_four(void) {
    const gchar *allowed[] = { "virtio", "e1000", "e1000e", "rtl8139" };
    for (guint i = 0; i < G_N_ELEMENTS(allowed); i++)
        g_assert_true(nic_attach_model_is_valid(allowed[i]));
}

static void test_nic_attach_model_allowlist_rejects_injection(void) {
                                                               
                                                  
    g_assert_false(nic_attach_model_is_valid("xen"));
    g_assert_false(nic_attach_model_is_valid("virtio'/><disk type='file"));
    g_assert_false(nic_attach_model_is_valid(""));
    g_assert_false(nic_attach_model_is_valid(NULL));
}

static void test_nic_attach_xml_interpolation_uses_requested_model(void) {
                                                                
                                                           
                                                                 
    const gchar *model = "e1000";
    g_assert_true(nic_attach_model_is_valid(model));
    gchar *xml = g_strdup_printf(
        "<interface type='bridge'>\n"
        "  <source bridge='%s'/>\n"
        "  <model type='%s'/>\n"
        "</interface>", "virbr0", model);
    g_assert_nonnull(strstr(xml, "type='e1000'"));
    g_assert_null(strstr(xml, "type='virtio'"));
    g_free(xml);
}
...
static void test_memtune_floor_small_guest(void) {
    g_assert_cmpuint(pcv_memtune_safe_floor_mb(512),  ==, 512 + 256);                         
    g_assert_cmpuint(pcv_memtune_safe_floor_mb(1024), ==, 1024 + 256);                         
    g_assert_cmpuint(pcv_memtune_safe_floor_mb(0),    ==, 256);                      
}

                                      
static void test_memtune_floor_large_guest(void) {
                                           
    g_assert_cmpuint(pcv_memtune_safe_floor_mb(2048), ==, 2048 + 307);
                                     
    g_assert_cmpuint(pcv_memtune_safe_floor_mb(8192), ==, 8192 + 1228);
}

                                                            
                                    
static void test_memtune_floor_rejects_bare_guest_ram(void) {
    guint64 ram = 4096;
    g_assert_cmpuint((guint64)ram, <, pcv_memtune_safe_floor_mb(ram));                   
}

void test_handler_params_register(void) {
                                       
    g_test_add_func("/handler_params/memtune/floor_small_guest",
                    test_memtune_floor_small_guest);
    g_test_add_func("/handler_params/memtune/floor_large_guest",
                    test_memtune_floor_large_guest);
    g_test_add_func("/handler_params/memtune/floor_rejects_bare_guest_ram",
                    test_memtune_floor_rejects_bare_guest_ram);
    ...
    g_test_add_func("/handler_params/nic_attach/model_default_virtio",
                    test_nic_attach_model_default_virtio);
    g_test_add_func("/handler_params/nic_attach/model_allowlist_accepts_all",
                    test_nic_attach_model_allowlist_accepts_all_four);
    g_test_add_func("/handler_params/nic_attach/model_allowlist_rejects_injection",
                    test_nic_attach_model_allowlist_rejects_injection);
    g_test_add_func("/handler_params/nic_attach/model_xml_interpolation",
                    test_nic_attach_xml_interpolation_uses_requested_model);
}
