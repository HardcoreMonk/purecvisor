...
cleanup() {
  local node
  set +e
  if [ -n "$NETEM_NODE" ]; then
    ssh_cmd "$NETEM_NODE" 'sudo tc qdisc del dev eth0 root 2>/dev/null; sudo tc qdisc del dev ens18 root 2>/dev/null' >/dev/null
  fi
  if [ -n "$DISK_PRESSURE_NODE" ]; then
    ssh_cmd "$DISK_PRESSURE_NODE" 'sudo rm -f /var/lib/purecvisor/disk_pressure_test' >/dev/null
  fi
  for node in "${ETCD_STOPPED_NODES[@]}"; do
    ssh_cmd "$node" 'sudo systemctl start etcd' >/dev/null
  done
  if [ -n "$LIBVIRT_STOPPED_NODE" ]; then
    ssh_cmd "$LIBVIRT_STOPPED_NODE" 'sudo systemctl start libvirtd.socket libvirtd' >/dev/null
  fi
  if [ -n "$DAEMON_VICTIM" ]; then
    ssh_cmd "$DAEMON_VICTIM" 'sudo systemctl start purecvisormd' >/dev/null
  fi
}
trap cleanup EXIT
trap 'exit 130' INT
