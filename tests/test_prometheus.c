...
typedef struct {
    gchar    name[128];
    gchar    labels[PCV_PROM_LABELS_MAX];
    gdouble  value;
    gboolean used;
} FakeSlot;
...
static gboolean
_fake_register(FakeSlot *s, const gchar *name, const gchar *labels, gdouble value)
{
    if (!pcv_prom_labels_fit(labels)) return FALSE;                    
    g_strlcpy(s->name,   name,               sizeof(s->name));
    g_strlcpy(s->labels, labels ? labels : "", sizeof(s->labels));
    s->value = value;
    s->used  = TRUE;
    return TRUE;
}
...
static gchar *
_fake_render(const FakeSlot *s)
{
    if (!s->used) return g_strdup("");
    if (s->labels[0] && s->labels[0] != '=')
        return g_strdup_printf("%s{%s} %.17g\n", s->name, s->labels, s->value);
    return g_strdup_printf("%s %.17g\n", s->name, s->value);
}
...
static gboolean
_line_is_closed(const gchar *line)
{
    const gchar *lb = strchr(line, '{');
    if (!lb) return FALSE;
    const gchar *rb = strchr(lb, '}');
    if (!rb) return FALSE;
    gint quotes = 0;
    for (const gchar *p = lb + 1; p < rb; p++)
        if (*p == '"') quotes++;
...
static gchar *
_score_label_for_name_len(gsize n)
{
    gchar *vm = g_strnfill(n, 'a');
    gchar *s  = g_strdup_printf(
        "metric=\"purecvisor_cgroup_pressure_memory_full_avg10\","
        "vm_name=\"%s\",kind=\"vm\"", vm);
    g_free(vm);
    return s;
}

                                                 
static void test_label_budget_boundaries(void)
{
                                                        
    gchar *l0 = _score_label_for_name_len(0);
    g_assert_cmpuint(strlen(l0), ==, 74);
    g_free(l0);

                                              
    gchar *l53 = _score_label_for_name_len(53);
    g_assert_cmpuint(strlen(l53), ==, 127);
    g_assert_true(pcv_prom_labels_fit(l53));
    g_free(l53);

                                                        
    gchar *l54 = _score_label_for_name_len(54);
    g_assert_cmpuint(strlen(l54), ==, 128);
    g_assert_true(pcv_prom_labels_fit(l54));                       
    g_free(l54);

                                              
    gchar *lmax = _score_label_for_name_len(127);
    g_assert_cmpuint(strlen(lmax), ==, 201);
    g_assert_true(pcv_prom_labels_fit(lmax));
    g_free(lmax);

                                           
    gchar *fit_edge = g_strnfill(PCV_PROM_LABELS_MAX - 1, 'x');
    g_assert_true(pcv_prom_labels_fit(fit_edge));
    g_free(fit_edge);
    gchar *over_edge = g_strnfill(PCV_PROM_LABELS_MAX, 'x');
    g_assert_false(pcv_prom_labels_fit(over_edge));
    g_free(over_edge);

                                         
    g_assert_true(pcv_prom_labels_fit(NULL));
    g_assert_true(pcv_prom_labels_fit(""));
}
...
static void test_render_line_closed_at_max_legal_label(void)
{
    gchar *labels = _score_label_for_name_len(127);             
    FakeSlot slot = {0};

    g_assert_true(_fake_register(&slot, "purecvisor_anomaly_score", labels, 3.2));
    g_assert_cmpstr(slot.labels, ==, labels);                     

    gchar *line = _fake_render(&slot);
    g_assert_true(_line_is_closed(line));
    g_assert_nonnull(strstr(line, "kind=\"vm\"}"));                     
    g_free(line);

                                                  
    gchar legacy[128];
    g_strlcpy(legacy, labels, sizeof(legacy));
    gchar *broken = g_strdup_printf("purecvisor_anomaly_score{%s} %.17g\n",
                                    legacy, 3.2);
    g_assert_false(_line_is_closed(broken));
    g_free(broken);
...
static void test_render_omits_over_budget_label(void)
{
                                                             
                                                 
    gchar *labels = _score_label_for_name_len(182);
    g_assert_cmpuint(strlen(labels), ==, PCV_PROM_LABELS_MAX);

    FakeSlot slot = {0};
    g_assert_false(_fake_register(&slot, "purecvisor_anomaly_score", labels, 3.2));
    g_assert_false(slot.used);

    gchar *out = _fake_render(&slot);
    g_assert_cmpstr(out, ==, "");
    g_assert_null(strstr(out, "purecvisor_anomaly_score"));
    g_free(out);
    g_free(labels);

                        
    gchar *huge = _score_label_for_name_len(1000);
    FakeSlot slot2 = {0};
    g_assert_false(_fake_register(&slot2, "purecvisor_anomaly_score", huge, 1.0));
    g_assert_false(slot2.used);
    g_free(huge);
}
...
void test_prometheus_register(void) {
    ...
    g_test_add_func("/prometheus/label_budget/boundaries",
                    test_label_budget_boundaries);
    g_test_add_func("/prometheus/label_budget/render_line_closed",
                    test_render_line_closed_at_max_legal_label);
    g_test_add_func("/prometheus/label_budget/over_budget_omitted",
                    test_render_omits_over_budget_label);
}
