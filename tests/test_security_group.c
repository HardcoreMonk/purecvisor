...
extern void pcv_security_group_unbind_vm_all(const gchar *vm);
...
static void test_unbind_all_noop_unbound(void) {
    pcv_security_group_unbind_vm_all(NULL);                               
    pcv_security_group_unbind_vm_all("never-bound-vm");                           
    g_assert_false(pcv_security_group_vm_is_bound("never-bound-vm"));
    pcv_security_group_unbind_vm_all("bad;name");                                       
}

                                                                     
                                                              
static void test_unbind_all_removes_all_bindings(void) {
    if (geteuid() != 0) { g_test_skip("root(+netns) 필요 — nft rebuild 성공 전제"); return; }

    (void)pcv_security_group_create("sgub1", "unbind-all g1");
    (void)pcv_security_group_create("sgub2", "unbind-all g2");
    JsonObject *r1 = _rule("ingress", "tcp", "10.11.0.0/24", 443);
    g_assert_true(pcv_security_group_rule_add("sgub1", r1));
    json_object_unref(r1);
    JsonObject *r2 = _rule("ingress", "tcp", "10.12.0.0/24", 8443);
    g_assert_true(pcv_security_group_rule_add("sgub2", r2));
    json_object_unref(r2);

                           
    g_assert_true(pcv_security_group_apply_to_vm("ub-vm", "sgub1"));
    g_assert_true(pcv_security_group_apply_to_vm("ub-vm", "sgub2"));
    g_assert_true(pcv_security_group_vm_is_bound("ub-vm"));

                                  
    g_assert_true(pcv_security_group_apply_to_vm("other-vm", "sgub1"));

                                                           
    pcv_security_group_unbind_vm_all("ub-vm");
    g_assert_false(pcv_security_group_vm_is_bound("ub-vm"));
                     
    g_assert_true(pcv_security_group_vm_is_bound("other-vm"));

                 
    (void)pcv_security_group_detach_vm("other-vm", "sgub1");
    pcv_vm_vnet_cache_evict("ub-vm");
    pcv_vm_vnet_cache_evict("other-vm");
    g_assert_true(pcv_security_group_delete("sgub1"));
    g_assert_true(pcv_security_group_delete("sgub2"));
}
...
void test_security_group_register(void) {
    ...
    g_test_add_func("/security_group/unbind_all_noop",         test_unbind_all_noop_unbound);
    g_test_add_func("/security_group/unbind_all_removes_all",  test_unbind_all_removes_all_bindings);
}
