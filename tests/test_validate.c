...
static void test_base_image_valid(void) {
    ...
    g_assert_true(pcv_validate_base_image_path("/tmp/cloud.QCOW2"));              
}
...
static void test_base_image_invalid(void) {
    ...
    g_assert_false(pcv_validate_base_image_path("/etc/shadow"));                                
    g_assert_false(pcv_validate_base_image_path("relative/base.qcow2"));            
    g_assert_false(pcv_validate_base_image_path("/pcvpool/../../etc/shadow.img"));            
    g_assert_false(pcv_validate_base_image_path("/var/lib/images/base.iso"));                     
}
...
static void test_network_mode_tenant_unspecified(void) {
    GError *err = NULL;
                                                       
    g_assert_true(pcv_vm_validate_network_mode_tenant(NULL, NULL, &err));
    g_assert_null(err);
    g_assert_true(pcv_vm_validate_network_mode_tenant("", "", &err));
    g_assert_null(err);
}

static void test_network_mode_tenant_valid_modes(void) {
    GError *err = NULL;
    g_assert_true(pcv_vm_validate_network_mode_tenant("bridge", NULL, &err));
    g_assert_null(err);
    g_assert_true(pcv_vm_validate_network_mode_tenant("dpdk", NULL, &err));
    g_assert_null(err);
    g_assert_true(pcv_vm_validate_network_mode_tenant("sriov", NULL, &err));
    g_assert_null(err);
    g_assert_true(pcv_vm_validate_network_mode_tenant("tenant-overlay", "acme", &err));
    g_assert_null(err);
}

static void test_network_mode_tenant_invalid_mode(void) {
    GError *err = NULL;
    g_assert_false(pcv_vm_validate_network_mode_tenant("routed", NULL, &err));
    g_assert_nonnull(err);
    g_error_free(err);
}

static void test_network_mode_tenant_overlay_without_tenant(void) {
    GError *err = NULL;
                                           
    g_assert_false(pcv_vm_validate_network_mode_tenant("tenant-overlay", NULL, &err));
    g_assert_nonnull(err);
    g_error_free(err);

    err = NULL;
    g_assert_false(pcv_vm_validate_network_mode_tenant("tenant-overlay", "", &err));
    g_assert_nonnull(err);
    g_error_free(err);
}

static void test_network_mode_tenant_non_overlay_with_tenant(void) {
    GError *err = NULL;
                                                           
    g_assert_false(pcv_vm_validate_network_mode_tenant("bridge", "acme", &err));
    g_assert_nonnull(err);
    g_error_free(err);

    err = NULL;
                                   
    g_assert_false(pcv_vm_validate_network_mode_tenant(NULL, "acme", &err));
    g_assert_nonnull(err);
    g_error_free(err);
}

static void test_network_mode_tenant_invalid_charset(void) {
    GError *err = NULL;
                                                     
    g_assert_false(pcv_vm_validate_network_mode_tenant("tenant-overlay", "acme corp", &err));
    g_assert_nonnull(err);
    g_error_free(err);

    err = NULL;
    g_assert_false(pcv_vm_validate_network_mode_tenant("tenant-overlay", "acme;rm -rf", &err));
    g_assert_nonnull(err);
    g_error_free(err);
}
...
static void test_iface_name_valid(void) {
    ...
    g_assert_true(pcv_validate_iface_name("a"));                     
...
static void test_iface_name_invalid(void) {
    ...
    g_assert_false(pcv_validate_iface_name(""));                      
    g_assert_false(pcv_validate_iface_name("-x"));                                        
    g_assert_false(pcv_validate_iface_name("eth0;rm"));                
    g_assert_false(pcv_validate_iface_name("eth 0"));              
...
static void test_mac_valid(void) {
    ...
    g_assert_true(pcv_validate_mac("AA:BB:CC:dd:ee:ff"));                   
}

static void test_mac_invalid(void) {
    g_assert_false(pcv_validate_mac(NULL));
    g_assert_false(pcv_validate_mac(""));
    g_assert_false(pcv_validate_mac("52:54:00:00:00"));                             
    g_assert_false(pcv_validate_mac("gg:54:00:00:00:01"));                    
    g_assert_false(pcv_validate_mac("52:54:00:00:00:01 vlan 4095"));           
    g_assert_false(pcv_validate_mac("52540000001"));                          
    g_assert_false(pcv_validate_mac("52-54-00-00-00-01"));                      
}
...
static void test_ip_literal_invalid(void) {
    ...
    g_assert_false(pcv_validate_ip_literal("10.0.0.1/24"));                   
    g_assert_false(pcv_validate_ip_literal("10.0.0.256"));                 
...
static void test_ipv6_prefix_valid(void) {
    ...
    g_assert_true(pcv_validate_ipv6_prefix("::/0"));                        
    g_assert_true(pcv_validate_ipv6_prefix("fd00::1/128"));                   
}
...
static void test_ipv6_prefix_invalid(void) {
    ...
    g_assert_false(pcv_validate_ipv6_prefix("fd00::/129"));              
    g_assert_false(pcv_validate_ipv6_prefix("fd00::"));                  
    g_assert_false(pcv_validate_ipv6_prefix("fd00::/64 foo"));                
    g_assert_false(pcv_validate_ipv6_prefix("gg::/64"));                    
}
...
static void test_l4_proto_invalid(void) {
    ...
    g_assert_false(pcv_validate_l4_proto("TCP"));                    
    g_assert_false(pcv_validate_l4_proto("tcp; drop"));             
    g_assert_false(pcv_validate_l4_proto("sctp"));                   
}
...
void test_validate_register(void) {
    ...
    g_test_add_func("/validate/network_mode_tenant/unspecified",
                     test_network_mode_tenant_unspecified);
    g_test_add_func("/validate/network_mode_tenant/valid_modes",
                     test_network_mode_tenant_valid_modes);
    g_test_add_func("/validate/network_mode_tenant/invalid_mode",
                     test_network_mode_tenant_invalid_mode);
    g_test_add_func("/validate/network_mode_tenant/overlay_without_tenant",
                     test_network_mode_tenant_overlay_without_tenant);
    g_test_add_func("/validate/network_mode_tenant/non_overlay_with_tenant",
                     test_network_mode_tenant_non_overlay_with_tenant);
    g_test_add_func("/validate/network_mode_tenant/invalid_charset",
                     test_network_mode_tenant_invalid_charset);
