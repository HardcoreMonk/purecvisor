...
static void test_net_create_nat_valid(void) {
    ...
    g_assert_true(pcv_validate_network_create_params(
        "pcvbr0", "nat", "10.0.0.1/24", NULL, NULL, NULL, &err));
    g_assert_null(err);
}
...
static void test_net_create_nat_no_cidr(void) {
    ...
    g_assert_false(pcv_validate_network_create_params(
        "pcvbr0", "nat", NULL, NULL, NULL, NULL, &err));
...
static void test_net_create_bridge_valid(void) {
    GError *err = NULL;
    g_assert_true(pcv_validate_network_create_params(
        "pcvbr0", "bridge", NULL, "enp42s0", "dedicated", "dedicated-uplink", &err));
    g_assert_null(err);
}
...
static void test_net_create_bridge_no_phys(void) {
    GError *err = NULL;
    g_assert_false(pcv_validate_network_create_params(
        "pcvbr0", "bridge", NULL, NULL, "dedicated", "dedicated-uplink", &err));
    g_assert_nonnull(err);
    g_error_free(err);
}
...
static void test_net_create_invalid_mode(void) {
    GError *err = NULL;
    g_assert_false(pcv_validate_network_create_params(
        "pcvbr0", "invalid_mode", "10.0.0.1/24", NULL, NULL, NULL, &err));
    g_assert_nonnull(err);
    g_error_free(err);
}
...
static void test_net_create_invalid_bridge_name(void) {
    GError *err = NULL;
    g_assert_false(pcv_validate_network_create_params(
        NULL, "nat", "10.0.0.1/24", NULL, NULL, NULL, &err));
    g_assert_nonnull(err);
    g_error_free(err);
}
...
static void test_net_create_null_mode_defaults_nat(void) {
    ...
    g_assert_true(pcv_validate_network_create_params(
        "br0", NULL, "10.0.0.1/24", NULL, NULL, NULL, &err));
    g_assert_null(err);
}
...
static void test_net_create_isolated_valid(void) {
    GError *err = NULL;
    g_assert_true(pcv_validate_network_create_params(
        "isol0", "isolated", "192.168.100.1/24", NULL, NULL, NULL, &err));
    g_assert_null(err);
}
...
static void test_net_create_routed_valid(void) {
    GError *err = NULL;
    g_assert_true(pcv_validate_network_create_params(
        "rout0", "routed", "172.16.0.1/16", NULL, NULL, NULL, &err));
    g_assert_null(err);
}

static void test_net_create_bridge_requires_ack(void) {
    GError *err = NULL;
    g_assert_false(pcv_validate_network_create_params(
        "pcvbr0", "bridge", NULL, "enp42s0", "dedicated", NULL, &err));
    g_assert_nonnull(err);
    g_clear_error(&err);
    g_assert_false(pcv_validate_network_create_params(
        "pcvbr0", "bridge", NULL, "enp42s0", "dedicated", "yes", &err));
    g_assert_nonnull(err);
    g_clear_error(&err);
}
...
static void test_net_create_bridge_shared_and_legacy(void) {
    GError *err = NULL;
    g_assert_true(pcv_validate_network_create_params(
        "pcvbr0", "bridge", NULL, "enp42s0", "shared", "shared-uplink", &err));
    g_assert_no_error(err);
                                                                 
    g_assert_true(pcv_validate_network_create_params(
        "pcvbr0", "bridge", NULL, "enp42s0", NULL, "dedicated-uplink", &err));
    g_assert_no_error(err);
}

static void test_net_create_bridge_rejects_uplink_ack_mismatch(void) {
    GError *err = NULL;
    g_assert_false(pcv_validate_network_create_params(
        "pcvbr0", "bridge", NULL, "enp42s0", "shared", "dedicated-uplink", &err));
    g_assert_nonnull(err);
    g_clear_error(&err);
    g_assert_false(pcv_validate_network_create_params(
        "pcvbr0", "bridge", NULL, "enp42s0", "dedicated", "shared-uplink", &err));
    g_assert_nonnull(err);
    g_clear_error(&err);
}
...
void test_validate_ext_register(void) {
    ...
    g_test_add_func("/validate_ext/net_create/bridge_requires_ack",test_net_create_bridge_requires_ack);
    g_test_add_func("/validate_ext/net_create/bridge_rejects_cidr",test_net_create_bridge_rejects_cidr);
    g_test_add_func("/validate_ext/net_create/bridge_shared_legacy",test_net_create_bridge_shared_and_legacy);
    g_test_add_func("/validate_ext/net_create/bridge_ack_mismatch",test_net_create_bridge_rejects_uplink_ack_mismatch);
