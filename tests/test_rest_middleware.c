...
void pcv_test_capture_proxy_consumers(const gchar *forwarded_ip,
                                      gchar **audit_address,
                                      gchar **rate_key);
...
static void test_forwarded_clients_have_distinct_rate_buckets(void) {
    g_autofree gchar *first_ip = NULL;
    g_autofree gchar *second_ip = NULL;
    g_autofree gchar *first_key = NULL;
    g_autofree gchar *second_key = NULL;

    pcv_test_capture_proxy_consumers(
        "198.51.100.20", &first_ip, &first_key);
    pcv_test_capture_proxy_consumers(
        "198.51.100.21", &second_ip, &second_key);

    g_assert_cmpstr(first_key, !=, second_key);
    g_assert_true(g_str_has_prefix(first_key, "198.51.100.20:"));
    g_assert_true(g_str_has_prefix(second_key, "198.51.100.21:"));
}
...
void test_rest_middleware_register(void) {
    ...
    g_test_add_func("/middleware/forwarded_clients_have_distinct_rate_buckets", test_forwarded_clients_have_distinct_rate_buckets);
