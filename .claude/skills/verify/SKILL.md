...
4. **evaluate 컨텍스트 파괴 재시도**: 로그인 직후·`navigateTo()` 직후 evaluate가 해시 전환과 경합해 "Execution context was destroyed"로 죽는다. 모든 evaluate를 재시도 래퍼(파괴 메시지면 400ms 대기 후 최대 4회)로 감싼다. `navigateTo`는 `p.evaluate(n => navigateTo(n), tab).catch(파괴무시)` 후 `waitForTimeout(350)`.
   **로그인 완료 검증 필수**: `PCV.state.currentTab`은 로그인 없이도 모듈 로드 시점에 참이 된다 — 이 조건만 기다리면 **비로그인 스윕**이 소리 없이 통과한다(2026-07-08 실측: fill/press 실입력이 유실돼 전 탭을 비인증으로 캡처, 목 서버가 무인증 응답이라 콘텐츠도 그럴듯하게 렌더됨). 반드시 `#login-page`의 `style.display === 'none'`을 기다리고, 실패 시 evaluate로 input value 설정+Enter KeyboardEvent dispatch(+로그인 버튼 .click()) 폴백 후 재확인.
