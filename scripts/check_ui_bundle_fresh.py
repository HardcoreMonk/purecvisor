...
# This non-mutating check rebuilds the expected bytes in memory and compares
# them with the committed bundle plus service-worker cache name.
# It catches the common failure where a module edit is committed without running
# scripts/bundle-ui.sh.
"""
Verify that generated UI artifacts match ui/modules/*.js.

This is intentionally non-mutating. If it fails, regenerate with:
...
def expected_bundle_bytes(order: list[str]) -> bytes:
    chunks: list[bytes] = [
        "/* PureCVisor UI bundle — auto-generated by scripts/bundle-ui.sh */\n".encode("utf-8"),
        "/* DO NOT EDIT directly — modify ui/modules/*.js instead */\n".encode("utf-8"),
    ]
    for module in order:
