...
THEMES = ("supanova", "supanova-cyan", "supanova-hicontrast", "supanova-mockup")
...
def main() -> int:
    ...
    for theme in THEMES:
        ...
        require(f"{theme} is allowed in ui/index.html", quoted in index_html or double_quoted in index_html)
        require(f"{theme} is allowed in ui/app.js", quoted in app_js)
        if theme != "supanova":
            require(f"{theme} has a theme module id", f"id: '{theme}'" in theme_js)
                                                                          
            require(f"{theme} is present in generated bundle", quoted in app_bundle or double_quoted in app_bundle)
