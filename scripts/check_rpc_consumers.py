...
# check_rbac_policies.py 와 동일 — g_rpc_routes 해시 등록 메서드
ROUTE_RE = re.compile(
    r'g_hash_table_insert\s*\(\s*g_rpc_routes\s*,\s*"([a-z][a-z0-9_.]+)"'
)
# 특수 인라인 디스패치 — 해시 등록이 아니라 g_strcmp0(method, "x.y") 로 직접
# 분기되는 메서드(예: vm.create). 이것도 "등록"으로 간주해야 오검출을 막는다.
SPECIAL_RE = re.compile(
    r'g_strcmp0\s*\(\s*method\s*,\s*"([a-z][a-z0-9_.]+)"'
)


def strip_comments(s: str) -> str:
    """C/JS 주석(/* */, //) 제거 — 문서 예시 안의 메서드 리터럴 오검출 방지."""
...
# FE generic /rpc: {jsonrpc:'2.0', ... method:'x.y' ...} — jsonrpc 근접으로 스코프해
# 일반 method: 키(HTTP 'GET' 등)의 오검출을 배제한다. 양방향 순서 모두 매칭.
FE_RE_A = re.compile(
