...
from check_dead_exports import find_dead  # noqa: E402
...
def test_sec1_class_detected_synthetic():
    ...
    # 대조군(배선됨): 정의 + 세션 강제 경로에서 호출 = 2회 출현 → dead 아님.
    c_wired = (
    ...
    # SEC-1 재현(미배선): 정의만 있고 아무도 호출 안 함(무동작) → dead로 탐지되어야.
    c_unwired = (
        "gboolean pcv_session_is_revoked(const gchar *sid){ return check(sid); }\n"
...
def test_handlers_not_false_flagged():
    ...
    # pcv_rbac_check_permission: rest_server.c:4073에서 실호출(권한 검사 경로).
    assert "pcv_rbac_check_permission" not in dead, "실사용 함수 오탐(pcv_rbac_check_permission)"
    # pcv_rest_server_start: main.c:957에서 실호출(REST 서버 기동).
