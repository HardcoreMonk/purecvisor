...
from check_ws_token_url import scan_text, TARGET  # noqa: E402

GATE = Path(__file__).resolve().parent.parent / "check_ws_token_url.py"

# URL-query 토큰 인증(제거 대상) — 반드시 위반으로 잡혀야 한다.
URL_AUTH_BLOCK = '''
...
# 메시지 경로 인증(유지 대상) — URL 쿼리를 읽지 않으므로 위반이 아니다.
MSG_AUTH_BLOCK = '''
...
# 시정 후 deprecated URL 토큰 경고(인증 아님) — URL+"token" 을 읽지만 verify 없음.
WARN_ONLY_BLOCK = '''
...
def test_reverted_source_fails():
    ...
    # 추출 직후에 URL 토큰 검증(=인증)을 다시 삽입 → URL-query 인증 부활.
