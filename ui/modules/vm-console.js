...
async function renderConsole(b, v) {
  ...
  b.appendChild(el('div', { style: 'background:#000;border:1px solid var(--border);border-radius:var(--r);min-height:500px;height:100%;display:flex;flex-direction:column;position:relative', id: 'vnc-frame' }, vncHtml));
  try {
    ...
    if (port && v.state === 'running') {
      ...
      frame.appendChild(el('div', { class: 'p-12', style: 'display:flex;flex-direction:column;height:100%;box-sizing:border-box' },
        el('div', { class: 'flex gap-12 items-center mb-12 flex-wrap', 'data-role': 'OPERATOR,ADMIN' },
          HN.statusPill('ok', 'VNC ' + t('vnc.connected')),
        ...
        el('div', { id: 'vnc-placeholder', style: 'background:#111;flex:1;min-height:0;border-radius:var(--r);display:flex;align-items:center;justify-content:center;color:var(--fg2)' },
      ...
      frame.appendChild(el('div', { class: 'text-center color-muted p-20' },
        ...
        el('button', { class: 'btn mt-12', onclick: 'showVnc()', 'data-role': 'OPERATOR,ADMIN' }, _L('VNC 확인', 'Check VNC'))));
    }
  } catch (e) {
    const frame = document.getElementById('vnc-frame');
    PCV.uxlib.clearEl(frame);
    frame.appendChild(el('div', { class: 'text-center color-muted p-20' },
      ...
      el('button', { class: 'btn mt-8', onclick: 'showVnc()', 'data-role': 'OPERATOR,ADMIN' }, t('vnc.manual_check'))));
  }
  _applyVmSurfaceRoles();
}
...
function openNoVNC(addr, port) {
  ...
  frame.appendChild(el('div', { id: 'vnc-container', style: 'width:100%;flex:1;min-height:0;background:#000;border-radius:var(--r);position:relative' },
...
document.addEventListener('fullscreenchange', function() {
  if (!document.fullscreenElement && document.getElementById('vnc-container')) vncFitWindow();
});
...
async function renderSnapshots(b, v) {
  ...
  b.appendChild(el('div', null,
    el('div', { class: 'justify-between items-center mb-12' },
      ...
      el('div', { class: 'flex gap-6' },
        el('button', { class: 'btn btn-g', onclick: 'takeSnap()', 'data-role': 'OPERATOR,ADMIN' }, '+ ' + t('btn.create')),
        el('button', { class: 'btn btn-r', onclick: "snapDeleteAll('" + escapeAttr(v.name) + "')", 'data-role': 'ADMIN' }, '🗑 Delete All'))),
    el('div', { id: 'stree' },
      el('span', { class: 'spinner' }),
      ' ' + t('loading'))));
  _applyVmSurfaceRoles();
  try {
    ...
    if (!snaps.length) { PCV.uxlib.setMsg('stree', 'muted', { tag: 'p' }, t('snap.none')); _applyVmSurfaceRoles(); return; }
...
async function snapCreateExec() {
  ...
  try {
    ...
    closeModal();
    if (PCV.ui.navGen() === _navGen) renderSnapshots(PCV.ui.renderTarget(), v);
...
async function showVmCompare() {
  ...
  var bodyRows = props.map(function(p) {
    ...
    selected.forEach(function(v) {
      var val = v[p.key];
      if (p.key === 'state') val = HN.statusPill(val === 'running' ? 'ok' : 'crit', String(val || '?').toUpperCase());
