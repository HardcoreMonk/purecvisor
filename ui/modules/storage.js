...
async function renderStorage(b) {
  ...
  try {
    ...
    if (p && p.error) throw new Error(p.error.message || _L('스토리지 풀 조회 실패', 'Unable to load storage pools'));
    const pl = unwrapList(p);
    var parts = [HN.pagehead({
      title: _L('스토리지 운영 개요', 'Storage operations overview'),
      desc: _L('풀 상태, 용량 계획, Zvol 작업을 한 화면에서 정리합니다.', 'Review pool health, capacity planning, and zvol operations in one place.'),
      actions: [el('button', { class: 'btn btn-g', onclick: 'showPoolCreate()', 'data-role': 'ADMIN' }, '+ ' + _L('풀 생성', 'Create pool'))]
    })];
    if (pl.length === 0) {
      parts.push(el('div', { class: 'empty-state', style: 'text-align:center;padding:40px 20px' },
        ...
        el('button', { class: 'btn btn-g', onclick: 'showPoolCreate()', 'data-role': 'ADMIN' }, '+ ' + _L('풀 생성', 'Create pool'))));
    }
    if (pl.length > 0) {
      ...
      parts.push(el('div', { class: 'sg grid-3' },
        ...
        HN.card(_L('사용 중 용량', 'Used capacity'), [
          ...
          HN.gauge({ value: Math.min(totalPct, 100), warn: 80, crit: 90, inline: true }),
    ...
    pl.forEach(v => {
      ...
      parts.push(HN.card(
        ['💾 ' + v.name + ' ', HN.statusPill(v.health === 'ONLINE' ? 'ok' : 'crit', v.health || '?')],
        [
          HN.row(_L('총 용량', 'Total size'), fmtBytes(sz)),
          HN.row(_L('사용량', 'Used'), fmtBytes(us)),
          HN.row(_L('건강 상태', 'Health'), v.health || '-'),
          HN.gauge({ value: Math.min(pct, 100), warn: 70, crit: 90, inline: true }),
    ...
    parts.push(el('div', { class: 'ops-section-heading' },
      ...
      el('div', { class: 'flex gap-8 ops-action-row', 'data-role': 'ADMIN' },
    ...
    var tbody = el('tbody', null, zl.map(function(v) {
      ...
      return el('tr', { class: selected ? 'multi-selected' : null, oncontextmenu: "event.preventDefault();zvolCtxMenu(event,'" + escapeAttr(vn) + "')" },
        ...
        el('td', { 'data-role': 'ADMIN' }, el('button', { class: 'btn btn-r', style: 'font-size:10px;padding:3px 8px', onclick: "zvolDel('" + escapeAttr(vn) + "')" }, t('btn.delete'))));
    }));
    ...
    if(_DEBUG) console.warn('z:', e.message);
    PCV.uxlib.renderLoadError(b, {
      title: _L('스토리지 운영 개요', 'Storage operations overview'),
      message: e.message,
      retry: function() { renderStorage(b); }
    });
  }
}
...
async function doPoolCreate() {
  ...
  try {
    ...
    closeModal(); if (PCV.ui.navGen() === _navGen) renderStorage(PCV.ui.renderTarget());
  ...
  destroyConfirm({
    ...
    onConfirm: async function () {
      try {
        ...
        if (PCV.ui.navGen() === _navGen) renderStorage(PCV.ui.renderTarget());
...
async function doZvol() {
  ...
  try { var _navGen = PCV.ui.navGen(); const r = await fetchPost(EP.STORAGE_ZVOLS(), { name: name, size_gb: size }); if (r && r.error) { toast(r.error.message || t('error'), false); return; } toast(t('stg.zvol_created')); addEvt(t('stg.zvol_created')); closeModal(); if (PCV.ui.navGen() === _navGen) renderStorage(PCV.ui.renderTarget()); } catch (e) { toast(e.message, false); }
  finally { if (_btn && _btn.tagName === 'BUTTON') { _btn.disabled = false; _btn.removeAttribute('aria-busy'); } }
}
...
async function doZvolDel(name) { const c = document.getElementById('del-zvol-confirm')?.value; if (c !== name) { toast(t('vm.name_mismatch'), false); return; }
  ...
  try { pf.style.width = '50%';
    ...
    pf.style.width = '100%'; ps.textContent = '✅ ' + t('stg.zvol_destroyed'); toast(t('stg.zvol_destroyed')); addEvt(t('stg.zvol_destroyed') + ': ' + name); setTimeout(() => { closeModal(); if (PCV.ui.navGen() === _navGen) renderStorage(PCV.ui.renderTarget()); }, 1500);
  ...
  try {
    ...
    var tbody = mk('tbody', null, pools.map(function(p) {
      ...
      return mk('tr', null,
        ...
        mk('td', null, HN.gauge({ value: parseFloat(usedPct), warn: 80, crit: 90, inline: true }), mk('span', { class: 'text-xs' }, usedPct + '%')),
        ...
        mk('td', null, HN.statusPill(severity.status, severity.label)));
    }));
    PCV.uxlib.clearEl(el);
...
function _forecastSeverity(daysToFull) {
  if (daysToFull <= 0) return { color: 'var(--green)', label: 'Stable', status: 'ok' };
  if (daysToFull < 30) return { color: 'var(--red)', label: 'Critical', status: 'crit' };
  if (daysToFull < 60) return { color: 'var(--yellow)', label: 'Warning', status: 'warn' };
  return { color: 'var(--green)', label: 'Healthy', status: 'ok' };
}

                                                              
                                                
function _iscsiStateStatus(state) {
  var st = String(state || '?').toLowerCase();
  return ['active', 'online', 'up', 'running', 'ok'].indexOf(st) >= 0 ? 'ok' : 'warn';
}
...
async function renderIscsi(b) {
  ...
  try {
    ...
    if (!Array.isArray(l) || l.length === 0) {
      ...
      var tbody = el('tbody', null, l.map(function(tgt) {
        return el('tr', null,
          ...
          el('td', null, tgt.lun || '-'),
          el('td', null, tgt.size || '-'),
          el('td', null, HN.statusPill(_iscsiStateStatus(tgt.state), String(tgt.state || '?').toUpperCase())));
      }));
      body = el('table', null,
    ...
    clearEl(b);
    b.appendChild(frag(HN.pagehead({ title: 'iSCSI Targets' }), body));
  } catch (e) {
    clearEl(b);
    b.appendChild(frag(HN.pagehead({ title: 'iSCSI Targets' }), el('p', { class: 'color-muted' }, 'Failed to load')));
  }
}
...
async function renderBackup(b) {
  ...
  clearEl(b);
  b.appendChild(frag(
    HN.pagehead({ title: _L('백업 관리', 'Backup Management') }),
                     
    el('div', { class: 'hc mb-14' },
      el('h4', { role: 'heading', 'aria-level': '3' }, _L('백업 정책', 'Backup Policies')),
      el('div', { class: 'flex gap-8 mb-8' },
        el('button', { class: 'btn btn-g', onClick: function(){ window.backupAddPolicy(); }, 'data-role': 'ADMIN' }, _L('정책 추가', 'Add Policy'))
      ),
      el('div', { id: 'backup-policies', class: 'skeleton-box', style: 'min-height:100px' })
    ),
                 
    el('div', { class: 'hc mb-14' },
      el('h4', { role: 'heading', 'aria-level': '3' }, _L('스냅샷 히스토리', 'Snapshot History')),
      el('div', { class: 'flex gap-8 mb-8' },
    ...
    el('div', { class: 'hc mb-14' },
      el('h4', { role: 'heading', 'aria-level': '3' }, _L('복원', 'Restore')),
      el('p', { class: 'stat-label' }, _L('VM의 스냅샷을 선택하여 롤백합니다.', 'Select a VM snapshot to rollback.')),
      el('div', { class: 'flex gap-8 mt-8', 'data-role': 'ADMIN' },
        ...
        el('button', { class: 'btn btn-r', onClick: function(){ window.backupRestore(); }, 'data-role': 'ADMIN' }, _L('롤백', 'Rollback'))
      )
    )
  ));
  if (typeof applyRoleVisibility === 'function') applyRoleVisibility(window.currentUser && window.currentUser.role);
      ...
      if (policies.length === 0) {
        ...
        policies.forEach(function(p) {
          tbody.appendChild(el('tr', null,
            ...
            el('td', null, p.enabled ? HN.statusPill('ok', 'ON') : HN.statusPill('idle', 'OFF')),
            el('td', null, el('button', { class: 'btn btn-sm btn-r', onClick: function(){ window.backupDeletePolicy(p.vm_name); }, 'data-role': 'ADMIN' }, _L('삭제', 'Del')))
          ));
        });
        clearEl(pe);
...
function backupAddPolicy() {
  ...
  showModal([
    ...
    el('div', { class: 'text-right mt-12' },
      el('button', { class: 'btn btn-g', onclick: 'doBackupAddPolicy()', 'data-role': 'ADMIN' }, _L('추가', 'Add')),
      ' ',
...
async function doBackupAddPolicy() {
  ...
  try {
    var r = await fetchPost(EP.RPC(), { jsonrpc:'2.0', method:'backup.policy.set', params:{ vm_name:vm, interval_hours:interval, retention_count:retention }, id:'bps1' });
...
async function backupRestore() {
  ...
  try {
    var r = await fetchPost(EP.RPC(), { jsonrpc:'2.0', method:'backup.restore', params:{ vm_name:vm, snapshot_name:snap }, id:'br1' });
    if (r.error) { toast(r.error.message || 'Failed', false); return; }
...
async function backupDeletePolicy(vm) {
  var ok = await customConfirm(_L('백업 정책 삭제', 'Delete Backup Policy'),
    _L('정책 ', 'Delete policy ') + "'" + vm + "'" +
    _L('을(를) 삭제합니다. 이후 이 정책에 따른 자동 백업이 생성되지 않습니다.', '? Future automatic backups under this policy will no longer be created.'));
  if (!ok) return;
  try {
...
async function zvolBulkDelete() {
  ...
  for (var i = 0; i < names.length; i++) {
    try {
      ...
      if (r && r.error) { failed.push(names[i] + ': ' + (r.error.message || '')); failedNames.push(names[i]); }
    }
    catch (e) { failed.push(names[i] + ': ' + e.message); failedNames.push(names[i]); }
  }
  window._zvolSel.clear();
  failedNames.forEach(function(n) { window._zvolSel.add(n); });
  if (failed.length === 0) {
...
function zvolCtxMenu(ev, name) {
  var isAdmin = String(window.currentUser && window.currentUser.role || '').toUpperCase() === 'ADMIN';
  if (typeof showCtxMenu === 'function') {
    var items = [{ label: '⚙ ' + _L('선택', 'Select'), fn: function(){ zvolToggleSel(name, !window._zvolSel.has(name)); renderStorage(PCV.ui.renderTarget()); } }];
    if (isAdmin) items.unshift({ label: '🗑 ' + t('btn.delete'), fn: function(){ zvolDel(name); } });
    showCtxMenu(ev, items);
  } else {
                                                    
    if (isAdmin && window.confirm(_L('삭제하시겠습니까? ', 'Delete? ') + name)) zvolDel(name);
  }
}
