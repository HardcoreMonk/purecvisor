...
async function renderContainers(b) {
  ...
  const pagehead = () => HN.pagehead({
    ...
    actions: [el('button', { class: 'btn btn-g', onclick: 'showCtrCreate()', 'data-role': 'OPERATOR,ADMIN' }, '+ ' + t('ctr.new'))]
  });
  try {
    const c = await fetchGet(EP.CTR_LIST());
                                                             
                                                       
    if (c && c.error) throw new Error(c.error.message || _L('컨테이너 목록 조회 실패', 'Unable to load containers'));
    const l = unwrapList(c);
    if (l.length === 0 && typeof emptyStatePro === 'function') {
      PCV.uxlib.clearEl(b);
      b.appendChild(pagehead());
    ...
    const listItems = l.map(v => {
      ...
      return el('div', {
        onclick: "selCtr='" + escapeAttr(v.name) + "';ctrTab='summary';renderContainers(PCV.ui.renderTarget())",
    ...
    const sidebar = el('div', { style: 'min-width:220px;max-width:220px;border-right:1px solid var(--border);overflow-y:auto;padding:8px' },
      el('div', { class: 'justify-between items-center mb-8' },
        ...
        el('div', { class: 'flex gap-6 items-center' },
          HN.statusPill('idle', String(l.length)),
        el('button', { class: 'btn btn-g', onclick: 'showCtrCreate()', style: 'font-size:11px;padding:4px 10px', 'data-role': 'OPERATOR,ADMIN' }, '+ ' + t('ctr.new')))),
      listItems);
    ...
    if (cv) {
      ...
      const actions = [];
      if (!on) actions.push(el('button', { class: 'btn btn-g text-12 px-12 py-4', onclick: "ctrA('" + escapeAttr(cv.name) + "','start')", 'data-role': 'OPERATOR,ADMIN' }, '▶ ' + t('power.start')));
      if (on) {
        actions.push(el('button', { class: 'btn btn-r text-12 px-12 py-4', onclick: "ctrA('" + escapeAttr(cv.name) + "','stop')", 'data-role': 'OPERATOR,ADMIN' }, '■ ' + t('power.stop')));
        actions.push(el('button', { class: 'btn text-12 px-12 py-4', onclick: "ctrReboot('" + escapeAttr(cv.name) + "')", 'data-role': 'OPERATOR,ADMIN' }, '↻ Reboot'));
      }
      actions.push(el('button', { class: 'btn btn-r text-12 px-12 py-4', onclick: "ctrDel('" + escapeAttr(cv.name) + "')", 'data-role': 'ADMIN' }, '🗑 ' + t('btn.delete')));
      detailKids.push(el('div', { class: 'justify-between items-center', style: 'padding:10px 14px;border-bottom:1px solid var(--border)' },
        el('div', null,
          ...
          ' ',
          HN.statusPill(on ? 'ok' : 'idle', String(cv.state || '?').toUpperCase())),
        el('div', { class: 'flex gap-4' }, actions)));
      const tabs = ['summary', 'console', 'resources', 'network', 'dns', 'options', 'snapshots', 'notes', 'tasks'];
      detailKids.push(el('div', { class: 'flex ctr-tabs', role: 'tablist', 'aria-label': _L('컨테이너 상세', 'Container details') },
        tabs.map(t2 => el('button', {
          type: 'button', role: 'tab', id: 'ctr-tab-' + t2, 'aria-controls': 'ctr-tab-content',
          'aria-selected': ctrTab === t2 ? 'true' : 'false', tabindex: ctrTab === t2 ? '0' : '-1',
          onclick: "ctrTab='" + t2 + "';renderContainers(PCV.ui.renderTarget())",
          style: 'padding:9px 14px;font-size:13px;cursor:pointer;white-space:nowrap;background:none;border-top:0;border-right:0;border-left:0;border-bottom:2px solid ' + (ctrTab === t2 ? 'var(--accent)' : 'transparent') + ';color:' + (ctrTab === t2 ? 'var(--accent)' : 'var(--fg2)') + ';font-weight:' + (ctrTab === t2 ? '600' : '400') + ';transition:color .15s'
        }, t2.charAt(0).toUpperCase() + t2.slice(1)))));
      detailKids.push(el('div', { id: 'ctr-tab-content', role: 'tabpanel', 'aria-labelledby': 'ctr-tab-' + ctrTab, style: 'padding:14px;flex:1' }));
    } else {
    ...
    b.appendChild(el('div', { style: 'display:flex;flex-direction:column;height:100%;min-height:0' },
      ...
      el('div', { style: 'display:flex;gap:0;height:100%;min-height:0' }, sidebar, detail)));
    ...
    PCV.uxlib.renderLoadError(b, {
      title: _L('컨테이너', 'Containers'),
      message: e.message,
      retry: function() { renderContainers(b); }
    });
  }
}
...
async function ctrRenderTab(tb, cv) {
  ...
  if (ctrTab === 'summary') {
    ...
    if (on) { const cmds = [['hostname', 'hostname'], ['uptime', 'uptime -p'], ['nproc', 'nproc'], ['kernel', 'uname -r']];
      for (const [k, c] of cmds) { try { const r = await fetchPost(EP.CTR_EXEC(n), { command: c }); if (r && r.error) { info[k] = _L('조회 실패', 'Fetch failed'); continue; } info[k] = unwrapData(r).output?.trim() || ''; } catch (e) { info[k] = _L('조회 실패', 'Fetch failed'); if(_DEBUG) console.warn('ctrRenderTab:', e.message); } } }
    ...
    tb.appendChild(frag(
      HN.grid(4,
        ...
        HN.card('CPU', [
          ...
          HN.gauge({ value: cpu, warn: 80, crit: 95, inline: true }),
          el('div', { class: 'stat-label' }, (info.nproc || '?') + ' cores')
        ]),
        HN.card('Memory', [
          el('div', { class: 'stat-md' }, mem_l > 0 ? mem_p.toFixed(1) + '%' : mem_u.toFixed(0) + ' MB'),
          mem_l > 0 ? HN.gauge({ value: mem_p, warn: 80, crit: 95, inline: true }) : null
        ]),
        HN.card('Network', [
    ...
    tb.appendChild(
      el('div', { style: 'background:var(--bg);border:1px solid var(--border);border-radius:var(--r);padding:0;font-family:monospace;display:flex;flex-direction:column;height:100%' },
        ...
        el('div', { class: 'flex', style: 'border-top:1px solid var(--border)', 'data-role': 'ADMIN' },
    ...
    if (on) { for (const [k, c] of [['cpu', 'nproc'], ['mem', 'free -h | head -2'], ['disk', 'df -h / 2>/dev/null | tail -1'], ['procs', 'ps aux --no-headers 2>/dev/null | wc -l']]) { try { const r = await fetchPost(EP.CTR_EXEC(n), { command: c }); if (r && r.error) { info[k] = _L('조회 실패', 'Fetch failed'); continue; } info[k] = unwrapData(r).output?.trim() || '-'; } catch (e) { info[k] = _L('조회 실패', 'Fetch failed'); } } }
    ...
    tb.appendChild(frag(
      ...
      el('div', { class: 'sg grid-2' },
        HN.card('Set CPU Limit', [
          ...
          el('button', { class: 'btn btn-g mt-8', onclick: "ctrSetLimits('" + n + "','cpu')", 'data-role': 'ADMIN' }, 'Apply CPU Limit')
        ]),
        HN.card('Set Memory Limit', [
          ...
          el('button', { class: 'btn btn-g mt-8', onclick: "ctrSetLimits('" + n + "','mem')", 'data-role': 'ADMIN' }, 'Apply Memory Limit')
        ]))
    ));
    ...
    tb.appendChild(frag(
      el('h3', { class: 'section-title-md' }, 'Network Interfaces ',
        el('button', { class: 'btn btn-g', onclick: "ctrNicAdd('" + n + "')", style: 'font-size:10px;margin-left:8px', 'data-role': 'ADMIN' }, '+ Add NIC')),
      ...
      el('div', { class: 'sg grid-2' },
        HN.card('Set Bandwidth Limit', [
          ...
          el('button', { class: 'btn btn-g mt-8', onclick: "ctrSetBandwidth('" + n + "')", 'data-role': 'ADMIN' }, 'Apply QoS')
        ]),
      ...
      if (nicEl) {
        if (nics.length === 0) { PCV.uxlib.setMsg(nicEl, null, { tag: 'p', cls: 'color-muted' }, 'No NICs configured'); }
          ...
          const rows = nics.map(nc => el('tr', null,
            ...
            el('td', null, HN.statusPill('idle', nc.bridge || '-')),
            el('td', { class: 'text-xs' }, nc.hwaddr || 'auto'),
            el('td', { class: 'color-accent' }, nc.ipv4 || '-'),
            el('td', null,
              nc.name !== 'eth0'
                ? el('button', { class: 'btn btn-r', onclick: "ctrNicDel('" + n + "','" + nc.name + "')", style: 'font-size:9px;padding:2px 6px', 'data-role': 'ADMIN' }, 'Remove')
    ...
    let dns = '', dnsErr = false;
    if (on) { try { const r = await fetchPost(EP.CTR_EXEC(n), { command: 'cat /etc/resolv.conf 2>/dev/null' }); if (r && r.error) { dnsErr = true; } else { dns = unwrapData(r).output || ''; } } catch (e) { dnsErr = true; if(_DEBUG) console.warn('dns:', e.message); } }
    clearEl(tb);
    tb.appendChild(frag(
      ...
      HN.card('Resolver Configuration',
        on
          ? [
              el('pre', { style: 'font-size:11px;color:var(--fg);margin:8px 0;white-space:pre-wrap;background:var(--bg);padding:10px;border-radius:4px;border:1px solid var(--border)' }, dnsErr ? _L('조회 실패', 'Fetch failed') : dns),
              el('div', { class: 'mt-8' },
                el('div', { class: 'fr' },
                  el('label', { for: 'dns-ns' }, 'Add Nameserver'),
                  el('input', { id: 'dns-ns', placeholder: '8.8.8.8', class: 'flex-1' }),
                  el('button', { class: 'btn btn-g', onclick: "ctrDnsAdd('" + n + "')", style: 'margin-left:6px', 'data-role': 'ADMIN' }, 'Add')))
            ]
    ...
    tb.appendChild(frag(
      el('h3', { class: 'section-title-md' }, t('vm.snapshot') + ' ',
        el('button', { class: 'btn btn-g', onclick: "ctrSnapCreate('" + n + "')", style: 'font-size:10px;margin-left:8px', 'data-role': 'ADMIN' }, '+ ' + t('btn.create'))),
    ...
    try {
      ...
      sl.forEach(s => { const sn = typeof s === 'string' ? s : (s.name || s); tbodyKids.push(el('tr', null,
        el('td', null, sn),
        el('td', null,
          el('button', { class: 'btn text-9', onclick: "ctrSnapRb('" + n + "','" + sn + "')", 'data-role': 'ADMIN' }, 'Rollback'),
          ' ',
          el('button', { class: 'btn btn-r text-9', onclick: "ctrSnapDel('" + n + "','" + sn + "')", 'data-role': 'ADMIN' }, t('btn.delete'))))); });
...
async function ctrDnsAdd(n) { const ns = document.getElementById('dns-ns')?.value; if (!ns) return;
  try { var _navGen = PCV.ui.navGen(); const r = await fetchPost(EP.CTR_EXEC(n), { command: 'echo "nameserver ' + ns + '" >> /etc/resolv.conf' }); if (r && r.error) { toast(r.error.message || t('error'), false); return; } toast('Nameserver added'); ctrTab = 'dns'; if (PCV.ui.navGen() === _navGen) renderContainers(PCV.ui.renderTarget()); } catch (e) { toast(e.message, false); } }
...
async function ctrReboot(n) {
  ...
  try {
    ...
    setTimeout(function() { closeModal(); if (PCV.ui.navGen() === _navGen) renderContainers(PCV.ui.renderTarget()); }, 2000);
  } catch (e) {
...
async function ctrSnapCreate(n) { var _navGen = PCV.ui.navGen(); var s = await showInputModal(t('snap.name_prompt') || 'Snapshot name', t('snap.name_prompt') || 'Name', 'snap-' + Date.now()); if (!s) return;
  ...
  try {
    ...
    setTimeout(function() { closeModal(); ctrTab = 'snapshots'; if (PCV.ui.navGen() === _navGen) renderContainers(PCV.ui.renderTarget()); }, 1500);
  } catch (e) {
    if (pf) { pf.style.width = '100%'; pf.style.background = 'var(--red)'; }
...
async function ctrSnapRb(n, s) { var _navGen = PCV.ui.navGen(); if (!await customConfirm('Rollback', n + ' → ' + s + '?')) return;
  var el = PCV.uxlib.el;
  showModal([
    el('h2', null, '⏪ Rollback'),
    el('p', null, el('b', { class: 'color-accent' }, n + '@' + s)),
  ...
  try {
    ...
    setTimeout(function() { closeModal(); if (PCV.ui.navGen() === _navGen) renderContainers(PCV.ui.renderTarget()); }, 1500);
  } catch (e) {
    if (pf) { pf.style.width = '100%'; pf.style.background = 'var(--red)'; }
    if (ps) PCV.uxlib.setMsg(ps, null, null, '❌ ' + e.message); toast(e.message, false);
  }
...
async function ctrSnapDel(n, s) { var _navGen = PCV.ui.navGen(); if (!await customConfirm(t('btn.delete'), s + '?')) return;
  var el = PCV.uxlib.el;
  showModal([
...
async function ctrExec(n) {
  try { var _navGen = PCV.ui.navGen(); const c = await fetchGet(EP.CTR_LIST()); const l = unwrapList(c); const ct = l.find(x => x.name === n);
    if (!ct || ct.state !== 'RUNNING') { toast(n + ' is not running', false); return; } } catch (e) { if(_DEBUG) console.warn('c:', e.message); }
  selCtr = n; ctrTab = 'console'; if (PCV.ui.navGen() === _navGen) renderContainers(PCV.ui.renderTarget());
}
...
function showCtrCreate() {
  ...
  var dlg = PCV.modalCore.currentDialog();
  if (dlg) dlg.classList.add('modal-wide');
...
async function doCtrCreate() {
  ...
  try {
    ...
    if (created) {
      ...
      if (document.getElementById('cb') && PCV.ui.navGen() === _navGen) renderContainers(PCV.ui.renderTarget());
    } else {
...
async function doCtrNicAdd(name) {
  ...
  try {
    ...
    closeModal(); ctrTab = 'network'; if (PCV.ui.navGen() === _navGen) renderContainers(PCV.ui.renderTarget());
  } catch (e) { toast(e.message, false); }
}
...
async function ctrNicDel(name, nicName) {
  ...
  try {
    ...
    ctrTab = 'network'; if (PCV.ui.navGen() === _navGen) renderContainers(PCV.ui.renderTarget());
  } catch (e) { toast(e.message, false); }
}
