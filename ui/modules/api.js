  ...
  ws.onmessage = function(e) {
    ...
    try {
      ...
      if (m.type && (m.type.startsWith('vm-') || m.type.startsWith('vm.'))) {
        ...
        if (window.currentTab === 'mon-alerts' &&
            PCV.monitor &&
            typeof PCV.monitor.refreshAlertHistoryFromEvent === 'function') {
...
async function doLoginPage() {
  ...
  try {
    ...
    if (r.ok && d.totp_required) { _totpBeginVerify(user, d.pending_token); return; }
    if (r.ok && d.totp_enroll_required) {
      if (window.PCV && PCV.totp && typeof PCV.totp.beginForcedEnroll === 'function') {
        PCV.totp.beginForcedEnroll(user, d.pending_token, _finishLogin);
      } else if (errEl) {
        errEl.textContent = _loginI18n('login.totp.title', 'TOTP verification');
      }
      return;
    }
...
var _totpLoginCtx = null;

                                                 
                                                       
function _setLoginTotpMode(on) {
  var fields = document.getElementById('login-cred-fields');
  var actions = document.getElementById('login-cred-actions');
  var step = document.getElementById('login-totp-step');
  if (fields) fields.hidden = !!on;
  if (actions) actions.hidden = !!on;
  if (step) step.hidden = !on;
}

function _totpResetLoginStep() {
  _totpLoginCtx = null;
  var codeInput = document.getElementById('login-totp-code');
  if (codeInput) codeInput.value = '';
  _setLoginTotpMode(false);
}

function _totpBeginVerify(user, pendingToken) {
  _totpLoginCtx = { user: user, pendingToken: pendingToken };
  var errEl = document.getElementById('login-err');
  if (errEl) errEl.textContent = '';
  _setLoginTotpMode(true);
  var codeInput = document.getElementById('login-totp-code');
  if (codeInput) { codeInput.value = ''; try { codeInput.focus(); } catch (e) {                } }
}

                                        
                                                                             
function _totpFailureText(status, d) {
  var code = d && d.error && d.error.code;
  if (status === 401 && code === 'TOTP_PENDING_EXPIRED') return _loginI18n('login.totp.expired', 'Expired — please log in again');
  if (status === 429 || code === 'TOTP_LOCKED') return _loginI18n('login.totp.locked', 'Too many attempts');
  return _loginI18n('login.totp.invalid', 'Invalid code');
}

async function _totpSubmitLogin() {
  if (!_totpLoginCtx) return;
  var errEl = document.getElementById('login-err');
  if (errEl) errEl.textContent = '';
  var codeInput = document.getElementById('login-totp-code');
  var code = codeInput ? codeInput.value.trim() : '';
  if (!code) { if (errEl) errEl.textContent = _loginI18n('login.required', 'Required'); return; }
  var ctx = _totpLoginCtx;
  try {
    const r = await _fetchWithTimeout(EP.AUTH_TOTP_VERIFY(), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (r.ok && d.access_token) {
      ...
      if (d.recovery_codes && window.PCV && PCV.totp && typeof PCV.totp.showRecoveryCodes === 'function') {
        PCV.totp.showRecoveryCodes(d.recovery_codes);
      }
      _finishLogin(ctx.user, d);
    } else {
      if (errEl) errEl.textContent = _totpFailureText(r.status, d);
                                                    
      if (r.status === 401 && d && d.error && d.error.code === 'TOTP_PENDING_EXPIRED') _totpResetLoginStep();
    }
  } catch (e) {
    if (errEl) errEl.textContent = _loginNetworkFailureText(e);
  }
...
function _totpToggleRecovery() {
  var codeInput = document.getElementById('login-totp-code');
  var label = document.getElementById('login-totp-label');
  var link = document.getElementById('login-totp-recovery-link');
  if (!codeInput) return;
  var recovery = codeInput.getAttribute('data-recovery') !== '1';
  codeInput.setAttribute('data-recovery', recovery ? '1' : '0');
  codeInput.setAttribute('inputmode', recovery ? 'text' : 'numeric');
  codeInput.setAttribute('placeholder', recovery ? 'xxxxx-xxxxx' : '000000');
  if (label) label.textContent = recovery ? _loginI18n('login.totp.recovery_link', 'Use recovery code') : _loginI18n('login.totp.prompt', 'Enter the 6-digit code');
  if (link) link.textContent = recovery ? _loginI18n('login.totp.prompt', 'Enter the 6-digit code') : _loginI18n('login.totp.recovery_link', 'Use recovery code');
...
function doLogout() {
  ...
  PCV.ui._renderTarget = null;                                          
  var _vcEl = document.getElementById('vc');                                     
  if (_vcEl) _vcEl.textContent = '0';
...
PCV.api = {
  RPC_ERROR: Object.freeze({
    CONFLICT: -32002,
    TOTP_REQUIRED: -32007,
    TOTP_ENROLL_REQUIRED: -32008,
    TOTP_INVALID_CODE: -32009,
    TOTP_PENDING_EXPIRED: -32010,
    TOTP_LOCKED: -32011
  }),
  fetchGet: fetchGet,
  fetchPost: fetchPost,
...
window.doLoginTotpSubmit = _totpSubmitLogin;
window.doLoginTotpRecoveryToggle = _totpToggleRecovery;
