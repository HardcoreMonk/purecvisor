...
function _fmtRelative(ts) { return ts ? PCV.uxlib.formatRelativeTime(ts * 1000) : '-'; }
...
function render() {
  ...
  var ctrls = _el('div', { class: 'sh-controls' }, [
    modeBadge,
    _el('button', { onclick: function() { setMode('active'); }, text: '→ active', 'data-role': 'ADMIN' }),
    _el('button', { onclick: function() { setMode('dry_run'); }, text: '→ dry_run', 'data-role': 'ADMIN' }),
    _el('button', { onclick: triggerAgent, title: 'AI Agent 직접 호출', text: '🤖 Agent 분석', 'data-role': 'ADMIN' }),
    _el('button', { onclick: resetBaseline, title: 'Z-Score 통계 리셋', text: '⟲ Baseline', 'data-role': 'ADMIN' }),
  ...
  if (_state.pending.length === 0) {
    ...
    _state.pending.forEach(function(p) {
      tbody.appendChild(_el('tr', null, [
        ...
        _el('td', null, [
          _el('button', { class: 'sh-approve', 'data-role': 'ADMIN', onclick: (function(id){
            return function() { approve(id); };
          })(p.id), text: '✓ 승인' }),
          _el('button', { class: 'sh-reject', 'data-role': 'ADMIN', onclick: (function(id){
  ...
  if (!ah || ah.status === 'no_data') {
    ...
    agentSec.appendChild(_el('p', null, [
      ...
      _el('code', { text: ah.consensus || '?' }),
      ' (신뢰도 ' + (ah.confidence || 0).toFixed(2) + ')'
    ]));
    agentSec.appendChild(_el('p', { text: '평균 latency: ' +
      (ah.avg_latency_ms || 0).toFixed(0) + 'ms' }));
    if (ah.providers && ah.providers.length) {
      ...
      ah.providers.forEach(function(r) {
...
function renderSelfHealing(b) {
  ...
  var header = HN.pagehead({
    title: 'AI Self-Healing 관리',
                                                                         
                                                                          
                                                                             
                                                                 
                                                             
    desc: '8개 정책 + AI Agent 합의 + 5중 안전장치. ADR-0020/0021 참조.'
  });
