...
async function renderCloudMigration(b) {
  ...
  var importPanel = el('div', { class: 'hc cloud-mutation-panel' },
    ...
    el('button', { class: 'btn btn-g', onclick: 'cmDoImport()', style: 'margin-top:8px;width:100%', 'data-role': 'ADMIN' }, '📥 Start Import'));

  var exportPanel = el('div', { class: 'hc cloud-mutation-panel' },
    ...
    el('button', { class: 'btn btn-g', onclick: 'cmDoExport()', style: 'margin-top:8px;width:100%', 'data-role': 'ADMIN' }, '📦 Start Export'));
  ...
  clearEl(b);
  b.appendChild(frag(
    HN.pagehead({ title: 'Cloud Migration — AWS EC2 ↔ PureCVisor' }),
    el('div', { class: 'sg grid-2 mb-14' }, importPanel, exportPanel),
    jobsPanel,
    pipelineCard
  ));

  var cloudRole = String((window.currentUser && window.currentUser.role) || '').toUpperCase();
  var canMutateCloud = typeof pcvRoleAllows === 'function'
    ? pcvRoleAllows('admin')
    : cloudRole === 'ADMIN';
  if (!canMutateCloud) {
    b.querySelectorAll('.cloud-mutation-panel input,.cloud-mutation-panel select').forEach(function(node) {
      node.disabled = true;
    });
  }
...
async function cmLoadJobs() {
  ...
  try {
    ...
    var tbody = mk('tbody', null, jobs.map(function(j) {
      ...
      var actions = [];
      if (active && !awaitingCutover) actions.push(mk('button', { class: 'btn btn-r', style: 'font-size:10px;padding:2px 8px', onclick: "cmCancelJob('" + esc(j.name) + "')", 'data-role': 'ADMIN' }, 'Cancel'));
      if (awaitingCutover) actions.push(mk('button', { class: 'btn btn-g', style: 'font-size:10px;padding:2px 8px', onclick: "cmFinalize('" + esc(j.name) + "')", 'data-role': 'ADMIN' }, 'Finalize'));
      return mk('tr', null,
        mk('td', null, mk('b', null, j.name || '')),
        mk('td', null, HN.statusPill('idle', String(j.direction || '?').toUpperCase())),
        mk('td', null, HN.statusPill(st === 'done' ? 'ok' : st === 'failed' ? 'crit' : 'warn', String(st).toUpperCase())),
