...
var ISO_SVG_NS = 'http://www.w3.org/2000/svg';
function _isoIcon(symbol, cls) {
  var svg = document.createElementNS(ISO_SVG_NS, 'svg');
  svg.setAttribute('class', 'ci-icon' + (cls ? ' ' + cls : ''));
  svg.setAttribute('aria-hidden', 'true');
  svg.setAttribute('focusable', 'false');
  var use = document.createElementNS(ISO_SVG_NS, 'use');
...
async function browseISO() {
  closeISOBrowser();                                                            
  var el = PCV.uxlib.el, clearEl = PCV.uxlib.clearEl;
  const box = el('div', { class: 'iso-browser-box' },
    el('h2', { class: 'iso-browser-title' },
      _isoIcon('ci-data', 'iso-browser-title-icon'),
      el('span', null, t('iso.browser_title'))),
    el('div', { class: 'stat-label mb-10' }, t('iso.browser_desc')),
    el('div', { id: 'iso-modal-list', class: 'iso-browser-list' },
      el('div', { class: 'p-12 color-muted text-xs' }, el('span', { class: 'spinner' }), ' ', t('loading'))),
    el('div', { class: 'iso-browser-actions mt-10' },
      el('input', {
        'aria-label': _L('ISO 직접 경로', 'Direct ISO path'),
        id: 'iso-manual-path',
        class: 'iso-browser-path',
        placeholder: _L('ISO 직접 경로 입력', 'Enter direct ISO path'),
        onKeydown: function(event) { if (event.key === 'Enter') isoSelectManual(); }
      }),
  ...
  var mine = PCV.modalCore.openBare(box, {
    dialogClass: 'iso-browser',
                                                                              
    onClose: function () { if (isoDialog !== mine) return; isoDialog = null; }
  });
  isoDialog = mine;
  try {
    ...
    if (il.length === 0) {
      listEl.appendChild(el('div', { class: 'text-center', style: 'padding:16px;color:var(--fg2)' },
        _isoIcon('ci-file-document', 'iso-browser-empty-icon'),
        el('div', null, t('iso.not_found'))));
    } else {
      Object.entries(dirs).forEach(([dir, files]) => {
        listEl.appendChild(el('div', { class: 'iso-browser-dir' },
          _isoIcon('ci-data', 'iso-browser-dir-icon'),
          el('span', null, dir + ' (' + files.length + ' files)')));
        files.forEach(iso => {
          ...
          listEl.appendChild(el('button', {
            type: 'button',
            class: 'iso-browser-file',
            onClick: function() { isoSelect(p); }
          },
            _isoIcon('ci-file-document', 'iso-browser-file-icon'),
            el('span', { class: 'iso-browser-file-name' }, fn),
            HN.statusPill('idle', ext),
            el('span', { class: 'iso-browser-file-size' }, sz)));
        });
      });
    }
  } catch (e) { const listEl = document.getElementById('iso-modal-list'); if (listEl) PCV.uxlib.setMsg(listEl, null, { tag: 'div', cls: 'p-12 color-red' }, _L('오류: ', 'Error: ') + e.message); }
}
...
function closeISOBrowser() { if (isoDialog) { var d = isoDialog; isoDialog = null; PCV.modalCore.closeDialog(d); } }
...
function showSettings() {
  const v = vmList[selectedVmIndex]; if (!v) return;
  var el = PCV.uxlib.el;
  var hwItem = function(cls, icon, label, val, action, role) {
    return el('div', { class: cls, onclick: "setHw(this,'" + action + "')", 'data-role': role || null },
    ...
    el('div', { class: 'split' },
      el('div', { class: 'left' },
        el('div', { class: 'hw-list' },
          ...
          hwItem('hw-item', '🛡', 'AutoProtect', 'Backup', 'autoprotect', 'ADMIN'))),
...
function hwNic() {
  ...
  var nodes = [
    ...
    el('div', { class: 'mt-8', 'data-role': 'ADMIN' },
      el('div', { class: 'fr' },
...
function hwAP() {
  ...
  var nodes = [
    ...
    el('div', { class: 'mt-8' },
      ...
      el('button', { class: 'btn btn-g', onclick: 'bpSet()', 'data-role': 'ADMIN' }, 'Set Policy'))
  ];
...
async function bpSet() { try { const r = await fetchPost(EP.BACKUP_POLICIES(), { vm_name: document.getElementById('bp-vm')?.value || '*', interval_hours: +(document.getElementById('bp-int')?.value || 24), retention_count: +(document.getElementById('bp-ret')?.value || 7) }); if (r && r.error) { toast(r.error.message || t('error'), false); return; } toast(t('backup.policy_set')); loadBP(); } catch (e) { toast(e.message, false); } }
...
function showSnap() { navigateTo('snapshots'); }
...
async function showVnc() { const v = vmList[selectedVmIndex]; if (!v) return; var el = PCV.uxlib.el; showModal([
  ...
  try { const r = await fetchGet(EP.VNC(v.name)); const d = unwrapData(r); const stBadge = v.state === 'running' ? HN.statusPill('ok', 'Available') : HN.statusPill('crit', 'VM stopped'); const info = document.getElementById('vnc-info'); PCV.uxlib.clearEl(info); info.appendChild(HN.card('', [HN.row('Address', d.vnc_address || d.address || 'localhost'), HN.row('Port', String(d.vnc_port || d.port || '-')), HN.row('Status', stBadge)])); } catch (e) { const info = document.getElementById('vnc-info'); PCV.uxlib.clearEl(info); info.appendChild(HN.card('', PCV.uxlib.el('p', { class: 'color-muted' }, 'VNC info unavailable'))); } }
  ...
  return el('div', { class: 'vm-clone-choice-grid' },
    el('label', { class: 'vm-clone-choice', 'data-vm-clone-mode-choice': 'full' },
      ...
      el('span', { class: 'vm-clone-choice-head' },
        el('span', null, 'Full'),
        HN.statusPill('ok', _L('독립', 'Independent'))),
  ...
  return el('div', { class: 'vm-clone-choice-grid' },
    el('label', { class: 'vm-clone-choice', 'data-vm-clone-safety-choice': 'guest-reset' },
      ...
      el('span', { class: 'vm-clone-choice-head' },
        ...
        HN.statusPill('warn', 'libguestfs-tools')),
    ...
    el('label', { class: 'vm-clone-choice', 'data-vm-clone-safety-choice': 'template' },
      ...
      el('span', { class: 'vm-clone-choice-head' },
        ...
        HN.statusPill('ok', _L('도구 불필요', 'No tools'))),
