...
function renderAdminOnlyNotice(b, title) {
  var mk = PCV.uxlib.el, clearEl = PCV.uxlib.clearEl;
  clearEl(b);
  b.appendChild(PCV.uxlib.frag(
    HN.pagehead({ title: title || _L('계정 관리', 'Account management') }),
...
async function renderAccounts(b) {
  if (!await isAdminAccountView()) {
    renderAdminOnlyNotice(b, _L('계정 관리', 'Account management'));
    return;
  }
  showSkeleton(b);
  try { const r = await fetchGet(EP.AUTH_USERS());
                                                            
                                                                 
    if (r && r.error) throw new Error(r.error.message || _L('사용자 목록 조회 실패', 'Unable to load users'));
    ...
    setTimeout(function() {
      createDataTable('acct-table', {
        headers: [
          ...
          {key:'totp', label:'TOTP', sortable: false},
        ...
        rows: l.map(function(u) {
          ...
          var totpSt = u.totp || {};
          var totpBadge = HN.statusPill(totpSt.confirmed ? 'ok' : 'idle',
            totpSt.confirmed ? t('totp.settings.enabled') : t('totp.settings.disabled'));
          var totpCell;
          if (u.username !== 'admin' && PCV.totp && PCV.totp.adminResetButton) {
            totpCell = [totpBadge, ' ', PCV.totp.adminResetButton(u.username, function() {
              if (PCV.ui && PCV.ui.renderTarget) renderAccounts(PCV.ui.renderTarget());
            })];
          } else {
            totpCell = totpBadge;
          }
          return [
            ...
            HN.statusPill('idle', String(u.role).toUpperCase()),
            u.tenant || '---',
            totpCell,
            actions
          ];
    ...
    var heading = HN.pagehead({
      ...
      actions: [HN.statusPill('idle', 'RBAC')]
    });
    ...
    var grid = mk('div', { class: 'sg grid-2 accounts-management-grid', style: 'gap:14px' },
      mk('div', null, mk('div', { id: 'acct-table' })),
      card);
                                                               
    var totpCardHost = mk('div', { class: 'mt-14' });
    if (PCV.totp && PCV.totp.renderSettingsCard) PCV.totp.renderSettingsCard(totpCardHost);
    clearEl(b);
    b.appendChild(frag(heading, grid, totpCardHost));
  } catch (e) {
    PCV.uxlib.renderLoadError(b, {
      title: _L('계정 관리', 'Account Management'),
      message: e.message,
      retry: function() { renderAccounts(b); }
    });
  }
}
...
async function acctCreate() { const u = document.getElementById('acct-user')?.value; const p = document.getElementById('acct-pass')?.value; const r = document.getElementById('acct-role')?.value;
  ...
  try { var _navGen = PCV.ui.navGen(); const res = await fetchPost(EP.AUTH_USERS(), { username: u, password: p, role: r || 'operator' });
    if (res.error) { toast(res.error.message || 'Failed', false); } else { toast(t('auth.user_created') + ': ' + u); addEvt('IAM User created — ' + u + ' (role: ' + (r || 'operator') + ')'); if (PCV.ui.navGen() === _navGen) renderAccounts(PCV.ui.renderTarget()); } } catch (e) { toast(e.message, false); } }
...
async function acctRole(u) { const r = document.getElementById('role-' + u)?.value; if (!r) return;
  try { var _navGen = PCV.ui.navGen(); const res = await fetchPost(EP.AUTH_ROLE(), { username: u, role: r });
    if (res.error) { toast(res.error.message || 'Failed', false); } else { toast(t('auth.role_changed') + ': ' + u + ' → ' + r); addEvt('IAM Role changed — ' + u + ' → ' + r); if (PCV.ui.navGen() === _navGen) renderAccounts(PCV.ui.renderTarget()); } } catch (e) { toast(e.message, false); } }
...
async function acctDel(u) { var _navGen = PCV.ui.navGen(); if (!await customConfirm(t('btn.delete'), 'User ' + u + '?')) return;
                                                       
                                                                          
                                                                  
                                                                           
                                                                
  try { const res = await fetchDelete(EP.AUTH_USER(u), { username: u });
    if (res.error) { toast(res.error.message || 'Failed', false); } else { toast(t('auth.user_deleted') + ': ' + u); addEvt('IAM User deleted — ' + u); if (PCV.ui.navGen() === _navGen) renderAccounts(PCV.ui.renderTarget()); } } catch (e) { toast(e.message, false); } }
...
async function renderApiManagement(b) {
  if (!await isAdminAccountView()) {
    renderAdminOnlyNotice(b, 'API Management');
    return;
  }
  var mk = PCV.uxlib.el, frag = PCV.uxlib.frag, clearEl = PCV.uxlib.clearEl;
  var header = HN.pagehead({ title: 'API Management' });
  ...
  var navRow = mk('div', { class: 'flex gap-8 flex-wrap' },
    ...
    mk('a', { class: 'btn', href: '/ui/docs.html#14-rest-api', target: '_blank', rel: 'noopener' }, '📜 REST API Guide'),
  ...
  setTimeout(() => {
    ...
    clearEl(el);
    el.appendChild(frag(
      HN.statusPill('idle', 'Config-based'),
...
async function apiMgmtSend() { const m = document.getElementById('apimgmt-method').value, path = document.getElementById('apimgmt-path').value; const body = document.getElementById('apimgmt-body').value, el = document.getElementById('apimgmt-result');
  ...
  try { const opts = { method: m, headers: {} }; if (authToken) opts.headers['Authorization'] = 'Bearer ' + authToken;
    ...
    el.appendChild(frag(
      mk('div', { class: 'mb-6' },
        HN.statusPill(r.ok ? 'ok' : 'crit', String(r.status)),
        ' ',
    ...
    if (d.providers && d.providers.length) {
      var provRows = d.providers.map(p =>
        mk('tr', null,
          ...
          mk('td', null, HN.statusPill(p.urgency === 'high' ? 'crit' : p.urgency === 'medium' ? 'warn' : 'ok', String(p.urgency || '-').toUpperCase())),
...
async function testProvider(i, name) {
  ...
  try { let ok = false, detail = ''; const t0 = performance.now();
    ...
    if (ok) { clearEl(rEl); rEl.appendChild(frag(HN.statusPill('ok', 'Connected'), ' ', mk('span', { class: 'stat-label' }, ms + 'ms'))); }
    else { clearEl(rEl); rEl.appendChild(frag(HN.statusPill('crit', 'Failed'), ' ', mk('span', { class: 'stat-label' }, detail))); }
  } catch (e) { clearEl(rEl); rEl.appendChild(frag(HN.statusPill('crit', 'Error'), ' ', mk('span', { class: 'text-xs color-red' }, e.message))); }
}
...
async function renderApiPerf(b) {
  ...
  var rows = results.map(function(r) {
    ...
    var gradeStatus = gradeColor === 'g' ? 'ok' : gradeColor === 'y' ? 'warn' : 'crit';
    return mk('tr', null,
      mk('td', null, mk('code', null, r.endpoint)),
      mk('td', null, mk('b', null, r.time + 'ms')),
      mk('td', null, HN.statusPill(r.status === 'ok' ? 'ok' : 'crit', String(r.status).toUpperCase())),
      mk('td', null, HN.statusPill(gradeStatus, grade)));
  });
  var table = mk('table', { class: 'text-12 mt-12' },
    ...
    mk('tbody', null, rows));
  var rerunBtn = mk('button', { class: 'btn mt-12', onclick: "renderApiPerf(PCV.ui.renderTarget())" }, '🔄 ' + _L('재측정', 'Re-run'));
  var benchBtn = mk('button', { class: 'btn', onclick: 'runApiBenchmark()', style: 'margin-left:8px' }, '⚡ ' + _L('벤치마크', 'Benchmark') + ' (5x)');
  clearEl(b);
  b.appendChild(frag(HN.pagehead({ title: _L('API 응답 시간', 'API Response Times') }), grid, table, rerunBtn, benchBtn));
}
...
async function runApiBenchmark() {
  ...
  var rows = Object.keys(results).map(function(ep) {
    ...
    var p95 = times[Math.floor(times.length * 0.95)] || times.at(-1);
    return mk('tr', null,
      mk('td', null, mk('code', null, ep)),
      mk('td', null, mk('b', null, avg + 'ms')),
      mk('td', { class: 'color-green' }, times[0] + 'ms'),
      mk('td', { class: 'color-red' }, times.at(-1) + 'ms'),
      mk('td', null, p95 + 'ms'));
  });
  ...
  var mc = PCV.modalCore.currentBody();
...
function renderActivityLog(b) {
  ...
  var toolbar = mk('div', { class: 'flex gap-6 mb-8' },
    ...
    mk('button', { class: 'btn btn-xs', onclick: "window.eventLog=[];renderActivityLog(PCV.ui.renderTarget())" }, 'Clear'));
  var body;
  if (log.length === 0) {
    ...
    var rows = log.toReversed().map(function(l) {
  ...
  clearEl(b);
  b.appendChild(frag(HN.pagehead({ title: _L('API 활동 로그', 'API Activity Log') }), toolbar, body));
}
  ...
  try {
    ...
    if (list.length === 0) {
      ...
      var rows = list.map(function(k) {
        var st = k.revoked ? HN.statusPill('idle', _L('폐기', 'Revoked')) : HN.statusPill('ok', _L('활성', 'Active'));
                                                                            
        var expCell;
        if (!k.expires_at) {
          expCell = mk('td', { class: 'color-muted' }, _L('무기한', 'Never'));
        } else if (k.expires_at * 1000 < Date.now()) {
          expCell = mk('td', null, HN.statusPill('warn', _L('만료', 'Expired')));
        } else {
...
window.setAgentTab = function (t) { agentTab = t; showAgentConfig(); };
