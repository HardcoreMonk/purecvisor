...
async function showCpuStats() {
  ...
  try {
    ...
    parts.push(mk('div', { class: 'mb-12' },
      ...
      HN.row('CPU Time (ns)', mk('span', { class: 'color-green' }, d.cpu_time_ns || 0))));
    ...
    if (vcpus.length > 0) {
      ...
      parts.push(mk('table', null,
        mk('thead', null, mk('tr', null,
          mk('th', null, 'vCPU'), mk('th', null, 'State'), mk('th', null, 'CPU Time (ns)'))),
        tbody));
    } else {
...
function _vmDiskUsageSeverity(pct) {
  if (pct === null || isNaN(pct)) return 'warn';
  ...
  var summary = el('div', { class: 'mb-12' },
    el('div', { class: 'sg grid-3' },
      HN.card(_L('전체 사용량', 'Total Usage'), [
        ...
        HN.row(_L('상태', 'Status'), HN.statusPill(_vmDiskUsageSeverity(pct), pct === null ? _L('알 수 없음', 'Unknown') : pct.toFixed(1) + '%')),
        _vmDiskUsageBar(pct)
      ]),
  ...
  showModal([
    ...
    el('div', { class: 'mb-14 p-10 border-muted rounded-md' },
      el('div', { style: 'display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px' },
        ...
        el('button', { class: 'btn btn-g', onclick: 'gaEnsureChannel()', 'data-role': 'OPERATOR,ADMIN' }, 'Channel'),
        el('button', { class: 'btn btn-g', onclick: 'gaPing()' }, '🟢 Ping'),
        el('button', { class: 'btn btn-r', onclick: 'gaShutdown()', 'data-role': 'OPERATOR,ADMIN' }, '⚠ Graceful Shutdown')),
    ...
    el('div', { class: 'mb-14 p-10 border-muted rounded-md' },
      ...
      el('button', { class: 'btn btn-g', onclick: 'gaExec()', style: 'margin-top:6px', 'data-role': 'ADMIN' }, '▶ Execute'),
...
function gaStatusBadge(status) {
  if (status === 'ok') return HN.statusPill('ok', 'OK');
  if (status === 'vm_stopped') return HN.statusPill('warn', 'Stopped');
  if (status === 'reboot_required') return HN.statusPill('warn', 'Reboot needed');
  if (status === 'agent_unavailable') return HN.statusPill('warn', 'Install needed');
  return HN.statusPill('crit', 'Channel missing');
}
...
function gaRenderStatus(d) {
  ...
  var grid = el('div', { style: 'display:grid;grid-template-columns:1fr 1fr;gap:8px 16px' },
    ...
    HN.row('Running', d.running ? HN.statusPill('ok', 'Yes') : HN.statusPill('warn', 'No')),
    HN.row('Config channel', d.channel_configured ? HN.statusPill('ok', 'Yes') : HN.statusPill('crit', 'No')),
    HN.row('Live channel', d.channel_live ? HN.statusPill('ok', 'Yes') : HN.statusPill(d.running ? 'warn' : 'crit', 'No')),
    HN.row('Agent ping', d.agent_ping ? HN.statusPill('ok', 'OK') : HN.statusPill('warn', 'No response')),
