...
>
> **현재 범위**: 이 문서는 `purecvisor` Single Edge 기준으로 정리되며, 실제로 제공하는 기능과 운영 절차만 안내합니다.
>
...
+-------+-------+--------------------+
  ...
  +-- network_manager (OVS)    +-- zfs_driver (ZFS)
  +-- ovn_core (local SDN)     +-- auth_manager (SQLite)
        |
        v
...
이 저장소는 Single Edge 공개 배포용 소스 트리입니다. 기존 개발 저장소의 과거 commit 이력과 운영 실기록은 포함하지 않습니다.
>
...
```bash
git clone <public-repo-url> purecvisor
cd purecvisor
```
...
# peers는 Single Edge에서 수동 peer add/remove 시에만 필요
# peers =
...
| 옵션 | 설명 |
|------|------|
| `--network_bridge` | 연결할 브릿지. 미지정 시 `[vm] default_bridge`, 없으면 `virbr0` |
...
# NIC 추가
pcvctl nic add web-prod --bridge pcvbr0 --model virtio
...
```bash
# NAT 모드 (기본, 인터넷 접근 가능)
pcvctl network create --name mgmt-net --mode nat --cidr 10.10.100.0/24
...
> ⚠ 자동 풀 메시와 클러스터 피어 기반 자동화는 **비공개 멀티 제어면 참고 범위**입니다. **Single Edge**도 `overlay.*` 코어 기능(생성/삭제/조회/수동 peer 추가·제거)은 포함하지만, 자동 터널 구성은 공개 빌드 범위에서 제외됩니다.
...
`daemon.conf [overlay]` 섹션을 설정하면 부팅 시 기본 오버레이 브리지가 자동으로 준비됩니다. Single Edge에서는 로컬 브리지 생성까지만 자동화되며, 클러스터 기반 자동 구성은 공개 범위 밖 멀티 제어면 참고 기능으로 남겨 둡니다.

Single Edge는 `overlay.*` 코어 기능을 지원하지만, `tunnel_ip`는 자동 추론하지 않습니다. 따라서 Single Edge에서 overlay를 활성화하려면 운영자가 `[overlay]` 섹션에 `tunnel_ip`를 명시해야 합니다.
...
> ⚠ `ovn.*` 코어 기능은 **Single Edge** 공개 범위에 포함합니다. Single Edge는 서비스 기동 시 local OVN controller를 자동 준비해 로컬 SDN 데이터면을 구성하고, encap 설정과 자동 프로비저닝처럼 클러스터 제어면에 의존하는 자동화는 공개 범위 밖 멀티 제어면 참고 기능으로 남겨 둡니다.
...
- `purecvisor`의 지원 표면은 단일 노드 `purecvisorsd`와 Single Edge UI/API/CLI다.
...
Grafana 대시보드: `https://purecvisor.example.com/grafana/`
...
> 💡 **공통 코어 지원** — 가져오기 기능 자체는 Single Edge에서도 동작합니다. 멀티 노드 제어면 참고 범위에서는 후속 라이브 마이그레이션까지 연결할 수 있습니다.
...
> 💡 **공통 코어 지원** — 자가치유 기본 액션은 Single Edge에서도 동작합니다. 멀티 노드 제어면 참고 범위에서는 노드 간 VM 마이그레이션까지 확장됩니다.
...
루트 `DESIGN.md`에서 관리한다. `DESIGN.md`는 색상 token, typography,
component state, dashboard density, table/card/button/modal 규칙을 정의하고,
[`ui/samples/design-system-preview.html`](samples/design-system-preview.html)은
...
# → {"status":"ok","service":"purecvisorsd","node_name":"standalone",
#     "capabilities":{"cluster":false,"ovn":true,...}}
```
...
| `template create <name> --vcpu N --memory N --disk N` | 생성 |
...
다음 수치는 2026-04-28 로컬 작업트리 기준 스냅샷이다. 고정 계약이 아니라 현행 확인용이며, 릴리스 판단은 `git`, `Makefile`, 정적 게이트 출력이 우선한다.
