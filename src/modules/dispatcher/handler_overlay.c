                            ...
                            UdsServer *server, GSocketConnection *connection)
{
    ...
    if (!pcv_validate_bridge_name(name)) {
        gchar *resp = pure_rpc_build_error_response(rpc_id, -32602, "Invalid: name");
        pure_uds_server_send_response(server, connection, resp); g_free(resp);
        return;
    }
    if (cidr && !pcv_validate_cidr(cidr)) {
        gchar *resp = pure_rpc_build_error_response(rpc_id, -32602, "Invalid: cidr");
        pure_uds_server_send_response(server, connection, resp); g_free(resp);
        return;
    }
                              ...
                              UdsServer *server, GSocketConnection *connection)
{
    ...
    if (!peer_ip || !*peer_ip) {
        gchar *resp = pure_rpc_build_error_response(rpc_id, -32602, "Missing: peer_ip");
        pure_uds_server_send_response(server, connection, resp); g_free(resp);
        return;
    }
    ...
    if (!pcv_validate_bridge_name(name) || !pcv_validate_ip_literal(peer_ip)) {
        gchar *resp = pure_rpc_build_error_response(rpc_id, -32602, "Invalid: name or peer_ip");
        pure_uds_server_send_response(server, connection, resp); g_free(resp);
        return;
    }
                                 ...
                                 UdsServer *server, GSocketConnection *connection)
{
    ...
    if (!vm_name || !zvol) {
        gchar *resp = pure_rpc_build_error_response(rpc_id, -32602, "Missing: vm_name or zvol_path");
        pure_uds_server_send_response(server, connection, resp); g_free(resp);
        return;
    }
                                 ...
                                 UdsServer *server, GSocketConnection *connection)
{
    ...
    if (!vm_name) {
        gchar *resp = pure_rpc_build_error_response(rpc_id, -32602, "Missing: vm_name");
        pure_uds_server_send_response(server, connection, resp); g_free(resp);
        return;
    }
                           ...
                           UdsServer *server, GSocketConnection *connection)
{
    ...
    if (!target_ip || !vm_name) {
        gchar *resp = pure_rpc_build_error_response(rpc_id, -32602, "Missing: target_ip or vm_name");
        pure_uds_server_send_response(server, connection, resp); g_free(resp);
        return;
    }
...
void handle_ovn_switch_create(JsonObject *p, const gchar *id, UdsServer *s, GSocketConnection *c) {
    ...
    if (!name) { gchar *r=pure_rpc_build_error_response(id,-32602,"Missing: name"); pure_uds_server_send_response(s,c,r); g_free(r); return; }
    GError *e=NULL; pcv_ovn_switch_create(name,subnet,&e);
    if (e) { gchar *r=pure_rpc_build_error_response(id,-32000,e->message); pure_uds_server_send_response(s,c,r); g_free(r); g_error_free(e); return; }
...
void handle_ovn_port_add(JsonObject *p, const gchar *id, UdsServer *s, GSocketConnection *c) {
    ...
    if (!sw||!port) { gchar *r=pure_rpc_build_error_response(id,-32602,"Missing: switch,port"); pure_uds_server_send_response(s,c,r); g_free(r); return; }
...
void handle_ovn_acl_add(JsonObject *p, const gchar *id, UdsServer *s, GSocketConnection *c) {
    ...
    if (!sw||!match) { gchar *r=pure_rpc_build_error_response(id,-32602,"Missing: switch,match"); pure_uds_server_send_response(s,c,r); g_free(r); return; }
...
void handle_ovn_router_add_port(JsonObject *p, const gchar *id, UdsServer *s, GSocketConnection *c) {
    ...
    if (!router||!sw||!mac||!cidr) { gchar *r=pure_rpc_build_error_response(id,-32602,"Missing: router,switch,mac,cidr"); pure_uds_server_send_response(s,c,r); g_free(r); return; }
...
void handle_ovn_dhcp_enable(JsonObject *p, const gchar *id, UdsServer *s, GSocketConnection *c) {
    ...
    if (!subnet||!gw) { gchar *r=pure_rpc_build_error_response(id,-32602,"Missing: subnet,gateway"); pure_uds_server_send_response(s,c,r); g_free(r); return; }
...
void handle_ovn_nat_add(JsonObject *p, const gchar *id, UdsServer *s, GSocketConnection *c) {
    ...
    if (!router||!type||!ext_ip||!log_ip) { gchar *r=pure_rpc_build_error_response(id,-32602,"Missing: router,type,external_ip,logical_ip"); pure_uds_server_send_response(s,c,r); g_free(r); return; }
...
void handle_ovn_nat_list(JsonObject *p, const gchar *id, UdsServer *s, GSocketConnection *c) {
    const gchar *router=json_object_has_member(p,"router")?json_object_get_string_member(p,"router"):NULL;
    if (!router) { gchar *r=pure_rpc_build_error_response(id,-32602,"Missing: router"); pure_uds_server_send_response(s,c,r); g_free(r); return; }
...
void handle_ovn_tenant_create(JsonObject *p, const gchar *id, UdsServer *s, GSocketConnection *c) {
    ...
    const gchar *subnet=json_object_has_member(p,"subnet")?json_object_get_string_member(p,"subnet"):NULL;
    if (!tenant||!subnet) { gchar *r=pure_rpc_build_error_response(id,-32602,"Missing: tenant,subnet"); pure_uds_server_send_response(s,c,r); g_free(r); return; }
