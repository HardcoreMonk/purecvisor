...
_execute_action(HealingPolicy *p, const gchar *reason, const gchar *target_vm)
{
    ...
    gint64 dur_ms = (g_get_monotonic_time() - start_us) / 1000;
    _record_healing_action(p->action,
                           (target_vm && *target_vm) ? target_vm : p->name, reason,
...
constexpr int MAX_TARGET_COOLDOWNS = 128;
static_assert(MAX_TARGET_COOLDOWNS >= 1);
...
static TargetCooldown g_target_cooldowns[MAX_TARGET_COOLDOWNS] = {0};
...
static TargetCooldown *
_target_cooldown_slot(const gchar *policy, const gchar *target,
                      gint64 now, gboolean create)
{
    TargetCooldown *free_slot = NULL;                          
    TargetCooldown *evict     = NULL;                                

    for (gint i = 0; i < MAX_TARGET_COOLDOWNS; i++) {
        TargetCooldown *e = &g_target_cooldowns[i];

                                                        
                                                           
        if (e->policy[0] && now >= e->expires_us) e->policy[0] = '\0';

        if (!e->policy[0]) {
            if (!free_slot) free_slot = e;
            continue;
        }
        if (g_strcmp0(e->policy, policy) == 0 && g_strcmp0(e->target, target) == 0)
            return e;                                        
        if (!evict || e->expires_us < evict->expires_us) evict = e;
    }
    ...
    if (!warned) {
        warned = TRUE;
        PCV_LOG_WARN(HEALING_LOG_DOM,
            "(policy x target) cooldown table saturated (%d slots) — evicting the "
            "soonest-expiring entry for policy='%s' target='%s'. The evicted pair may "
            "alert once more before its window would have ended.",
            MAX_TARGET_COOLDOWNS, policy, target);
    }
    return evict;
}
            ...
            gdouble zscore, const gchar *reason, const gchar *target_vm)
{
    ...
    if (per_target) {
        TargetCooldown *c = _target_cooldown_slot(p->name, target_vm, now, TRUE);
        if (c) {
            g_strlcpy(c->policy, p->name, sizeof(c->policy));
            g_strlcpy(c->target, target_vm, sizeof(c->target));
            c->expires_us = now + cooldown_us;
        }
...
pcv_healing_init(void)
{
    ...
    _add_policy("vm-mem-pressure", PCV_ANOMALY_AUTO_METRIC_MEM_FULL,
                        ...
                        const gchar *target_vm)
{
    ...
    for (gint i = 0; i < G.policy_count; i++) {
        ...
        gchar reason[256];
        if (target_vm && *target_vm)
            g_snprintf(reason, sizeof(reason),
                "%s vm=%s Z=%.2f (>%.1f) val=%.2f",
                metric, target_vm, zscore, threshold, value);
        else
            g_snprintf(reason, sizeof(reason),
