...
void purecvisor_vm_config_free(PureCVisorVmConfig *config) {
    ...
    g_free(config->network_mode);
    g_free(config->tenant);
    g_free(config);
}
...
void purecvisor_vm_config_set_network_mode(PureCVisorVmConfig *config, const gchar *mode) {
    if (!config) return;
    if (config->network_mode) g_free(config->network_mode);
    config->network_mode = mode ? g_strdup(mode) : NULL;
}

                                                        
                                                                         
const gchar *purecvisor_vm_config_get_network_mode(PureCVisorVmConfig *config) {
    return config ? config->network_mode : NULL;
}
...
void purecvisor_vm_config_set_tenant(PureCVisorVmConfig *config, const gchar *tenant) {
    if (!config) return;
    if (config->tenant) g_free(config->tenant);
    config->tenant = tenant ? g_strdup(tenant) : NULL;
}

                                                  
                                                                   
const gchar *purecvisor_vm_config_get_tenant(PureCVisorVmConfig *config) {
...
GVirConfigDomain *purecvisor_vm_config_build(PureCVisorVmConfig *config) {
    ...
    if (config->disk_path) {
        ...
        gvir_config_domain_disk_driver_set_io_policy(
            driver, GVIR_CONFIG_DOMAIN_DISK_DRIVER_IO_POLICY_NATIVE);
        gvir_config_domain_disk_driver_set_discard(
            driver, GVIR_CONFIG_DOMAIN_DISK_DRIVER_DISCARD_UNMAP);
