...
_ebpf_thread(gpointer data)
{
    ...
    while (G.running) {
        ...
        PcvAuditChainHealth audit_chain = pcv_audit_get_chain_health();
        pcv_prom_gauge_set_labels("purecvisor_audit_chain_ok", "",
                                  audit_chain.current_ok ? 1.0 : 0.0);
        pcv_prom_gauge_set_labels("purecvisor_audit_chain_historical_break", "",
                                  audit_chain.historical_break ? 1.0 : 0.0);
