                                    ...
                                    gsize argv_len)
{
    ...
    if (!validate_ipv4_literal(ip) || !argv || argv_len < 10) {
        return FALSE;
    }
...
gchar *
pcv_hips_action_build_block_set_script(void)
{
    ...
    return g_strdup_printf(
        "add table inet purecvisor\n"
        "add set inet purecvisor " PCV_HIPS_BLOCK_SET " { type ipv4_addr; }\n"
        "add chain inet purecvisor input "
            "{ type filter hook input priority filter; policy accept; }\n"
        "flush chain inet purecvisor input\n"
        "add rule inet purecvisor input ip saddr @" PCV_HIPS_BLOCK_SET
            " drop comment \"%s\"\n",
        PCV_HIPS_RULE_COMMENT);
}
...
pcv_hips_action_ensure_nft_input_chain(GError **error)
{
    ...
    if (!pcv_spawn_sync_stdin(argv, script, -1, NULL, &std_err, error)) {
        if (error && *error && std_err && *std_err)
            g_prefix_error(error, "ensure nft block set (%s): ", std_err);
        return FALSE;
    }
    return TRUE;
}
...
pcv_hips_action_execute(const gchar *action, const gchar *target, GError **error)
{
    ...
    if (g_strcmp0(action, "block_ip") == 0) {
        ...
        return run_nft_allow_exists(argv, "block_ip add element", error);
    }
