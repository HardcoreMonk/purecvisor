...
init_schema(GError **error)
{
    ...
    static const gchar *schema[] = {
        ...
        "CREATE TABLE IF NOT EXISTS overlay_wg_keys ("
        ...
        "CREATE TABLE IF NOT EXISTS overlay_tenants ("
    ...
    {
        gchar *aerr = NULL;
        gint arc = sqlite3_exec(
            G.db,
            "ALTER TABLE overlay_wg_keys ADD COLUMN slot INTEGER NOT NULL DEFAULT -1",
            NULL, NULL, &aerr);
        if (arc != SQLITE_OK) {
                                                                           
            gboolean dup = (aerr && strstr(aerr, "duplicate column") != NULL);
            if (!dup) {
                                                           
                if (error) {
                    g_set_error(error, security_store_error_quark(), arc,
                                "%s", aerr ? aerr : sqlite3_errmsg(G.db));
                }
                sqlite3_free(aerr);
                G.degraded = TRUE;
                return FALSE;
...
gboolean
pcv_security_store_put_wg_key(const gchar *tenant, const gchar *vm,
                              const gchar *pubkey,
                              const gchar *privkey_enc,
                              const gchar *overlay_ip,
                              GError **error)
{
    if (!tenant || !*tenant || !vm || !*vm || !pubkey || !*pubkey ||
        !privkey_enc || !*privkey_enc) {
        g_set_error(error, security_store_error_quark(), SQLITE_MISUSE,
                    "tenant/vm/pubkey/privkey_enc are required");
        return FALSE;
    }
    ...
    const gchar *sql =
        "INSERT INTO overlay_wg_keys(tenant,vm,pubkey,privkey_enc,overlay_ip) "
        "VALUES(?,?,?,?,?) "
        "ON CONFLICT(tenant,vm) DO UPDATE SET "
        "pubkey=excluded.pubkey, privkey_enc=excluded.privkey_enc, "
        "overlay_ip=excluded.overlay_ip";
    sqlite3_stmt *stmt = NULL;
    gint rc = sqlite3_prepare_v2(G.db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        set_sqlite_error(error, rc, "prepare put wg key");
        G.degraded = TRUE;
        g_mutex_unlock(&G.mu);
        return FALSE;
    }
    ...
    sqlite3_bind_text(stmt, 2, vm, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, pubkey, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 4, privkey_enc, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 5, overlay_ip ? overlay_ip : "", -1, SQLITE_TRANSIENT);
    rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    gboolean ok = (rc == SQLITE_DONE);
    if (!ok) {
        set_sqlite_error(error, rc, "execute put wg key");
        G.degraded = TRUE;
    }
    g_mutex_unlock(&G.mu);
    return ok;
...
gboolean
pcv_security_store_get_wg_key(const gchar *tenant, const gchar *vm,
                              gchar **pubkey_out,
                              ...
                              GError **error)
{
    ...
    if (privkey_enc_out) *privkey_enc_out = NULL;
    if (overlay_ip_out) *overlay_ip_out = NULL;

    if (!tenant || !*tenant || !vm || !*vm) {
        g_set_error(error, security_store_error_quark(), SQLITE_MISUSE,
                    "tenant/vm are required");
        return FALSE;
    }
    ...
    const gchar *sql =
        "SELECT pubkey,privkey_enc,overlay_ip FROM overlay_wg_keys "
        "WHERE tenant=? AND vm=?";
    sqlite3_stmt *stmt = NULL;
    gint rc = sqlite3_prepare_v2(G.db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        set_sqlite_error(error, rc, "prepare get wg key");
        G.degraded = TRUE;
        g_mutex_unlock(&G.mu);
        return FALSE;
    }
    ...
    sqlite3_bind_text(stmt, 2, vm, -1, SQLITE_TRANSIENT);
    rc = sqlite3_step(stmt);
    gboolean found = (rc == SQLITE_ROW);
    if (found) {
                                                                     
        if (pubkey_out) *pubkey_out = g_strdup(col_text(stmt, 0));
        if (privkey_enc_out) *privkey_enc_out = g_strdup(col_text(stmt, 1));
        if (overlay_ip_out) *overlay_ip_out = g_strdup(col_text(stmt, 2));
    } else if (rc != SQLITE_DONE) {
                                                            
        set_sqlite_error(error, rc, "execute get wg key");
        G.degraded = TRUE;
    }
    sqlite3_finalize(stmt);
    g_mutex_unlock(&G.mu);
...
gboolean
pcv_security_store_del_wg_key(const gchar *tenant, const gchar *vm,
                              GError **error)
{
    if (!tenant || !*tenant || !vm || !*vm) {
        g_set_error(error, security_store_error_quark(), SQLITE_MISUSE,
    ...
    const gchar *sql = "DELETE FROM overlay_wg_keys WHERE tenant=? AND vm=?";
    sqlite3_stmt *stmt = NULL;
    gint rc = sqlite3_prepare_v2(G.db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        set_sqlite_error(error, rc, "prepare del wg key");
        G.degraded = TRUE;
        g_mutex_unlock(&G.mu);
        return FALSE;
    }
    ...
    gboolean ok = (rc == SQLITE_DONE);
    if (!ok) {
        set_sqlite_error(error, rc, "execute del wg key");
        G.degraded = TRUE;
    }
    g_mutex_unlock(&G.mu);
    return ok;
...
void
pcv_overlay_tenant_row_free(gpointer row)
{
    PcvOverlayTenantRow *r = row;
    if (!r) return;                                                               
    g_free(r->name);
    g_free(r);
}
...
void
pcv_overlay_wg_key_row_free(gpointer row)
{
    PcvOverlayWgKeyRow *r = row;
    if (!r) return;
    g_free(r->tenant);
    g_free(r->vm);
    g_free(r->pubkey);
...
gboolean
pcv_security_store_put_tenant(const gchar *name, gint subnet_index,
                              GError **error)
{
    if (!name || !*name) {
        g_set_error(error, security_store_error_quark(), SQLITE_MISUSE,
    ...
    const gchar *sql =
        "INSERT INTO overlay_tenants(name,subnet_index) VALUES(?,?) "
        "ON CONFLICT(name) DO UPDATE SET subnet_index=excluded.subnet_index";
    sqlite3_stmt *stmt = NULL;
    gint rc = sqlite3_prepare_v2(G.db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        set_sqlite_error(error, rc, "prepare put tenant");
        G.degraded = TRUE;
        g_mutex_unlock(&G.mu);
        return FALSE;
    }
    ...
    sqlite3_bind_int(stmt, 2, subnet_index);
    rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    gboolean ok = (rc == SQLITE_DONE);
    if (!ok) {
        set_sqlite_error(error, rc, "execute put tenant");
        G.degraded = TRUE;
    }
    g_mutex_unlock(&G.mu);
    return ok;
...
gboolean
pcv_security_store_del_tenant(const gchar *name, GError **error)
{
    if (!name || !*name) {
        g_set_error(error, security_store_error_quark(), SQLITE_MISUSE,
                    "tenant name is required");
    ...
    const gchar *sql = "DELETE FROM overlay_tenants WHERE name=?";
    sqlite3_stmt *stmt = NULL;
    gint rc = sqlite3_prepare_v2(G.db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        set_sqlite_error(error, rc, "prepare del tenant");
        G.degraded = TRUE;
        g_mutex_unlock(&G.mu);
        return FALSE;
    }
    ...
    gboolean ok = (rc == SQLITE_DONE);
    if (!ok) {
        set_sqlite_error(error, rc, "execute del tenant");
        G.degraded = TRUE;
    }
    g_mutex_unlock(&G.mu);
    return ok;
...
gboolean
pcv_security_store_list_tenants(GPtrArray **out_rows, GError **error)
{
    g_return_val_if_fail(out_rows != NULL, FALSE);
    *out_rows = NULL;                                                                 
    ...
    const gchar *sql = "SELECT name,subnet_index FROM overlay_tenants ORDER BY name";
    sqlite3_stmt *stmt = NULL;
    gint rc = sqlite3_prepare_v2(G.db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        set_sqlite_error(error, rc, "prepare list tenants");
        G.degraded = TRUE;
        g_mutex_unlock(&G.mu);
        return FALSE;
    }
    ...
    GPtrArray *rows = g_ptr_array_new_with_free_func(pcv_overlay_tenant_row_free);
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        PcvOverlayTenantRow *r = g_new0(PcvOverlayTenantRow, 1);
        r->name         = g_strdup(col_text(stmt, 0));                                   
        r->subnet_index = sqlite3_column_int(stmt, 1);
    ...
    if (rc != SQLITE_DONE) {
        ...
        set_sqlite_error(error, rc, "execute list tenants");
        G.degraded = TRUE;
...
gboolean
pcv_security_store_set_wg_key_slot(const gchar *tenant, const gchar *vm,
                                   gint slot, GError **error)
{
    ...
    const gchar *sql =
        "UPDATE overlay_wg_keys SET slot=? WHERE tenant=? AND vm=?";
    sqlite3_stmt *stmt = NULL;
    gint rc = sqlite3_prepare_v2(G.db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        set_sqlite_error(error, rc, "prepare set wg key slot");
        G.degraded = TRUE;
        g_mutex_unlock(&G.mu);
        return FALSE;
    }
    ...
    sqlite3_bind_text(stmt, 2, tenant, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, vm, -1, SQLITE_TRANSIENT);
    rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    gboolean ok = (rc == SQLITE_DONE);
    if (!ok) {
        set_sqlite_error(error, rc, "execute set wg key slot");
        G.degraded = TRUE;
    }
    g_mutex_unlock(&G.mu);
    return ok;
...
gboolean
pcv_security_store_list_wg_keys(GPtrArray **out_rows, GError **error)
{
    g_return_val_if_fail(out_rows != NULL, FALSE);
    *out_rows = NULL;                                                 
    ...
    const gchar *sql =
        "SELECT tenant,vm,pubkey,overlay_ip,slot FROM overlay_wg_keys "
        "ORDER BY tenant, vm";
    sqlite3_stmt *stmt = NULL;
    gint rc = sqlite3_prepare_v2(G.db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        set_sqlite_error(error, rc, "prepare list wg keys");
        G.degraded = TRUE;
        g_mutex_unlock(&G.mu);
        return FALSE;
    }
    ...
    GPtrArray *rows = g_ptr_array_new_with_free_func(pcv_overlay_wg_key_row_free);
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        PcvOverlayWgKeyRow *r = g_new0(PcvOverlayWgKeyRow, 1);
        r->tenant     = g_strdup(col_text(stmt, 0));
        r->vm         = g_strdup(col_text(stmt, 1));
        r->pubkey     = g_strdup(col_text(stmt, 2));
        r->overlay_ip = g_strdup(col_text(stmt, 3));
        r->slot       = sqlite3_column_int(stmt, 4);                                
        g_ptr_array_add(rows, r);
    }
    sqlite3_finalize(stmt);
    ...
    if (rc != SQLITE_DONE) {
        ...
        set_sqlite_error(error, rc, "execute list wg keys");
        G.degraded = TRUE;
        g_ptr_array_unref(rows);
        g_mutex_unlock(&G.mu);
        return FALSE;
