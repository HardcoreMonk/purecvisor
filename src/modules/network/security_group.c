...
extern gboolean pcv_vpc_vm_has_publish(const gchar *vm_identifier)
    __attribute__((weak));
extern gboolean pcv_vpc_security_group_sync_vm(const gchar *vm_name, GError **error)
    __attribute__((weak));

static gboolean
_sync_vpc_security_group_vm(const gchar *vm, const gchar *operation)
{
    if (!pcv_vpc_security_group_sync_vm)
        return TRUE;
    g_autoptr(GError) error = NULL;
    if (pcv_vpc_security_group_sync_vm(vm, &error))
        return TRUE;
    PCV_LOG_ERROR("SG", "%s OVN ACL 동기화 실패(VM '%s'): %s",
                  operation, vm, error ? error->message : "unknown");
    return FALSE;
}

static gboolean
_sync_vpc_security_group_vms(GPtrArray *vms, const gchar *operation)
{
    for (guint i = 0; vms && i < vms->len; i++)
        if (!_sync_vpc_security_group_vm(g_ptr_array_index(vms, i), operation))
            return FALSE;
    return TRUE;
}
...
pcv_security_group_delete(const gchar *name)
{
    ...
    for (guint i = 0; pcv_vpc_vm_has_publish && i < unbind_vms->len; i++) {
        const gchar *vm = g_ptr_array_index(unbind_vms, i);
                                                                
        if (pcv_vpc_vm_has_publish(vm) && !pcv_security_group_vm_is_bound(vm)) {
            PCV_LOG_WARN("SG", "Rejected delete '%s': VM '%s' would lose its last Security Group while published",
                         name, vm);
            g_mutex_lock(&g_sg_mu);
            g_hash_table_insert(g_sg_map, sg->name, sg);
            g_mutex_unlock(&g_sg_mu);
    ...
    if (ok)
        ok = _sync_vpc_security_group_vms(unbind_vms, "group delete");
    if (ok) {                                                 
    ...
    if (!ok) {                                
        ...
        g_hash_table_insert(g_sg_map, sg->name, sg);                         
        g_mutex_unlock(&g_sg_mu);
        g_autoptr(GPtrArray) rollback_view = _snapshot_rule_view(sg);
        g_autofree gchar *rollback_script = pcv_sg_nft_build_group_script(
            name, rollback_view);
        (void)_rebuild_dispatch_ex(rollback_script, NULL);
        (void)_sync_vpc_security_group_vms(unbind_vms, "group delete rollback");
        g_ptr_array_unref(unbind_vms);                                    
        return FALSE;
    }
...
pcv_security_group_rule_add(const gchar *name, JsonObject *rule)
{
    ...
    GPtrArray *affected_vms = g_ptr_array_new_with_free_func(g_free);
    for (guint i = 0; i < sg->vm_bindings->len; i++)
        g_ptr_array_add(affected_vms, g_strdup(g_ptr_array_index(sg->vm_bindings, i)));
    g_mutex_unlock(&g_sg_mu);
    ...
    if (!ok) {
        ...
        g_mutex_unlock(&g_sg_mu);
        g_ptr_array_unref(affected_vms);
        return FALSE;
    }
    if (!_sync_vpc_security_group_vms(affected_vms, "rule_add")) {
        g_mutex_lock(&g_sg_mu);
        SecurityGroup *sg2 = g_hash_table_lookup(g_sg_map, name);
        GPtrArray *rollback_view = NULL;
        if (sg2 && g_ptr_array_find(sg2->rules, r, NULL)) {
            g_ptr_array_remove(sg2->rules, r);
            rollback_view = _snapshot_rule_view(sg2);
        }
        g_mutex_unlock(&g_sg_mu);
        if (rollback_view) {
            g_autofree gchar *rollback_script = pcv_sg_nft_build_group_script(
                name, rollback_view);
            (void)_rebuild_dispatch_ex(rollback_script, NULL);
            _rule_view_free(rollback_view);
        }
        (void)_sync_vpc_security_group_vms(affected_vms, "rule_add rollback");
        g_ptr_array_unref(affected_vms);
        return FALSE;
    }
    ...
    if (db_id < 1) {
        ...
        (void)_sync_vpc_security_group_vms(affected_vms, "rule_add DB rollback");
        g_ptr_array_unref(affected_vms);
        return FALSE;
    }
...
pcv_security_group_apply_to_vm(const gchar *vm, const gchar *sg_name)
{
    ...
    if (pcv_tenant_overlay_vm_in_any_tenant(vm)) {
        PCV_LOG_WARN("SG", "Rejected bind '%s'→VM '%s': tenant-overlay 멤버 — 상호 배타(ADR-0030)",
                     sg_name, vm);
        return FALSE;
    }
    ...
    if (!_sync_vpc_security_group_vm(vm, "bind")) {
        g_mutex_lock(&g_sg_mu);
        SecurityGroup *sg2 = g_hash_table_lookup(g_sg_map, sg_name);
        for (guint i = 0; sg2 && i < sg2->vm_bindings->len; i++)
            if (g_strcmp0(g_ptr_array_index(sg2->vm_bindings, i), vm) == 0) {
                g_ptr_array_remove_index(sg2->vm_bindings, i);
                break;
            }
        g_mutex_unlock(&g_sg_mu);
        (void)_rebuild_dispatch(NULL);
        (void)_sync_vpc_security_group_vm(vm, "bind rollback");
        return FALSE;
    }
...
pcv_security_group_rule_remove(const gchar *name, gint64 rule_id)
{
    ...
    if (!_sync_vpc_security_group_vms(affected_vms, "rule_remove")) {
        g_mutex_lock(&g_sg_mu);
        SecurityGroup *sg2 = g_hash_table_lookup(g_sg_map, name);
        GPtrArray *rollback_view = NULL;
        if (sg2) {
            g_ptr_array_add(sg2->rules, stolen);
            rollback_view = _snapshot_rule_view(sg2);
        } else {
        ...
        (void)_sync_vpc_security_group_vms(affected_vms, "rule_remove rollback");
        g_ptr_array_unref(affected_vms);
        return FALSE;
    }
...
void
pcv_security_group_overlay_exclusion_audit(void)
{
    ...
    while (g_hash_table_iter_next(&it, &k, &v)) {
        SecurityGroup *sg = v;
        for (guint i = 0; i < sg->vm_bindings->len; i++) {
            g_ptr_array_add(pairs, g_strdup((const gchar *)k));                             
            g_ptr_array_add(pairs, g_strdup(g_ptr_array_index(sg->vm_bindings, i)));          
        }
    }
    g_mutex_unlock(&g_sg_mu);
    ...
    for (guint i = 0; i + 1 < pairs->len; i += 2) {                             
        const gchar *grp = g_ptr_array_index(pairs, i);
        const gchar *vm  = g_ptr_array_index(pairs, i + 1);
        if (!pcv_tenant_overlay_vm_in_any_tenant(vm)) continue;                                  
        ...
        pcv_audit_log("system", "sg.overlay_exclusion.cleanup", vm,
                      ok ? "removed" : "detach_failed", 0, 0, "local");
        if (ok)
            removed++;
        else
            PCV_LOG_ERROR("SG", "[M-4 감사] 바인딩 제거 실패: '%s'→'%s' — 수동 조치 필요", grp, vm);
    }
    if (removed)
        PCV_LOG_WARN("SG", "[M-4 감사] 상호 배타 위반 바인딩 %u건 정리 완료", removed);
...
pcv_security_group_resync_all(void)
{
    ...
    if (!_rebuild_dispatch(&error)) {
        PCV_LOG_WARN("SG", "resync_all: dispatch 재생성 실패 (다음 주기 재시도): %s",
                     error ? error->message : "unknown");
        g_clear_error(&error);
    } else if (!_sync_vpc_security_group_vms(vms, "periodic resync")) {
        PCV_LOG_WARN("SG", "resync_all: OVN ACL 재생성 실패 (다음 주기 재시도)");
    }
    g_ptr_array_unref(vms);
}
...
pcv_security_group_detach_vm(const gchar *vm, const gchar *sg_name)
{
    ...
    if (pcv_vpc_vm_has_publish && pcv_vpc_vm_has_publish(vm) &&
        !pcv_security_group_vm_is_bound(vm)) {
        g_mutex_lock(&g_sg_mu);
        SecurityGroup *sg2 = g_hash_table_lookup(g_sg_map, sg_name);
        ...
        if (sg2 && !duplicate) g_ptr_array_add(sg2->vm_bindings, g_strdup(vm));
        g_mutex_unlock(&g_sg_mu);
        PCV_LOG_WARN("SG", "Rejected detach: VM '%s' would lose its last Security Group while published", vm);
        return FALSE;
    }
    ...
    if (!_sync_vpc_security_group_vm(vm, "detach")) {
        g_mutex_lock(&g_sg_mu);
        SecurityGroup *sg2 = g_hash_table_lookup(g_sg_map, sg_name);
        gboolean duplicate = FALSE;
        for (guint i = 0; sg2 && i < sg2->vm_bindings->len; i++)
        ...
        if (sg2 && !duplicate)
            g_ptr_array_add(sg2->vm_bindings, g_strdup(vm));
        g_mutex_unlock(&g_sg_mu);
        (void)_rebuild_dispatch(NULL);
        (void)_sync_vpc_security_group_vm(vm, "detach rollback");
        return FALSE;
    }
...
void
pcv_security_group_unbind_vm_all(const gchar *vm)
{
    if (!vm) return;
    if (!pcv_validate_vm_name(vm)) {
        PCV_LOG_WARN("SG", "Rejected unbind-all: invalid vm '%s'", vm);
        return;
    }
    if (pcv_vpc_vm_has_publish && pcv_vpc_vm_has_publish(vm)) {
        PCV_LOG_WARN("SG", "Rejected unbind-all: VM '%s' has an active VPC Service Publish", vm);
        return;
    }
    ...
    g_mutex_lock(&g_sg_mu);
    _ensure_init();
    GPtrArray *removed_from = g_ptr_array_new_with_free_func(g_free);
    GHashTableIter it;
    gpointer k, v;
    g_hash_table_iter_init(&it, g_sg_map);
    while (g_hash_table_iter_next(&it, &k, &v)) {
        ...
        for (guint i = 0; i < sg->vm_bindings->len; i++) {
            if (g_strcmp0(g_ptr_array_index(sg->vm_bindings, i), vm) == 0) {
                g_ptr_array_remove_index(sg->vm_bindings, i);
                g_ptr_array_add(removed_from, g_strdup((const gchar *)k));           
                break;               
            }
        }
    }
    ...
    if (removed_from->len == 0) {                              
        g_ptr_array_unref(removed_from);
        return;
    }
    if (pcv_vpc_vm_has_publish && pcv_vpc_vm_has_publish(vm)) {
        g_mutex_lock(&g_sg_mu);
        for (guint i = 0; i < removed_from->len; i++) {
            SecurityGroup *sg2 = g_hash_table_lookup(
                g_sg_map, g_ptr_array_index(removed_from, i));
            gboolean duplicate = FALSE;
            for (guint j = 0; sg2 && j < sg2->vm_bindings->len; j++)
                if (g_strcmp0(g_ptr_array_index(sg2->vm_bindings, j), vm) == 0) {
                    duplicate = TRUE; break;
                }
            if (sg2 && !duplicate) g_ptr_array_add(sg2->vm_bindings, g_strdup(vm));
        }
        g_mutex_unlock(&g_sg_mu);
        PCV_LOG_WARN("SG", "Rejected unbind-all: VM '%s' acquired a VPC Service Publish", vm);
        g_ptr_array_unref(removed_from);
        return;
    }
    ...
    GError *error = nullptr;
    if (!_rebuild_dispatch(&error)) {
        PCV_LOG_ERROR("SG", "unbind-all VM '%s' nft 반영 실패 — 롤백: %s",
                      vm, error ? error->message : "unknown");
        g_clear_error(&error);
                                                                      
        g_mutex_lock(&g_sg_mu);
        for (guint i = 0; i < removed_from->len; i++) {
            SecurityGroup *sg2 = g_hash_table_lookup(g_sg_map,
                                     g_ptr_array_index(removed_from, i));
            if (!sg2) continue;
            gboolean dup = FALSE;
            for (guint j = 0; j < sg2->vm_bindings->len; j++)
                if (g_strcmp0(g_ptr_array_index(sg2->vm_bindings, j), vm) == 0) { dup = TRUE; break; }
            if (!dup) g_ptr_array_add(sg2->vm_bindings, g_strdup(vm));
        }
        g_mutex_unlock(&g_sg_mu);
        g_ptr_array_unref(removed_from);
    ...
    if (!_sync_vpc_security_group_vm(vm, "unbind-all")) {
        g_mutex_lock(&g_sg_mu);
        for (guint i = 0; i < removed_from->len; i++) {
            SecurityGroup *sg2 = g_hash_table_lookup(
                g_sg_map, g_ptr_array_index(removed_from, i));
        ...
        g_mutex_unlock(&g_sg_mu);
        (void)_rebuild_dispatch(NULL);
        (void)_sync_vpc_security_group_vm(vm, "unbind-all rollback");
        g_ptr_array_unref(removed_from);
        return;
    }
    ...
    for (guint i = 0; i < removed_from->len; i++)
        _sg_db_delete_binding(g_ptr_array_index(removed_from, i), vm);
    if (!pcv_security_group_vm_is_bound(vm))                                        
        pcv_vm_vnet_cache_evict(vm);
    PCV_LOG_INFO("SG", "Unbound VM '%s' from all %u security group(s) (permanent removal)",
                 vm, removed_from->len);
    g_ptr_array_unref(removed_from);
}
...
JsonObject *
pcv_security_group_policy_for_vm(const gchar *vm)
{
    ...
    gboolean egress_enforced = FALSE;

    if (!vm) {
        json_object_set_boolean_member(policy, "bound", FALSE);
        json_object_set_boolean_member(policy, "egress_enforced", FALSE);
        json_object_set_array_member(policy, "rules", rules);
        return policy;
    }
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        if (!group_bound)
            continue;
        for (guint i = 0; i < sg->rules->len; i++) {
            SgRule *rule = g_ptr_array_index(sg->rules, i);
            JsonObject *copy = json_object_new();
            json_object_set_string_member(copy, "direction", rule->direction);
            json_object_set_string_member(copy, "protocol", rule->protocol);
            json_object_set_int_member(copy, "port_start", rule->port_start);
            json_object_set_int_member(copy, "port_end", rule->port_end);
            json_object_set_string_member(copy, "source", rule->source);
            json_array_add_object_element(rules, copy);
            if (g_strcmp0(rule->direction, "egress") == 0)
                egress_enforced = TRUE;
        }
    }
    g_mutex_unlock(&g_sg_mu);

    json_object_set_boolean_member(policy, "bound", bound);
    json_object_set_boolean_member(policy, "egress_enforced", egress_enforced);
    json_object_set_array_member(policy, "rules", rules);
    return policy;
}
...
pcv_security_group_sync_vm(const gchar *vm_name)
{
    ...
    GError *error = nullptr;
    if (_rebuild_dispatch(&error) &&
        _sync_vpc_security_group_vm(vm_name, "lifecycle sync"))
        return;
    g_clear_error(&error);
    if (_rebuild_dispatch(&error) &&
        _sync_vpc_security_group_vm(vm_name, "lifecycle sync retry"))
        return;                         
