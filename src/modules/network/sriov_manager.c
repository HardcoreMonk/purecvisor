...
_run_cmd(const gchar *cmd, gchar **out, GError **error)
{
    ...
    if (!ok)
        PCV_LOG_WARN(SRIOV_LOG_DOM, "cmd failed: %s  stderr=%s", cmd,
...
pcv_sriov_enable(const gchar *pf, gint num_vfs, GError **error)
{
    ...
    if (current > 0) {
        gchar *cmd = g_strdup_printf("echo 0 > %s", path);
        _run_shell(cmd, NULL, NULL);
        g_free(cmd);
    }

    gchar *cmd = g_strdup_printf("echo %s > %s", val, path);
...
pcv_sriov_disable(const gchar *pf, GError **error)
{
    ...
    gchar *path = g_strdup_printf("%s/%s/device/sriov_numvfs", SRIOV_SYSFS_NET, pf);
    gchar *cmd = g_strdup_printf("echo 0 > %s 2>/dev/null; true", path);
              ...
              gint spoofchk, GError **error)
{
    ...
    if (mac && strlen(mac) > 0) {
        gchar *cmd = g_strdup_printf("ip link set %s vf %d mac %s", pf, vf_index, mac);
        ok = _run_cmd(cmd, NULL, error);
        g_free(cmd);
    }

    if (ok && vlan >= 0) {
        gchar *cmd = g_strdup_printf("ip link set %s vf %d vlan %d", pf, vf_index, vlan);
        ok = _run_cmd(cmd, NULL, error);
        g_free(cmd);
    }

    if (ok && spoofchk >= 0) {
        gchar *cmd = g_strdup_printf("ip link set %s vf %d spoofchk %s",
                                      pf, vf_index, spoofchk ? "on" : "off");
        ok = _run_cmd(cmd, NULL, error);
        g_free(cmd);
    }
                     ...
                     gint vf_index, GError **error)
{
    ...
    gchar *xml_path = g_strdup_printf("/tmp/pcv-sriov-%s-vf%d.xml", vm_name, vf_index);
    gboolean ok = g_file_set_contents(xml_path, xml, -1, error);
    g_free(xml);

    if (ok) {
        gchar *cmd = g_strdup_printf("virsh attach-device %s %s --live 2>&1",
                                      vm_name, xml_path);
        ok = _run_cmd(cmd, NULL, error);
        g_free(cmd);
    }
...
pcv_sriov_detach_vm(const gchar *vm_name, const gchar *pci_addr, GError **error)
{
    ...
    gchar *xml_path = g_strdup_printf("/tmp/pcv-sriov-detach-%s.xml", vm_name);
    gboolean ok = g_file_set_contents(xml_path, xml, -1, error);
    g_free(xml);
    ...
    if (ok) {
        ...
        gchar *cmd = g_strdup_printf(
            "virsh detach-device %s %s --live 2>/dev/null; true",
            vm_name, xml_path);
        ok = _run_shell(cmd, NULL, error);
        g_free(cmd);
    }
