...
typedef struct {
    ...
    PcvRole role;                                            
    gchar  *tenant;                                              
    gboolean totp_enrolled;                                     
    gboolean totp_confirmed;                       
} PcvUser;
...
gboolean pcv_rbac_password_check(const gchar *username,
                                 const gchar *password,
                                 GError     **error);
...
gchar *pcv_rbac_issue_tokens(const gchar *username,
                             gchar      **out_refresh_token,
                             GError     **error);
...
gboolean   pcv_rbac_totp_status(const gchar *username, PcvTotpStatus *out);
...
gchar     *pcv_rbac_totp_enroll(const gchar *username, GError **error);
...
gboolean   pcv_rbac_totp_verify_code(const gchar *username, const gchar *code,
...
GPtrArray *pcv_rbac_totp_generate_recovery(const gchar *username, GError **error);
...
gboolean   pcv_rbac_totp_disable(const gchar *username, GError **error);

                                                          
                                                       
gint       pcv_rbac_totp_get_remaining_lockout(const gchar *username);                                       
...
gboolean   pcv_rbac_totp_role_required(gint role);
