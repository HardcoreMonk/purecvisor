...
typedef struct {
    ...
    struct {
        ...
        struct {
            ...
            gint     detail_for;
            gchar    detail_ports[8][64];
            gint     detail_port_count;
            gchar    detail_acls[8][80];
            gint     detail_acl_count;
            gchar    detail_nats[4][80];
            gint     detail_nat_count;
            gchar    detail_port_mac[8][24];
            gchar    detail_port_net[8][32];
        } ovn;
        ...
        struct {
            ...
            gint     sriov_pf_count;
            gchar    sriov_pfs[4][32];
            gint     sriov_vf_counts[4];
            gint64   last_fetch;
        } accel;
    } net;
    ...
    double       mem_hist[HIST_SIZE];
    double       dsk_hist[HIST_SIZE];
    double       net_rx_hist[HIST_SIZE];
    double       net_tx_hist[HIST_SIZE];
    int          hist_pos;
    int          hist_count;
    double       cpu_pct;
...
static GAsyncQueue *g_event_queue   = NULL;
static GMutex       g_fleet_mu;
static FleetSnapshot g_fleet_snap   = {0};
static gint         g_quit_fleet    = 0;
...
static void draw_top_braille(WINDOW *win, int y0, int w, int panel_h) {
    ...
    int y_label_w = 5;
    int chart_w_cells = (left_w - y_label_w - 3) / 1;
    int chart_h_cells = panel_h - 3;
...

    {
        ...
        int warn_row = (int)(0.25 * chart_h_cells);
        int crit_row = (int)(0.15 * chart_h_cells);
        ...
        char utf8[4];
        const uint8_t MEM_DOT_MASK = 0x44;
...
static void fetch_ovn_detail(int selected_idx) {
    if (selected_idx == g_state.net.ovn.detail_for) return;
            ...
            if (json_object_has_member(ro, "result")) {
                ...
                if (JSON_NODE_TYPE(rn) == JSON_NODE_ARRAY) {
                    ...
                    for (int i = 0; i < g_state.stg.zvol_count; i++) {
                        ...
                        strncpy(zi->size, safe_str(zo,"volsize","?"), sizeof(zi->size) - 1);
                        zi->size[sizeof(zi->size) - 1] = '\0';
                        strncpy(zi->used, safe_str(zo,"used","?"),    sizeof(zi->used) - 1);
...
_tui_acquire_singleton_lock(void)
{
    ...
    if (write(fd, pid_str, (size_t)n) < 0) {  }
    g_tui_lock_fd = fd;
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    init_pair(C_GRAD_0,  COLOR_GREEN,   COLOR_BLACK);
    init_pair(C_GRAD_1,  COLOR_GREEN,   COLOR_BLACK);
    init_pair(C_GRAD_2,  COLOR_YELLOW,  COLOR_BLACK);
    init_pair(C_GRAD_3,  COLOR_YELLOW,  COLOR_BLACK);
    init_pair(C_GRAD_4,  COLOR_RED,     COLOR_BLACK);
    init_pair(C_GRAD_5,  COLOR_RED,     COLOR_BLACK);
    init_pair(C_GRAD_6,  COLOR_RED,     COLOR_BLACK);
