...
typedef struct {
    ...
    gboolean allow_core_dumps;                                                        
                                         
    gboolean ips_enabled;                  
    gint     ips_queue_num;            
    gboolean ips_fail_open;               
    gchar   *ips_mode;                              
                                            
    gboolean webpush_enabled;                   
    gchar   *webpush_min_severity;                                      
    gchar   *webpush_contact;                                     
...
static gboolean
_cfg_bool(GKeyFile *kf, const gchar *key, const gchar *env_var, gboolean default_val)
{
    gchar *s = _cfg_str(kf, key, env_var, default_val ? "true" : "false");
...
static gchar *
_cfg_str_in(GKeyFile *kf, const gchar *section, const gchar *key,
            const gchar *env_var, const gchar *default_val)
{
    const gchar *env = g_getenv(env_var);
    if (env && env[0] != '\0')
        return g_strdup(env);
    if (kf) {
        GError *err = NULL;
        gchar *val = g_key_file_get_string(kf, section, key, &err);
        if (val) { g_strstrip(val); return val; }
...
static gboolean
_cfg_bool_in(GKeyFile *kf, const gchar *section, const gchar *key,
             const gchar *env_var, gboolean default_val)
{
    gchar *s = _cfg_str_in(kf, section, key, env_var, default_val ? "true" : "false");
    gboolean v = (g_ascii_strcasecmp(s, "true") == 0 ||
                  g_strcmp0(s, "1") == 0 ||
                  g_ascii_strcasecmp(s, "yes") == 0);
    g_free(s);
...
static gint
_cfg_int_in(GKeyFile *kf, const gchar *section, const gchar *key,
            const gchar *env_var, gint default_val)
{
    const gchar *env = g_getenv(env_var);
    if (env && env[0] != '\0') {
        gchar *end = NULL;
    ...
    if (kf) {
        GError *err = NULL;
        gint val = g_key_file_get_integer(kf, section, key, &err);
        if (!err) return val;
        g_error_free(err);
    }
    return default_val;
...
const gchar *
pcv_config_get_file_path(void)
{
    const gchar *override = g_getenv("PCV_CONFIG_PATH");
    return (override && override[0] != '\0')
        ? override
        : PCV_CONFIG_FILE_PATH;
}
...
pcv_config_init(void)
{
    ...
    const gchar *cfg_path = pcv_config_get_file_path();
    ...
    g_cfg.allow_core_dumps = _cfg_bool(kf, "allow_core_dumps",
                                       "PURECVISOR_ALLOW_CORE_DUMPS", FALSE);

                                                                       
    g_cfg.ips_enabled   = _cfg_bool_in(kf, "ips", "enabled",   "PURECVISOR_IPS_ENABLED",
                                       PCV_DEFAULT_IPS_ENABLED);
    g_cfg.ips_queue_num = _cfg_int_in (kf, "ips", "queue_num", "PURECVISOR_IPS_QUEUE_NUM",
                                       PCV_DEFAULT_IPS_QUEUE_NUM);
    g_cfg.ips_fail_open = _cfg_bool_in(kf, "ips", "fail_open", "PURECVISOR_IPS_FAIL_OPEN",
                                       PCV_DEFAULT_IPS_FAIL_OPEN);
    g_cfg.ips_mode      = _cfg_str_in (kf, "ips", "mode",      "PURECVISOR_IPS_MODE",
                                       PCV_DEFAULT_IPS_MODE);

                                                        
                                                                
                                                      
    g_cfg.webpush_enabled      = _cfg_bool_in(kf, "webpush", "enabled",
                                              "PURECVISOR_WEBPUSH_ENABLED",
                                              PCV_DEFAULT_WEBPUSH_ENABLED);
    g_cfg.webpush_min_severity = _cfg_str_in (kf, "webpush", "min_severity",
                                              "PURECVISOR_WEBPUSH_MIN_SEVERITY",
                                              PCV_DEFAULT_WEBPUSH_MIN_SEVERITY);
    g_cfg.webpush_contact      = _cfg_str_in (kf, "webpush", "contact",
                                              "PURECVISOR_WEBPUSH_CONTACT",
                                              PCV_DEFAULT_WEBPUSH_CONTACT);
    ...
    if (g_strcmp0(g_cfg.webpush_min_severity, "warn") != 0 &&
        g_strcmp0(g_cfg.webpush_min_severity, "crit") != 0) {
        PCV_LOG_WARN(CFG_LOG_DOM,
                     "webpush min_severity='%s' invalid — using default '%s'",
                     g_cfg.webpush_min_severity ? g_cfg.webpush_min_severity : "",
                     PCV_DEFAULT_WEBPUSH_MIN_SEVERITY);
        g_free(g_cfg.webpush_min_severity);
        g_cfg.webpush_min_severity = g_strdup(PCV_DEFAULT_WEBPUSH_MIN_SEVERITY);
    }
    ...
    if (g_cfg.webpush_contact && *g_cfg.webpush_contact
        && !g_str_has_prefix(g_cfg.webpush_contact, "mailto:")
        && !g_str_has_prefix(g_cfg.webpush_contact, "https:")) {
        PCV_LOG_WARN(CFG_LOG_DOM,
                     "webpush contact='%s' has no RFC 8292 scheme "
                     "(mailto: or https:) — dropping the VAPID 'sub' claim; "
                     "push delivery would be rejected (403) with this value",
                     g_cfg.webpush_contact);
        g_free(g_cfg.webpush_contact);
        g_cfg.webpush_contact = g_strdup(PCV_DEFAULT_WEBPUSH_CONTACT);
    }
    ...
    if (!loaded) {
        ...
        gchar *cfg_dir = g_path_get_dirname(cfg_path);
        if (g_mkdir_with_parents(cfg_dir, 0755) == 0) {
            GError *save_err = NULL;
            if (!g_key_file_save_to_file(sample, cfg_path, &save_err)) {
...
pcv_config_shutdown(void)
{
    ...
    g_free(g_cfg.log_level);
    g_free(g_cfg.ips_mode);
    g_free(g_cfg.webpush_min_severity);
    g_free(g_cfg.webpush_contact);
    g_free(g_cfg.admin_user);
                                                                  
    pcv_secure_free_str(&g_cfg.admin_password);
    pcv_secure_free_str(&g_cfg.jwt_secret);
...
gboolean     pcv_config_get_allow_core_dumps(void) { return g_cfg.allow_core_dumps; }

gboolean     pcv_config_get_ips_enabled(void)   { return g_cfg.ips_enabled; }
gint         pcv_config_get_ips_queue_num(void) { return g_cfg.ips_queue_num; }
gboolean     pcv_config_get_ips_fail_open(void) {
                                                                            
    return g_cfg.initialized ? g_cfg.ips_fail_open : PCV_DEFAULT_IPS_FAIL_OPEN;
}
                                                                                
const gchar *pcv_config_get_ips_mode(void)      {
    return g_cfg.ips_mode ? g_cfg.ips_mode : PCV_DEFAULT_IPS_MODE;
}

gboolean     pcv_config_get_webpush_enabled(void) {
                                                              
                               
    return g_cfg.initialized ? g_cfg.webpush_enabled : PCV_DEFAULT_WEBPUSH_ENABLED;
}
                                                                
const gchar *pcv_config_get_webpush_min_severity(void) {
    return g_cfg.webpush_min_severity ? g_cfg.webpush_min_severity
                                      : PCV_DEFAULT_WEBPUSH_MIN_SEVERITY;
}
                                                                    
const gchar *pcv_config_get_webpush_contact(void) {
    return g_cfg.webpush_contact ? g_cfg.webpush_contact
                                 : PCV_DEFAULT_WEBPUSH_CONTACT;
}
...
static gboolean
_get_https_enabled_locked(GKeyFile *kf, gboolean *enabled, GError **error)
{
    *enabled = TRUE;
    if (!kf || !g_key_file_has_key(kf, "tls", "https_enabled", NULL))
        return TRUE;

    GError *parse_error = NULL;
    gboolean value = g_key_file_get_boolean(kf, "tls", "https_enabled",
                                            &parse_error);
    if (parse_error) {
        g_clear_error(&parse_error);
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
                            "Invalid HTTPS transport configuration");
        return FALSE;
    }
...
gboolean
pcv_config_get_https_enabled(gboolean *enabled, GError **error)
{
    ...
    *enabled = TRUE;
    ...
    g_rw_lock_reader_lock(&g_cfg.kf_lock);
    gboolean valid =
        _get_https_enabled_locked(g_cfg.kf, enabled, error);
    g_rw_lock_reader_unlock(&g_cfg.kf_lock);
    return valid;
}
...
gboolean
pcv_config_validate_transport(GError **error)
{
    ...
    g_rw_lock_reader_lock(&g_cfg.kf_lock);
    if (!_get_https_enabled_locked(g_cfg.kf, &https_enabled, error)) {
        g_rw_lock_reader_unlock(&g_cfg.kf_lock);
        return FALSE;
    }
    if (https_enabled) {
    ...
    gchar *bind_mode = g_cfg.kf
        ? g_key_file_get_value(g_cfg.kf, "server", "bind_plaintext",
                               &read_error)
        : NULL;
    g_rw_lock_reader_unlock(&g_cfg.kf_lock);
    g_clear_error(&read_error);

    if (g_strcmp0(bind_mode, "loopback") == 0) {
        g_free(bind_mode);
        return TRUE;
    }
    g_free(bind_mode);
    ...
    g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
                        "Unsafe plaintext transport configuration");
    return FALSE;
}
...
gchar *
pcv_config_dup_raw_value(const gchar *section, const gchar *key)
{
    if (!section || !key || !g_cfg.initialized)
        return NULL;

    g_rw_lock_reader_lock(&g_cfg.kf_lock);
    GError *error = NULL;
    gchar *value = g_cfg.kf
        ? g_key_file_get_value(g_cfg.kf, section, key, &error)
        : NULL;
    g_rw_lock_reader_unlock(&g_cfg.kf_lock);
...
pcv_config_reload(void)
{
    ...
    if (!g_key_file_load_from_file(kf, cfg_path,
                                   G_KEY_FILE_NONE, &err)) {
        PCV_LOG_WARN(CFG_LOG_DOM,
                                          ...
                                          
    {
        ...
        pcv_secure_free_str(&new_secret);
    }
...
_decrypt_aes_gcm(const gchar *b64_ciphertext)
{
    ...
    if (!ok) {
        ...
        pcv_secure_wipe(plaintext, (gsize)cipher_len); g_free(plaintext);
        return NULL;
    }
...
static gchar *
_decrypt_aes_gcm_v2(const gchar *b64)
{
    guchar key[PCV_SECRET_KEY_LEN], unused_iv[PCV_SECRET_IV_LEN];
    if (!_derive_master_key(key, unused_iv))                                  
        return NULL;

    gsize raw_len = 0;
    guchar *raw = g_base64_decode(b64, &raw_len);
    if (!raw || raw_len < (gsize)PCV_SECRET_IV_LEN + PCV_SECRET_TAG_LEN) {
        pcv_secure_wipe(key, sizeof key);                                              
        pcv_secure_wipe(unused_iv, sizeof unused_iv);                            
        g_free(raw);
        return NULL;
    }

    const guchar *iv         = raw;                                               
    const guchar *ciphertext = raw + PCV_SECRET_IV_LEN;
    int cipher_len = (int)(raw_len - PCV_SECRET_IV_LEN - PCV_SECRET_TAG_LEN);
    const guchar *tag        = raw + raw_len - PCV_SECRET_TAG_LEN;
    ...
    if (!ok) {
        PCV_LOG_WARN(CFG_LOG_DOM,
                     "AES-256-GCM(ENC2) decryption failed — check machine-id or ciphertext");
        pcv_secure_wipe(plaintext, (gsize)cipher_len); g_free(plaintext);
        return NULL;
    }
    return (gchar *)plaintext;
...
static gchar *
_decrypt_secret_value(const gchar *value)
{
    if (!value) return NULL;
    if (g_str_has_prefix(value, "ENC2:"))
        return _decrypt_aes_gcm_v2(value + 5);                        
    if (g_str_has_prefix(value, "ENC:"))
        return _decrypt_aes_gcm(value + 4);                                 
    return NULL;
}
                       ...
                       const gchar *fallback)
{
    ...
    if (g_str_has_prefix(raw, "ENC2:") || g_str_has_prefix(raw, "ENC:")) {
        gchar *decrypted = _decrypt_secret_value(raw);
...
pcv_config_encrypt_value(const gchar *plaintext)
{
    ...
    guchar iv[PCV_SECRET_IV_LEN];
    if (RAND_bytes(iv, PCV_SECRET_IV_LEN) != 1) {
        PCV_LOG_WARN(CFG_LOG_DOM, "RAND_bytes failed — refusing to encrypt with a weak nonce");
        pcv_secure_wipe(key, sizeof key);                                              
        pcv_secure_wipe(unused_iv, sizeof unused_iv);                            
        pcv_secure_wipe(iv, sizeof iv);                                     
        return NULL;
    ...
    guchar *out_buf = g_malloc0((gsize)PCV_SECRET_IV_LEN + (gsize)pt_len +
                                EVP_MAX_BLOCK_LENGTH + PCV_SECRET_TAG_LEN);
    guchar *ciphertext = out_buf + PCV_SECRET_IV_LEN;
    int out_len = 0, final_len = 0;
    ...
    memcpy(out_buf, iv, PCV_SECRET_IV_LEN);
    gsize ct_len = (gsize)(out_len + final_len);
    memcpy(out_buf + PCV_SECRET_IV_LEN + ct_len, tag, PCV_SECRET_TAG_LEN);
    gsize total_len = (gsize)PCV_SECRET_IV_LEN + ct_len + PCV_SECRET_TAG_LEN;
    ...
    gchar *result = g_strdup_printf("ENC2:%s", b64);
    g_free(b64);
    return result;
}
...
gchar *
pcv_config_decrypt_value(const gchar *enc)
{
    if (!enc) return NULL;
    if (!g_str_has_prefix(enc, "ENC2:") && !g_str_has_prefix(enc, "ENC:"))
        return NULL;
    return _decrypt_secret_value(enc);
}
