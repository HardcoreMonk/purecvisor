...
guint64 pcv_memtune_safe_floor_mb(guint64 guest_ram_mb) {
    guint64 headroom = guest_ram_mb * 15 / 100;                               
    if (headroom < 256) headroom = 256;                                          
    return guest_ram_mb + headroom;
}
...
gboolean
pcv_vm_validate_network_mode_tenant(const gchar *mode, const gchar *tenant, GError **error)
{
    gboolean mode_specified = (mode && *mode);
    gboolean tenant_specified = (tenant && *tenant);

    if (mode_specified &&
        g_strcmp0(mode, "bridge") != 0 &&
        g_strcmp0(mode, "dpdk") != 0 &&
        g_strcmp0(mode, "sriov") != 0 &&
        g_strcmp0(mode, "tenant-overlay") != 0) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
            "Invalid network_mode '%s': must be bridge | dpdk | sriov | tenant-overlay", mode);
        return FALSE;
    }

    if (mode_specified && g_strcmp0(mode, "tenant-overlay") == 0) {
        if (!tenant_specified) {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                "network_mode 'tenant-overlay' requires a 'tenant' parameter");
            return FALSE;
        }
        if (!pcv_validate_vm_name(tenant)) {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                "Invalid tenant '%s': must be 1-%d chars [a-zA-Z0-9_-]",
                tenant, PCV_MAX_VM_NAME);
            return FALSE;
        }
        return TRUE;
    }
    ...
    if (tenant_specified) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
            "'tenant' is only valid with network_mode 'tenant-overlay' (got mode '%s')",
            mode_specified ? mode : "(unspecified)");
        return FALSE;
    }
...
gboolean pcv_validate_network_create_params(const gchar  *bridge_name,
                                            ...
                                            const gchar  *uplink_mode,
                                            const gchar  *safety_ack,
                                            GError      **error)
{
    ...
    if (g_strcmp0(eff_mode, "bridge") != 0) {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
            "bridge mode does not accept cidr; the host bridge remains unnumbered");
        return FALSE;
    }

                                                           
    if (g_strcmp0(eff_mode, "bridge") == 0) {
        if (!physical_if || !pcv_validate_iface_name(physical_if)) {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                "Invalid or missing physical_if '%s' for bridge mode",
                physical_if ? physical_if : "(null)");
            return FALSE;
        }
        const gchar *effective_uplink = uplink_mode;
        if ((!effective_uplink || !*effective_uplink)
            && g_strcmp0(safety_ack, "dedicated-uplink") == 0)
            effective_uplink = "dedicated";
        if (g_strcmp0(effective_uplink, "dedicated") != 0
            && g_strcmp0(effective_uplink, "shared") != 0) {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                "bridge mode requires uplink_mode='dedicated' or 'shared'");
            return FALSE;
        }
        const gchar *expected_ack = g_strcmp0(effective_uplink, "shared") == 0
            ? "shared-uplink" : "dedicated-uplink";
        if (g_strcmp0(safety_ack, expected_ack) != 0) {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                "bridge uplink_mode='%s' requires safety_ack='%s'",
                effective_uplink, expected_ack);
            return FALSE;
        }
    } else if (uplink_mode && *uplink_mode) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    "uplink_mode is only valid for bridge mode");
        return FALSE;
    }
...
gboolean pcv_validate_pci_addr(const gchar *addr) {
    ...
    if (len != 12 || addr[4] != ':' || addr[7] != ':' || addr[10] != '.')
        return FALSE;
    for (gsize i = 0; i < len; i++) {
        if (i == 4 || i == 7 || i == 10)
            continue;
        if (!g_ascii_isxdigit(addr[i]) || g_ascii_isupper(addr[i]))
            return FALSE;
    }
