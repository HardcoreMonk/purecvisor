...
pcv_jwt_init(const gchar *secret)
{
    ...
    if (!secret || !*secret) {
        ...
        pcv_secure_wipe(rnd, sizeof rnd);                                                 
        PCV_LOG_WARN(JWT_LOG_DOM,
...
pcv_jwt_shutdown(void)
{
    ...
    if (g_jwt_secret) { pcv_secure_wipe(g_jwt_secret, g_jwt_secret_len); g_free(g_jwt_secret); }
    g_jwt_secret     = NULL;
    g_jwt_secret_len = 0;
...
gchar *
pcv_jwt_sign_scoped(const gchar *subject,
                    guint        expires_in,
                    const gchar *client_ip,
                    const gchar *scope,
                    GError     **error)
{
    ...
    if (scope && *scope) {
        json_builder_set_member_name(jb, "scope");
        json_builder_add_string_value(jb, scope);
    }
...
static JsonParser *
_verify_core(const gchar *token_or_bearer,
             JsonObject **out_obj,
             GError     **error)
{
    g_return_val_if_fail(token_or_bearer, NULL);
    g_return_val_if_fail(out_obj, NULL);
    *out_obj = NULL;
               ...
               GError     **error)
{
    ...
    JsonObject *obj    = NULL;
    JsonParser *parser = _verify_core(token_or_bearer, &obj, error);
    if (!parser) return NULL;

                                                                   
                                                                                   
                                            
    if (json_object_has_member(obj, "scope")) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_PERMISSION_DENIED,
                    "JWT scoped token rejected by pcv_jwt_verify (use pcv_jwt_verify_scoped)");
        g_object_unref(parser);
        return NULL;
    }
...
gchar *
pcv_jwt_verify_scoped(const gchar *token_or_bearer,
                      const gchar *client_ip,
                      gchar      **out_scope,
                      GError     **error)
{
    g_return_val_if_fail(token_or_bearer, NULL);
    g_return_val_if_fail(out_scope, NULL);
    *out_scope = NULL;
    ...
    if (client_ip && *client_ip && json_object_has_member(obj, "ip")) {
        const gchar *bound_ip = json_object_get_string_member(obj, "ip");
        if (bound_ip && g_strcmp0(bound_ip, client_ip) != 0) {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_PERMISSION_DENIED,
                        "JWT IP binding mismatch");
            g_object_unref(parser);
    ...
    const gchar *sub   = json_object_get_string_member_with_default(obj, "sub", NULL);
    const gchar *jti   = json_object_get_string_member_with_default(obj, "jti", NULL);
    const gchar *scope = json_object_get_string_member_with_default(obj, "scope", NULL);
    gchar *jti_copy   = jti   ? g_strdup(jti)   : NULL;
    gchar *subject    = sub   ? g_strdup(sub)   : NULL;
    gchar *scope_copy = scope ? g_strdup(scope) : NULL;
    g_object_unref(parser);                                                
    ...
    if (!subject) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
                    "JWT missing 'sub' claim");
        g_free(jti_copy); g_free(scope_copy);
        return NULL;
    }
    ...
    if (jti_copy && pcv_jwt_blacklist_check(jti_copy)) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_PERMISSION_DENIED,
                    "JWT token revoked (logout)");
        g_free(subject); g_free(jti_copy); g_free(scope_copy);
        return NULL;
    }
    g_free(jti_copy);

    *out_scope = scope_copy;
    return subject;
}
