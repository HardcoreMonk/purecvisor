...
#include "modules/dispatcher/handler_vpc.h"
...
#ifndef PCV_DISPATCHER_POLICY_ONLY
...
static const PcvMethodPolicy g_method_policies[] = {
    ...
    { "tenant_overlay.create",     2 },
    { "tenant_overlay.delete",     2 },
    { "tenant_overlay.list",       2 },
    { "tenant_overlay.get",        2 },
    { "tenant_overlay.attach_vm",  2 },
    { "tenant_overlay.detach_vm",  2 },
                                                                     
                                                                      
    { "vpc.list",                   0 },
    { "vpc.get",                    0 },
    { "vpc.subnet.list",            0 },
    { "vpc.attachment.list",        0 },
    { "vpc.service.list",           0 },
    { "vpc.status",                 0 },
    { "vpc.backend.list",           0 },
    { "vpc.create",                 1 },
    ...
    { "vpc.subnet.create",          1 },
    { "vpc.subnet.delete",          1 },
    { "vpc.attachment.create",      1 },
    { "vpc.attachment.delete",      1 },
    { "vpc.service.publish",        1 },
    { "vpc.service.unpublish",      1 },
    { "vpc.reconcile",              2 },
    { "tls.reload",                2 },
    ...
    { "debug.trace.start",          2 },
    { "debug.trace.stop",           2 },
    { "debug.trace.status",         2 },
    { "debug.trace.list",           2 },
    { "debug.trace.report",         2 },
                                                                   
                                                            
                                                          
                                                             
    { "suricata.policy.set",        2 },
    { "suricata.policy.get",        2 },
    { "suricata.rules.update",      2 },
    { "suricata.status",            2 },
    { "suricata.ips.status",        2 },                                             
    { "suricata.ips.enable",        2 },
    { "suricata.ips.disable",       2 },
    ...
    { "ai.healing.approve",        2 },                               
    { "ai.healing.reject",         2 },                   
    { "node.resume",               2 },                                     
    { "security_group.rule.add",   2 },                                           
...
static gboolean
qos_applicable(const gchar *network_mode, const gchar *nic_type)
{
    return pcv_qos_vm_requires_sla(network_mode, nic_type);
}
...
static const gchar *
_dispatcher_audit_target_from_params(const gchar *method, JsonObject *params)
{
    const gchar *target = NULL;
    if (g_str_has_prefix(method ? method : "", "container."))
        target = _container_owner_scope_target_from_params(method, params);
    else
        target = _vm_owner_scope_target_from_params(method, params);
    if (target && *target) return target;

    static const gchar *keys[] = {
        "username", "network", "bridge", "tenant", "event_id", "job_id", NULL
    };
    for (guint i = 0; keys[i]; i++) {
        target = _json_string_member(params, keys[i]);
        if (target && *target) return target;
    }
    return NULL;
}
...
static void handle_vm_create(PureCVisorDispatcher *self, JsonObject *params,
    ...
    {
        g_autofree gchar *resolved_bridge = purecvisor_vm_resolve_network_bridge(bridge);
        if (resolved_bridge && pcv_vpc_bridge_is_managed(resolved_bridge)) {
            gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_CONFLICT,
    ...
    const gchar *network_mode = nullptr;
    const gchar *tenant = nullptr;
    if (json_object_has_member(params, "network_mode")) {
        network_mode = json_object_get_string_member(params, "network_mode");
    }
    if (json_object_has_member(params, "tenant")) {
        tenant = json_object_get_string_member(params, "tenant");
    }
    {
        GError *nmt_err = nullptr;
        if (!pcv_vm_validate_network_mode_tenant(network_mode, tenant, &nmt_err)) {
            gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
                nmt_err ? nmt_err->message : "Invalid network_mode/tenant combination");
            pure_uds_server_send_response(server, connection, err);
            g_free(err);
            g_clear_error(&nmt_err);
            return;
        }
    }
    ...
    gboolean vm_qos_required = qos_applicable(network_mode, nic_type);
    PcvQosSla vm_qos_sla;
    memset(&vm_qos_sla, 0, sizeof vm_qos_sla);
    if (vm_qos_required) {
        GError *qos_err = nullptr;
        if (!pcv_qos_sla_from_json(params, &vm_qos_sla, &qos_err)) {
            gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
                qos_err ? qos_err->message :
                "qos_min_mbps/qos_max_mbps required for bridge/tenant-overlay VMs");
            pure_uds_server_send_response(server, connection, err);
            g_free(err);
...
{
    GError *err = nullptr;
    gchar *response_str = handle_monitor_fleet(params, rpc_id, &err);
...
static void
_wipe_json_string_member(JsonObject *object, const gchar *key)
{
    ...
    if (value)
        pcv_secure_wipe((gpointer)value, strlen(value));
...
static void _handle_alert_config_set(JsonObject *params, const gchar *rpc_id,
                                      UdsServer *server, GSocketConnection *connection)
{
    JsonNode *expected_node = params
        ? json_object_get_member(params, "expected_revision")
        : NULL;
    if (!expected_node
        || !JSON_NODE_HOLDS_VALUE(expected_node)
        || json_node_get_value_type(expected_node) != G_TYPE_INT64
        || json_node_get_int(expected_node) < 1) {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "expected_revision must be a positive integer");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(NULL, "alert.config.set", "config", "fail", PURE_RPC_ERR_INVALID_PARAMS, 0, "local");
        _wipe_json_string_member(params, "webhook_secret");
        return;
    }
    ...
    for (GList *it = members; it; it = it->next) {
        ...
        if (g_strcmp0(key, "expected_revision") != 0) {
            json_object_set_member(
                patch, key,
                json_node_copy(json_object_get_member(params, key)));
        }
    }
    g_list_free(members);

    PcvAlertConfigSetResult result = pcv_alert_engine_set_config(
        patch, json_node_get_int(expected_node));
    _wipe_json_string_member(patch, "webhook_secret");
    _wipe_json_string_member(params, "webhook_secret");
    json_object_unref(patch);
    if (result == PCV_ALERT_CONFIG_SET_OK) {
        JsonObject *cfg = pcv_alert_engine_get_config();
        JsonNode *node = json_node_new(JSON_NODE_OBJECT);
        json_node_take_object(node, cfg);
        gchar *resp = pure_rpc_build_success_response(rpc_id, node);
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(NULL, "alert.config.set", "config", "ok", 0, 0, "local");
    } else if (result == PCV_ALERT_CONFIG_SET_CONFLICT) {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_CONFLICT,
            "Alert config revision conflict");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(NULL, "alert.config.set", "config", "fail", PURE_RPC_ERR_CONFLICT, 0, "local");
    } else {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INVALID_PARAMS, "Invalid alert config");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(NULL, "alert.config.set", "config", "fail", PURE_RPC_ERR_INVALID_PARAMS, 0, "local");
    }
...
{
    (void)params;
    PcvAlertConfigSetResult apply_result =
        pcv_alert_engine_reload_daemon_config();

    if (apply_result != PCV_ALERT_CONFIG_SET_OK) {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "Invalid alert daemon config");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(NULL, "alert.config.reload", "config", "fail", PURE_RPC_ERR_INVALID_PARAMS, 0, "local");
        return;
    }
    ...
    pure_uds_server_send_response(server, connection, resp);
    g_free(resp);
    pcv_audit_log(NULL, "alert.config.reload", "config", "ok", 0, 0, "local");
}
                                   ...
                                   UdsServer *server, GSocketConnection *connection)
{
    ...
    g_free(r); pcv_secure_free_str(&key_out);
}
                                   ...
                                   UdsServer *server, GSocketConnection *connection)
{
    ...
    PcvAlertConfigSetResult reload_result =
        pcv_alert_engine_reload_daemon_config();
    if (reload_result != PCV_ALERT_CONFIG_SET_OK) {
                                   ...
                                   UdsServer *server, GSocketConnection *connection)
{
    ...
    if (!metric) {
        ...
        pure_uds_server_send_response(server, connection, r); g_free(r);
        pcv_audit_log(NULL, "alert.silence", "", "fail", PURE_RPC_ERR_INVALID_PARAMS, 0, "local");
        return;
    }
    ...
    gchar *r = pure_rpc_build_success_response(rpc_id, n);
    pure_uds_server_send_response(server, connection, r); g_free(r);
    pcv_audit_log(NULL, "alert.silence", metric, "ok", 0, 0, "local");
}
...
static void _handle_alert_ack(JsonObject *params, const gchar *rpc_id,
                               UdsServer *server, GSocketConnection *connection)
{
                                                       
                                                         
    JsonNode *id_node = params ? json_object_get_member(params, "alert_id") : NULL;
    if (!id_node
        || !JSON_NODE_HOLDS_VALUE(id_node)
        || json_node_get_value_type(id_node) != G_TYPE_INT64) {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "alert_id must be an integer");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(NULL, "alert.ack", "", "fail", PURE_RPC_ERR_INVALID_PARAMS, 0, "local");
        return;
    }

    gint64 alert_id_val = json_node_get_int(id_node);
    gboolean acked = pcv_alert_acknowledge(alert_id_val);
    JsonNode *node = json_node_new(JSON_NODE_VALUE);
    json_node_set_boolean(node, acked);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
    g_free(resp);
    gchar id_buf[32];
    g_snprintf(id_buf, sizeof(id_buf), "%" G_GINT64_FORMAT, alert_id_val);
    pcv_audit_log(NULL, "alert.ack", id_buf, "ok", 0, 0, "local");
}
...
static void
_push_ep_summary(gchar *buf, gsize buf_len, const gchar *endpoint)
{
    g_snprintf(buf, buf_len, "%.80s", endpoint ? endpoint : "");
}

                                                                  
static const gchar *
_push_caller_username(JsonObject *params, GSocketConnection *connection)
{
    const gchar *sub = _dispatcher_caller_subject(params, connection);
    return (sub && *sub) ? sub : "admin";
}
...
static void _handle_push_vapid_get(JsonObject *params, const gchar *rpc_id,
                                    UdsServer *server, GSocketConnection *connection)
{
    (void)params;
    gchar *key = pcv_webpush_vapid_public();
    if (!key) {
                                                         
                                               
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INTERNAL_ERROR,
            "VAPID 공개키를 사용할 수 없습니다 (webpush 미초기화 또는 키 생성 실패)");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        return;
    }
    ...
    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "key", key);
    g_free(key);
    JsonNode *n = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(n, res);
...
static void _handle_push_subscribe(JsonObject *params, const gchar *rpc_id,
                                    UdsServer *server, GSocketConnection *connection)
{
    const gchar *endpoint = _json_string_member(params, "endpoint");
    const gchar *p256dh   = NULL;
    const gchar *auth     = NULL;

                                                                            
                                                
                  
    JsonNode *keys_node = params ? json_object_get_member(params, "keys") : NULL;
    if (keys_node && JSON_NODE_HOLDS_OBJECT(keys_node)) {
        JsonObject *keys = json_node_get_object(keys_node);
        p256dh = _json_string_member(keys, "p256dh");
        auth   = _json_string_member(keys, "auth");
    }

    if (!endpoint || !*endpoint || !p256dh || !*p256dh || !auth || !*auth) {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "endpoint, keys.p256dh, keys.auth 는 모두 비어 있지 않은 문자열이어야 합니다");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(NULL, "push.subscribe", "", "fail",
                      PURE_RPC_ERR_INVALID_PARAMS, 0, "local");
        return;
    }

    gchar ep_buf[96];
    _push_ep_summary(ep_buf, sizeof ep_buf, endpoint);
    const gchar *username = _push_caller_username(params, connection);

    GError *err = NULL;
    if (!pcv_webpush_subscribe(username, endpoint, p256dh, auth, &err)) {
                                                               
                                                                     
                                                           
                                                                   
                                                           
        gboolean client_error =
            g_error_matches(err, pcv_webpush_error_quark(), WP_ERR_ARG)
            || g_error_matches(err, pcv_webpush_error_quark(), WP_ERR_BLOCKED)
            || g_error_matches(err, pcv_webpush_error_quark(), WP_ERR_LIMIT);
        gint rpc_err = client_error ? PURE_RPC_ERR_INVALID_PARAMS : PURE_RPC_ERR_INTERNAL_ERROR;
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, rpc_err,
            err ? err->message : "구독 등록에 실패했습니다");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(username, "push.subscribe", ep_buf, "fail",
                      rpc_err, 0, "local");
        g_clear_error(&err);
        return;
    }
    ...
    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "status", "subscribed");
    JsonNode *n = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(n, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, n);
    pure_uds_server_send_response(server, connection, resp);
    g_free(resp);
    pcv_audit_log(username, "push.subscribe", ep_buf, "ok", 0, 0, "local");
}
...
static void _handle_push_unsubscribe(JsonObject *params, const gchar *rpc_id,
                                      UdsServer *server, GSocketConnection *connection)
{
    const gchar *endpoint = _json_string_member(params, "endpoint");
    if (!endpoint || !*endpoint) {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "endpoint 는 비어 있지 않은 문자열이어야 합니다");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(NULL, "push.unsubscribe", "", "fail",
                      PURE_RPC_ERR_INVALID_PARAMS, 0, "local");
        return;
    }
    ...
    _push_ep_summary(ep_buf, sizeof ep_buf, endpoint);
    const gchar *username = _push_caller_username(params, connection);
    gint         role     = _dispatcher_caller_role(params, connection);
                                                          
    const gchar *scope    = (role >= PCV_ROLE_ADMIN) ? NULL : username;

    GError *err = NULL;
    if (!pcv_webpush_unsubscribe(endpoint, scope, &err)) {
                                                        
                                                      
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_NOT_FOUND,
            err ? err->message : "해당 구독을 찾을 수 없습니다");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(username, "push.unsubscribe", ep_buf, "fail",
                      PURE_RPC_ERR_NOT_FOUND, 0, "local");
        g_clear_error(&err);
        return;
    }

    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "status", "unsubscribed");
    JsonNode *n = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(n, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, n);
    pure_uds_server_send_response(server, connection, resp);
    g_free(resp);
    pcv_audit_log(username, "push.unsubscribe", ep_buf, "ok", 0, 0, "local");
}
...
static void _handle_push_mine(JsonObject *params, const gchar *rpc_id,
                               UdsServer *server, GSocketConnection *connection)
{
    const gchar *username = _push_caller_username(params, connection);

                                                   
    JsonArray *arr = pcv_webpush_list_mine(username);
    guint      n   = json_array_get_length(arr);

    JsonObject *res = json_object_new();
    json_object_set_array_member(res, "subscriptions", arr);               
    json_object_set_int_member(res, "count", n);
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
...
static void _handle_push_list(JsonObject *params, const gchar *rpc_id,
                               UdsServer *server, GSocketConnection *connection)
{
    (void)params;
                                                   
    JsonArray *arr = pcv_webpush_list();
    JsonObject *res = json_object_new();
    json_object_set_array_member(res, "subscriptions", arr);               
    JsonNode *n = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(n, res);
...
static void _handle_push_vapid_rotate(JsonObject *params, const gchar *rpc_id,
                                       UdsServer *server, GSocketConnection *connection)
{
    ...
    GError *err     = NULL;
    guint   revoked = pcv_webpush_vapid_rotate(&err);
    if (err) {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INTERNAL_ERROR, err->message);
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(username, "push.vapid.rotate", "", "fail",
    ...
    JsonObject *res = json_object_new();
    json_object_set_int_member(res, "revoked", (gint64)revoked);
    json_object_set_string_member(res, "key", key);
    g_free(key);
    JsonNode *n = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(n, res);
    ...
    gchar cnt_buf[32];
    g_snprintf(cnt_buf, sizeof cnt_buf, "revoked=%u", revoked);
    pcv_audit_log(username, "push.vapid.rotate", cnt_buf, "ok", 0, 0, "local");
}
...
static void _handle_push_test(JsonObject *params, const gchar *rpc_id,
                               UdsServer *server, GSocketConnection *connection)
{
                                                       
    const gchar *target = _json_string_member(params, "username");
    if (target && !*target) target = NULL;

    const gchar *username = _push_caller_username(params, connection);

    GError *err = NULL;
    if (!pcv_webpush_send_test(target, &err)) {
        gchar *resp = pure_rpc_build_error_response(
            rpc_id, PURE_RPC_ERR_INTERNAL_ERROR,
            err ? err->message : "테스트 알림 큐잉에 실패했습니다");
        pure_uds_server_send_response(server, connection, resp);
        g_free(resp);
        pcv_audit_log(username, "push.test", target ? target : "*", "fail",
                      PURE_RPC_ERR_INTERNAL_ERROR, 0, "local");
        g_clear_error(&err);
        return;
    }
    ...
    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "status", "queued");
    JsonNode *n = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(n, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, n);
    pure_uds_server_send_response(server, connection, resp);
    g_free(resp);
    pcv_audit_log(username, "push.test", target ? target : "*", "ok", 0, 0, "local");
}
                                          ...
                                          UdsServer *server, GSocketConnection *connection)
{
    ...
    if (!ctr_name || !pcv_validate_vm_name(ctr_name)) {
        ...
        pure_uds_server_send_response(server, connection, e); g_free(e);
    } else if (pcv_vpc_bridge_is_managed(bridge ?: PCV_LXC_DEFAULT_BRIDGE)) {
        gchar *e = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_CONFLICT,
            "VPC-managed bridges cannot be attached through container.nic.attach");
        pure_uds_server_send_response(server, connection, e); g_free(e);
    } else {
        GError *e = nullptr;
                              ...
                              UdsServer *server, GSocketConnection *connection)
{
    ...
    if (pcv_vpc_vm_is_attached(source)) {
        gchar *e = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_CONFLICT,
            "VPC-attached VMs must detach before vm.clone to avoid copying managed NIC identity");
        pure_uds_server_send_response(server, connection, e); g_free(e);
        return;
    }
                                       ...
                                       UdsServer *server, GSocketConnection *connection)
{
    ...
    pcv_audit_log(NULL, "webhook.dlq.retry", "dlq", "ok", 0, 0, "local");
}
                                           ...
                                           UdsServer *server, GSocketConnection *connection)
{
    ...
    if (!vm_name || !sg_name) {
        ...
        extern gboolean pcv_tenant_overlay_vm_in_any_tenant(const gchar *vm);
        if (pcv_tenant_overlay_vm_in_any_tenant(vm_name)) {
            gchar *resp = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_ZFS_OPERATION,
                "SG와 tenant-overlay는 상호 배타입니다: 암호 격리=tenant-overlay, "
                "per-VM L3/L4 정책=SG(bridge 모드) — overlay 멤버 VM에는 SG를 바인딩할 수 없습니다");
            pure_uds_server_send_response(server, connection, resp);
            g_free(resp);
            return;
        }
...
void purecvisor_dispatcher_dispatch(PureCVisorDispatcher *self,
    ...
    const gchar *_audit_sub = _dispatcher_caller_subject(params, connection);
    const gchar *_audit_target = _dispatcher_audit_target_from_params(method, params);
    gchar *audit_username = g_strdup((_audit_sub && *_audit_sub) ? _audit_sub : "-");
    gchar *audit_target = g_strdup((_audit_target && *_audit_target) ? _audit_target : "");
    pcv_rpc_completion_begin(method);
...
rpc_done:
                                                                        ...
                                                                        
    {
        ...
        gboolean is_async = pcv_dispatcher_is_async_method(method);
                                                                              
                                                               
                                                                          
                                                    
        gboolean success = pcv_rpc_completion_dispatch_succeeded(completion, is_async);
        gint error_code = success ? 0
            : (completion.response_observed
                ? completion.error_code : PURE_RPC_ERR_INTERNAL_ERROR);
        pcv_prom_rpc_end(method, success, dur_ms);
                                                                     
                                                           
                                                           
        if (!is_async && !completion.direct_audit) {
            pcv_audit_log(audit_username, method, audit_target,
                          success ? "ok" : "fail", error_code,
                          (gint64)dur_ms, "local");
        }
...
{
    (void)params;
    JsonObject *result = cpu_allocator_get_numa_info(global_allocator);                   
    JsonNode *n = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(n, result);
    gchar *resp = pure_rpc_build_success_response(rpc_id, n);
    pure_uds_server_send_response(server, connection, resp); g_free(resp);
                                       ...
                                       UdsServer *server, GSocketConnection *connection)
{
    ...
    gboolean secret_value =
        key && g_strcmp0(key, "webhook_secret") == 0;
    ...
    if (!section || !key || !value) {
        ...
        if (secret_value)
            _wipe_json_string_member(params, "value");
        return;
    }
    ...
    const gchar *conf_path = pcv_config_get_file_path();
    GError *error = nullptr;
    PcvAlertConfigSetResult set_result =
        pcv_daemon_config_set_value(
            conf_path, section, key, value, &error);
    ...
    if (set_result != PCV_ALERT_CONFIG_SET_OK) {
    ...
    if (secret_value) {
        json_object_set_boolean_member(ok, "value_configured",
                                       value[0] != '\0');
        _wipe_json_string_member(params, "value");
    } else {
...
static void
_handle_qos_vm_set(JsonObject *params, const gchar *rpc_id,
                    UdsServer *server, GSocketConnection *connection)
{
    const gchar *vm = _json_string_member(params, "vm");
    if (!vm || !*vm || !pcv_validate_vm_name(vm)) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "Missing/invalid required parameter: vm");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    ...
    PcvQosSla sla;
    GError *verr = NULL;
    if (!pcv_qos_sla_from_json(params, &sla, &verr)) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            verr ? verr->message : "invalid qos_min_mbps/qos_max_mbps");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&verr);
        return;
    ...
    extern virDomainPtr pure_virt_get_domain(virConnectPtr conn, const gchar *identifier);
    virDomainPtr dom = pure_virt_get_domain(conn, vm);
    if (!dom) {
    ...
    GError *werr = NULL;
    if (!pcv_vm_qos_metadata_write(dom, &sla, &werr)) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_ZFS_OPERATION,
            werr ? werr->message : "qos metadata write failed");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&werr);
        pcv_audit_log(NULL, "qos.vm.set", vm, "fail", PURE_RPC_ERR_ZFS_OPERATION, 0, "local");
        virDomainFree(dom);
        virt_conn_pool_release(conn);
        return;
    }
    ...
    if (virDomainIsActive(dom) == 1) {
        ...
        if (pcv_qos_lookup_applied(vm, &tenant, &iface)) {
            PcvQosSla apply_sla = sla;
            g_strlcpy(apply_sla.tenant, tenant, sizeof(apply_sla.tenant));
            g_strlcpy(apply_sla.vm, vm, sizeof(apply_sla.vm));
            GError *aerr = NULL;
            if (pcv_qos_apply_vm(iface, &apply_sla, &aerr)) {
                live_applied = TRUE;
            } else {
                g_warning("[qos.vm.set] live 재적용 실패(vm=%s): %s",
                          vm, aerr ? aerr->message : "unknown");
                g_clear_error(&aerr);
            }
            g_free(tenant);
            g_free(iface);
    ...
    pcv_audit_log(NULL, "qos.vm.set", vm, "ok", 0, 0, "local");

    GError *save_err = NULL;
    if (!pcv_qos_ids_save(PCV_QOS_IDS_PATH, &save_err)) {
        g_warning("[qos.vm.set] qos_ids.json 저장 실패(WARN — 결정성에 영향 가능, vm=%s): %s",
                  vm, save_err ? save_err->message : "unknown");
        g_clear_error(&save_err);
    }

    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "vm", vm);
    json_object_set_int_member(res, "qos_min_mbps", sla.min_mbps);
    json_object_set_int_member(res, "qos_max_mbps", sla.max_mbps);
    json_object_set_int_member(res, "qos_burst_kb", sla.burst_kb);
    json_object_set_boolean_member(res, "live_applied", live_applied);
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
...
static void
_handle_qos_vm_get(JsonObject *params, const gchar *rpc_id,
                    UdsServer *server, GSocketConnection *connection)
{
    const gchar *vm = _json_string_member(params, "vm");
    if (!vm || !*vm || !pcv_validate_vm_name(vm)) {
    ...
    PcvQosSla sla;
    PcvQosMetaResult mr = pcv_vm_qos_metadata_read(dom, &sla);
    virDomainFree(dom);
    virt_conn_pool_release(conn);
    ...
    json_object_set_string_member(res, "vm", vm);
    switch (mr) {
    case PCV_QOS_META_OK:
        json_object_set_string_member(res, "status", "ok");
        json_object_set_int_member(res, "qos_min_mbps", sla.min_mbps);
        json_object_set_int_member(res, "qos_max_mbps", sla.max_mbps);
        json_object_set_int_member(res, "qos_burst_kb", sla.burst_kb);
        break;
    case PCV_QOS_META_ABSENT:
        json_object_set_string_member(res, "status", "absent");
        break;
    case PCV_QOS_META_INVALID:
    default:
        json_object_set_string_member(res, "status", "invalid");
        break;
    }
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
...
static void
_handle_qos_tenant_set(JsonObject *params, const gchar *rpc_id,
                        UdsServer *server, GSocketConnection *connection)
{
    const gchar *tenant = _json_string_member(params, "tenant");
    if (!tenant || !*tenant || !pcv_validate_vm_name(tenant)) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "Missing/invalid required parameter: tenant");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    gint min_mbps = json_object_has_member(params, "min_mbps")
        ? (gint)json_object_get_int_member(params, "min_mbps") : 0;
    gint max_mbps = json_object_has_member(params, "max_mbps")
        ? (gint)json_object_get_int_member(params, "max_mbps") : 0;
    if (min_mbps < 0 || max_mbps < 0) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "min_mbps/max_mbps must be >= 0");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    ...
    if (!pcv_qos_tenant_sla_set(tenant, (guint32)min_mbps, (guint32)max_mbps,
                                &live_applied, &live_error, &error)) {
        ...
        gint code = (error && error->domain == G_IO_ERROR
                     && error->code == G_IO_ERROR_INVALID_ARGUMENT)
            ? PURE_RPC_ERR_INVALID_PARAMS : PURE_RPC_ERR_ZFS_OPERATION;
        gchar *err = pure_rpc_build_error_response(rpc_id, code,
            error ? error->message : "qos.tenant.set failed");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&error);
        g_free(live_error);                                            
        pcv_audit_log(NULL, "qos.tenant.set", tenant, "fail", code, 0, "local");
        return;
    }
    ...
    pcv_audit_log(NULL, "qos.tenant.set", tenant, "ok", 0, 0, "local");

    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "tenant", tenant);
    json_object_set_int_member(res, "min_mbps", min_mbps);
    json_object_set_int_member(res, "max_mbps", max_mbps);
    json_object_set_boolean_member(res, "live_applied", live_applied);
    if (live_error) {
        json_object_set_string_member(res, "live_error", live_error);
        g_free(live_error);
    }
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
...
static void
_handle_qos_tenant_get(JsonObject *params, const gchar *rpc_id,
                        UdsServer *server, GSocketConnection *connection)
{
    const gchar *tenant = _json_string_member(params, "tenant");
    if (!tenant || !*tenant || !pcv_validate_vm_name(tenant)) {
    ...
    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "tenant", tenant);
    if (pcv_qos_tenant_sla_get(tenant, &sla)) {
        json_object_set_string_member(res, "status", "ok");
        json_object_set_int_member(res, "min_mbps", sla.min_mbps);
        json_object_set_int_member(res, "max_mbps", sla.max_mbps);
    } else {
        json_object_set_string_member(res, "status", "absent");
    }
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
...
static void
_handle_qos_stats(JsonObject *params, const gchar *rpc_id,
                   UdsServer *server, GSocketConnection *connection)
{
    const gchar *tenant_filter = _json_string_member(params, "tenant");
    const gchar *vm_filter     = _json_string_member(params, "vm");
    ...
    for (guint i = 0; i < stats->len; i++) {
        ...
        if (tenant_filter && g_strcmp0(s->tenant, tenant_filter) != 0) continue;
        if (vm_filter && g_strcmp0(s->vm, vm_filter) != 0) continue;

        JsonObject *entry = json_object_new();
        json_object_set_string_member(entry, "classid", s->classid);
        json_object_set_string_member(entry, "tenant", s->tenant ? s->tenant : "");
        json_object_set_string_member(entry, "vm", s->vm ? s->vm : "");
        json_object_set_int_member(entry, "bytes", (gint64)s->bytes);
        json_object_set_int_member(entry, "drops", (gint64)s->drops);
        json_array_add_object_element(arr, entry);
    }
    g_ptr_array_unref(stats);

    JsonObject *res = json_object_new();
    json_object_set_array_member(res, "classes", arr);
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
...
static void
_handle_qos_chaos_start(JsonObject *params, const gchar *rpc_id,
                         UdsServer *server, GSocketConnection *connection)
{
    const gchar *vm = _json_string_member(params, "vm");
    if (!vm || !*vm || !pcv_validate_vm_name(vm)) {
    ...
    const gchar *profile = _json_string_member(params, "profile");
    if (!profile || !*profile) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
    ...
    if (!json_object_has_member(params, "timebox_sec")) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "Missing required parameter: timebox_sec");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    gint timebox_i = (gint)json_object_get_int_member(params, "timebox_sec");
    if (timebox_i < 0) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "timebox_sec must be >= 0");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    guint timebox_sec = (guint)timebox_i;
    gboolean dry_run = pcv_qos_chaos_resolve_dry_run(params);
    const gchar *admin = _dispatcher_caller_subject(params, connection);

    GError *error = NULL;
    if (!pcv_qos_chaos_start(vm, profile, timebox_sec, admin, dry_run, &error)) {
                                                          
                                                
                                                   
        gint code = (error && g_strstr_len(error->message, -1, "not QoS-tracked"))
            ? PURE_RPC_ERR_VM_NOT_FOUND : PURE_RPC_ERR_ZFS_OPERATION;
        gchar *err = pure_rpc_build_error_response(rpc_id, code,
            error ? error->message : "qos.chaos.start failed");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&error);
        return;
    ...
    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "vm", vm);
    json_object_set_boolean_member(res, "dry_run", dry_run);
    json_object_set_string_member(res, "profile", profile);
    json_object_set_int_member(res, "timebox_sec", (gint64)timebox_sec);
    json_object_set_string_member(res, "note",
        "chaos targets the VM leaf on " PCV_QOS_IFB_DEV " (cake<->netem swap) — "
        "affects upstream (VM-originated) traffic only");

    gchar *tenant = NULL, *iface = NULL, *cid = NULL;
    if (pcv_qos_lookup_applied(vm, &tenant, &iface)) {
        cid = pcv_qos_classid(tenant, vm);
        json_object_set_string_member(res, "classid", cid);
    }
    g_free(tenant);
    g_free(iface);
    ...
    if (dry_run) {
        ...
        if (tokens) {
            JsonArray *argv_arr = json_array_new();
            const gchar *prefix[] = {"tc", "qdisc", "replace", "dev", PCV_QOS_IFB_DEV, "parent"};
            for (guint i = 0; i < G_N_ELEMENTS(prefix); i++)
                json_array_add_string_element(argv_arr, prefix[i]);
            json_array_add_string_element(argv_arr, cid ? cid : "?");
            json_array_add_string_element(argv_arr, "netem");
            for (guint i = 0; tokens[i]; i++)
                json_array_add_string_element(argv_arr, tokens[i]);
            json_object_set_array_member(res, "argv", argv_arr);
            g_strfreev(tokens);
        }
        g_clear_error(&perr);
    } else {
        json_object_set_int_member(res, "expires_at",
            (gint64)(g_get_real_time() / G_USEC_PER_SEC + timebox_sec));
    }
    g_free(cid);
...
static void
_handle_qos_chaos_stop(JsonObject *params, const gchar *rpc_id,
                        UdsServer *server, GSocketConnection *connection)
{
    const gchar *vm = _json_string_member(params, "vm");
    if (!vm || !*vm || !pcv_validate_vm_name(vm)) {
    ...
    const gchar *admin = _dispatcher_caller_subject(params, connection);
    GError *error = NULL;
    if (!pcv_qos_chaos_stop(vm, admin, &error)) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_ZFS_OPERATION,
            error ? error->message : "qos.chaos.stop failed");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&error);
        return;
...
static void
_handle_qos_chaos_status(JsonObject *params, const gchar *rpc_id,
                          UdsServer *server, GSocketConnection *connection)
{
    (void)params;
    GPtrArray *entries = pcv_qos_chaos_status();

    JsonArray *arr = json_array_new();
    for (guint i = 0; i < entries->len; i++) {
        PcvQosChaosStatusEntry *s = g_ptr_array_index(entries, i);
        JsonObject *o = json_object_new();
        json_object_set_string_member(o, "vm", s->vm);
        json_object_set_string_member(o, "profile", s->profile);
        json_object_set_string_member(o, "admin", s->admin);
        json_object_set_int_member(o, "timebox_sec", s->timebox_sec);
        json_object_set_int_member(o, "expires_at", s->expires_at);
        json_array_add_object_element(arr, o);
    }
    g_ptr_array_unref(entries);

    JsonObject *res = json_object_new();
    json_object_set_array_member(res, "active", arr);
    json_object_set_string_member(res, "note",
        "chaos targets the VM leaf on " PCV_QOS_IFB_DEV " (cake<->netem swap) — "
        "affects upstream (VM-originated) traffic only");
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
...
static gint
_trace_err_code(const GError *e)
{
    if (!e || e->domain != G_IO_ERROR) return PURE_RPC_ERR_ZFS_OPERATION;
    switch (e->code) {
        case G_IO_ERROR_INVALID_ARGUMENT: return PURE_RPC_ERR_INVALID_PARAMS;              
        case G_IO_ERROR_NOT_FOUND:        return PURE_RPC_ERR_NOT_FOUND;                   
        case G_IO_ERROR_EXISTS:                                                         
        case G_IO_ERROR_BUSY:             return PURE_RPC_ERR_BUSY;                    
        case G_IO_ERROR_TIMED_OUT:        return PURE_RPC_ERR_TIMEOUT;                     
        default:                          return PURE_RPC_ERR_ZFS_OPERATION;                      
    }
}
...
static void
_handle_debug_trace_start(JsonObject *params, const gchar *rpc_id,
                          UdsServer *server, GSocketConnection *connection)
{
    if (!json_object_has_member(params, "timebox_sec")) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
    ...
    PcvTraceFilter f = {0};
    const gchar *s;
    if ((s = _json_string_member(params, "vm")))     g_strlcpy(f.vm,     s, sizeof(f.vm));
    if ((s = _json_string_member(params, "tenant"))) g_strlcpy(f.tenant, s, sizeof(f.tenant));
    if ((s = _json_string_member(params, "proto")))  g_strlcpy(f.proto,  s, sizeof(f.proto));
    if ((s = _json_string_member(params, "src_ip"))) g_strlcpy(f.src_ip, s, sizeof(f.src_ip));
    if ((s = _json_string_member(params, "dst_ip"))) g_strlcpy(f.dst_ip, s, sizeof(f.dst_ip));

    gint iv;
    if (_json_int_member(params, "src_port", &iv))    f.src_port    = (guint)iv;
    if (_json_int_member(params, "dst_port", &iv))    f.dst_port    = (guint)iv;
    if (_json_int_member(params, "timebox_sec", &iv)) f.timebox_sec = (guint)iv;

    if (json_object_has_member(params, "overlay"))
        f.overlay = json_object_get_boolean_member(params, "overlay");

    const gchar *admin = _dispatcher_caller_subject(params, connection);
    GError *error = NULL;
    gchar *trace_id = pcv_trace_start(&f, admin, &error);
    if (!trace_id) {
        gchar *err = pure_rpc_build_error_response(rpc_id, _trace_err_code(error),
            error ? error->message : "debug.trace.start failed");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&error);
        return;
    ...
    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "trace_id", trace_id);
    json_object_set_string_member(res, "out_dir", out_dir);
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
...
_handle_debug_trace_stop(JsonObject *params, const gchar *rpc_id,
                         UdsServer *server, GSocketConnection *connection)
{
    const gchar *trace_id = _json_string_member(params, "trace_id");
    ...
    GError *error = NULL;
    if (!pcv_trace_stop(trace_id, &error)) {
        gchar *err = pure_rpc_build_error_response(rpc_id, _trace_err_code(error),
            error ? error->message : "debug.trace.stop failed");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&error);
        return;
...
static void
_handle_debug_trace_status(JsonObject *params, const gchar *rpc_id,
                           UdsServer *server, GSocketConnection *connection)
{
    const gchar *trace_id = _json_string_member(params, "trace_id");               
    JsonObject *res = pcv_trace_status(trace_id);                   
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
...
static void
_handle_debug_trace_list(JsonObject *params, const gchar *rpc_id,
                         UdsServer *server, GSocketConnection *connection)
{
    (void)params;
    JsonArray *traces = pcv_trace_list();
    JsonObject *res = json_object_new();
    json_object_set_array_member(res, "traces", traces);
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
...
static void
_handle_debug_trace_report(JsonObject *params, const gchar *rpc_id,
                           UdsServer *server, GSocketConnection *connection)
{
    const gchar *trace_id = _json_string_member(params, "trace_id");
    if (!trace_id || !*trace_id) {
    ...
    GError *error = NULL;
    JsonObject *res = pcv_trace_report(trace_id, &error);
    if (!res) {
        gchar *err = pure_rpc_build_error_response(rpc_id, _trace_err_code(error),
            error ? error->message : "debug.trace.report failed");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&error);
        return;
...
static gboolean
_json_opt_bool_member(JsonObject *params, const gchar *key, gboolean *out)
{
    if (!params || !key || !json_object_has_member(params, key))
        return FALSE;
    JsonNode *node = json_object_get_member(params, key);
    if (!node || !JSON_NODE_HOLDS_VALUE(node) ||
        json_node_get_value_type(node) != G_TYPE_BOOLEAN)
        return FALSE;
    if (out) *out = json_node_get_boolean(node);
    return TRUE;
}
...
static struct {
    ...
    gchar   *last_error;                                   
    gint64   last_ts;                            
} g_suricata_rules_job;

typedef struct {
    gchar *url;
    gchar *admin;
} SuricataRulesUpdateCtx;

                                                                      
                                                   
static void
_suricata_rules_update_ctx_free(gpointer data)
{
    SuricataRulesUpdateCtx *ctx = data;
    if (!ctx) return;
    g_free(ctx->url);
    g_free(ctx->admin);
    g_free(ctx);
...
static void
_suricata_rules_update_worker(GTask *task, gpointer src, gpointer task_data,
                              GCancellable *cancellable)
{
    (void)task; (void)src; (void)cancellable;
    SuricataRulesUpdateCtx *ctx = task_data;

    GError *error = NULL;
                                                            
    gboolean ok = pcv_suricata_rules_update(ctx->url, ctx->admin, &error);
    ...
    if (ok && g_file_test(PCV_IPS_RULES_PATH, G_FILE_TEST_EXISTS)) {
        gboolean acquired = FALSE;
        gchar *saved_op = NULL;
        g_mutex_lock(&g_suricata_ips_job.mu);
        if (!g_suricata_ips_job.running) {
            g_suricata_ips_job.running = TRUE;
            saved_op = g_suricata_ips_job.last_op;                        
            g_suricata_ips_job.last_op = g_strdup("rules.regen");
            acquired = TRUE;
        }
        g_mutex_unlock(&g_suricata_ips_job.mu);

        if (!acquired) {
            PCV_LOG_WARN("suricata_ips",
                "원본 룰 갱신 후 IPS 파생본 재생성 생략 — 다른 IPS 작업 진행 중"
                "(파생본은 이전 원본 기준으로 남는다)");
        } else {
            GHashTable *sids = pcv_suricata_policy_drop_sids_snapshot();
            GError *rerr = NULL;
            gboolean regen = pcv_suricata_ips_rules_apply(sids, ctx->admin, &rerr);
            if (!regen) {
                PCV_LOG_WARN("suricata_ips",
                    "원본 룰 갱신 후 IPS 파생본 재생성 실패 — 파생본은 이전 원본 기준으로 "
                    "남는다(원본 갱신은 성공 유지): %s", rerr ? rerr->message : "unknown");
            }
            g_hash_table_destroy(sids);

            g_mutex_lock(&g_suricata_ips_job.mu);
            g_suricata_ips_job.running = FALSE;
            g_free(g_suricata_ips_job.last_result);
            g_suricata_ips_job.last_result = g_strdup(regen ? "ok" : "fail");
            g_free(g_suricata_ips_job.last_error);
            g_suricata_ips_job.last_error = regen ? NULL
                : g_strdup(rerr ? rerr->message : "unknown error");
            g_suricata_ips_job.last_ts = g_get_real_time() / G_USEC_PER_SEC;
            g_mutex_unlock(&g_suricata_ips_job.mu);
    ...
    g_mutex_lock(&g_suricata_rules_job.mu);
    g_suricata_rules_job.running = FALSE;
    g_free(g_suricata_rules_job.last_result);
    g_suricata_rules_job.last_result = g_strdup(ok ? "ok" : "fail");
    g_free(g_suricata_rules_job.last_error);
    g_suricata_rules_job.last_error = ok ? NULL
        : g_strdup(error ? error->message : "unknown error");
    g_suricata_rules_job.last_ts = g_get_real_time() / G_USEC_PER_SEC;
    g_mutex_unlock(&g_suricata_rules_job.mu);
...
static void
_handle_suricata_policy_set(JsonObject *params, const gchar *rpc_id,
                            UdsServer *server, GSocketConnection *connection)
{
    const gchar *tenant       = _json_string_member(params, "tenant");                      
    const gchar *profile      = _json_string_member(params, "profile");                     
    const gchar *auto_isolate = _json_string_member(params, "auto_isolate");                
    gboolean inspect_val = FALSE;
    gboolean has_inspect = _json_opt_bool_member(params, "inspect", &inspect_val);

    const gchar *admin = _dispatcher_caller_subject(params, connection);

    GError *error = NULL;
    gboolean ok = pcv_suricata_policy_set(tenant, has_inspect ? &inspect_val : NULL,
                                          profile, auto_isolate, &error);

                                                                 
    pcv_audit_log(admin ? admin : "system", "suricata.policy.set",
                  tenant ? tenant : (auto_isolate ? "auto_isolate" : "?"),
                  ok ? "ok" : "fail", ok ? 0 : (error ? error->code : -1), 0, "local");

    if (!ok) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            error ? error->message : "suricata.policy.set failed");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&error);
        return;
    ...
    JsonObject *res = pcv_suricata_policy_get_json(tenant);
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
...
static void
_handle_suricata_policy_get(JsonObject *params, const gchar *rpc_id,
                            UdsServer *server, GSocketConnection *connection)
{
    const gchar *tenant = _json_string_member(params, "tenant");                
    JsonObject *res = pcv_suricata_policy_get_json(tenant);
...
static void
_handle_suricata_rules_update(JsonObject *params, const gchar *rpc_id,
                              UdsServer *server, GSocketConnection *connection)
{
    const gchar *url = _json_string_member(params, "url");
    if (!url || !*url) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
    ...
    g_mutex_lock(&g_suricata_rules_job.mu);
    if (g_suricata_rules_job.running) {
        g_mutex_unlock(&g_suricata_rules_job.mu);
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_BUSY,
            "suricata rules update already in progress");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    g_suricata_rules_job.running = TRUE;
    g_free(g_suricata_rules_job.last_url);
    g_suricata_rules_job.last_url = g_strdup(url);
    g_mutex_unlock(&g_suricata_rules_job.mu);

    const gchar *admin = _dispatcher_caller_subject(params, connection);
    SuricataRulesUpdateCtx *ctx = g_new0(SuricataRulesUpdateCtx, 1);
    ...
    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "status", "started");
    json_object_set_string_member(res, "url", url);
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
...
static void
_handle_suricata_status(JsonObject *params, const gchar *rpc_id,
                        UdsServer *server, GSocketConnection *connection)
{
    (void)params;
    JsonObject *res = json_object_new();
    ...
    PcvSuricataState state = PCV_SURICATA_INACTIVE;
    gboolean binary_present = FALSE;
    pcv_suricata_engine_status_cached(&state, &binary_present);
    JsonObject *engine = json_object_new();
    json_object_set_string_member(engine, "state", pcv_suricata_state_str(state));
    json_object_set_boolean_member(engine, "binary_present", binary_present);
    json_object_set_object_member(res, "engine", engine);

                                                            
    guint64 crit = 0, warn = 0, info = 0, dropped = 0;
    pcv_suricata_get_stats(&crit, &warn, &info, &dropped);
    JsonObject *eve = json_object_new();
    json_object_set_boolean_member(eve, "running", pcv_suricata_eve_tail_running());
    json_object_set_int_member(eve, "alerts_crit", (gint64)crit);
    json_object_set_int_member(eve, "alerts_warn", (gint64)warn);
    json_object_set_int_member(eve, "alerts_info", (gint64)info);
    json_object_set_int_member(eve, "dropped", (gint64)dropped);
    json_object_set_object_member(res, "eve_tail", eve);

                                             
    JsonObject *rules = json_object_new();
    json_object_set_int_member(rules, "count", (gint64)pcv_suricata_rules_count());
    JsonObject *update = json_object_new();
    g_mutex_lock(&g_suricata_rules_job.mu);
    json_object_set_boolean_member(update, "running", g_suricata_rules_job.running);
    if (g_suricata_rules_job.last_result)
        json_object_set_string_member(update, "last_result", g_suricata_rules_job.last_result);
    else
        json_object_set_null_member(update, "last_result");
    if (g_suricata_rules_job.last_url)
        json_object_set_string_member(update, "last_url", g_suricata_rules_job.last_url);
    else
        json_object_set_null_member(update, "last_url");
    if (g_suricata_rules_job.last_error)
        json_object_set_string_member(update, "last_error", g_suricata_rules_job.last_error);
    else
        json_object_set_null_member(update, "last_error");
    json_object_set_int_member(update, "last_ts", g_suricata_rules_job.last_ts);
    g_mutex_unlock(&g_suricata_rules_job.mu);
    json_object_set_object_member(rules, "update", update);
    json_object_set_object_member(res, "rules", rules);

                                           
    gboolean enforce = FALSE;
    guint tenant_count = 0;
    pcv_suricata_policy_summary(&enforce, &tenant_count);
    JsonObject *policy = json_object_new();
    json_object_set_string_member(policy, "auto_isolate", enforce ? "enforce" : "dry_run");
    json_object_set_int_member(policy, "tenants", (gint64)tenant_count);
    json_object_set_object_member(res, "policy", policy);
...
typedef struct {
    ...
    gboolean fail_open;
    gchar   *admin;
} SuricataIpsToggleCtx;

                                                                        
                                                
static void
_suricata_ips_toggle_ctx_free(gpointer data)
{
    SuricataIpsToggleCtx *ctx = data;
    if (!ctx) return;
    g_free(ctx->admin);
    g_free(ctx);
}
...
static void
_suricata_ips_toggle_worker(GTask *task, gpointer src, gpointer task_data,
                            GCancellable *cancellable)
{
    (void)task; (void)src; (void)cancellable;
    SuricataIpsToggleCtx *ctx = task_data;
    const gchar *op = ctx->enable ? "enable" : "disable";

    GError *error = NULL;
    gboolean ok = ctx->enable
        ? pcv_suricata_ips_enable(ctx->queue_num, ctx->fail_open, &error)
        : pcv_suricata_ips_disable(&error);
    ...
    gchar method[32];
    g_snprintf(method, sizeof method, "suricata.ips.%s", op);
    pcv_audit_log(ctx->admin ? ctx->admin : "system", method, "suricata-ips",
                  ok ? "ok" : "fail", ok ? 0 : (error ? error->code : -1), 0, "local");

    g_mutex_lock(&g_suricata_ips_job.mu);
    g_suricata_ips_job.running = FALSE;
                                                                          
    g_free(g_suricata_ips_job.last_result); g_suricata_ips_job.last_result = g_strdup(ok ? "ok" : "fail");
    g_free(g_suricata_ips_job.last_error);  g_suricata_ips_job.last_error = ok ? NULL
        : g_strdup(error ? error->message : "unknown error");
    g_suricata_ips_job.last_ts = g_get_real_time() / G_USEC_PER_SEC;
    g_mutex_unlock(&g_suricata_ips_job.mu);
...
static void
_suricata_ips_toggle_async(gboolean enable, JsonObject *params, const gchar *rpc_id,
                           UdsServer *server, GSocketConnection *connection)
{
    const gchar *op = enable ? "enable" : "disable";

    g_mutex_lock(&g_suricata_ips_job.mu);
    if (g_suricata_ips_job.running) {
        g_mutex_unlock(&g_suricata_ips_job.mu);
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_BUSY,
            "suricata ips toggle already in progress");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    ...
    g_free(g_suricata_ips_job.last_op);
    g_suricata_ips_job.last_op = g_strdup(op);
    g_mutex_unlock(&g_suricata_ips_job.mu);

    const gchar *admin = _dispatcher_caller_subject(params, connection);
    SuricataIpsToggleCtx *ctx = g_new0(SuricataIpsToggleCtx, 1);
    ctx->enable    = enable;
    ctx->queue_num = (guint)pcv_config_get_ips_queue_num();
    ...
    JsonObject *res = json_object_new();
    json_object_set_string_member(res, "status", "started");
    json_object_set_string_member(res, "op", op);
    JsonNode *node = json_node_new(JSON_NODE_OBJECT);
    json_node_take_object(node, res);
    gchar *resp = pure_rpc_build_success_response(rpc_id, node);
    pure_uds_server_send_response(server, connection, resp);
...
static void
_handle_suricata_ips_enable(JsonObject *params, const gchar *rpc_id,
                            UdsServer *server, GSocketConnection *connection)
{
    _suricata_ips_toggle_async(TRUE, params, rpc_id, server, connection);
}

                                                                                
                                                  
static void
_handle_suricata_ips_disable(JsonObject *params, const gchar *rpc_id,
                             UdsServer *server, GSocketConnection *connection)
{
    _suricata_ips_toggle_async(FALSE, params, rpc_id, server, connection);
}
...
static void
_handle_suricata_ips_status(JsonObject *params, const gchar *rpc_id,
                            UdsServer *server, GSocketConnection *connection)
{
    (void)params; (void)connection;
    JsonObject *res = json_object_new();

    PcvSuricataState st = PCV_SURICATA_INACTIVE;
    gboolean binary_present = FALSE;
    pcv_suricata_ips_engine_status_cached(&st, &binary_present);
    JsonObject *engine = json_object_new();
    json_object_set_string_member(engine, "state", pcv_suricata_state_str(st));
    json_object_set_boolean_member(engine, "binary_present", binary_present);
    json_object_set_object_member(res, "engine", engine);

    json_object_set_boolean_member(res, "enabled", pcv_suricata_ips_is_enabled());
    json_object_set_int_member(res, "queue_num", (gint64)pcv_config_get_ips_queue_num());
    json_object_set_boolean_member(res, "fail_open", pcv_config_get_ips_fail_open());
    json_object_set_string_member(res, "mode", pcv_config_get_ips_mode());
                                                                          
                                                           
                                                               
                                                
    json_object_set_boolean_member(res, "degraded", pcv_suricata_ips_is_degraded());

                                           
    g_mutex_lock(&g_suricata_ips_job.mu);
    JsonObject *lt = json_object_new();
    json_object_set_boolean_member(lt, "running", g_suricata_ips_job.running);
    if (g_suricata_ips_job.last_op)
        json_object_set_string_member(lt, "op", g_suricata_ips_job.last_op);
    if (g_suricata_ips_job.last_result)
        json_object_set_string_member(lt, "result", g_suricata_ips_job.last_result);
    if (g_suricata_ips_job.last_error)
        json_object_set_string_member(lt, "error", g_suricata_ips_job.last_error);
    json_object_set_int_member(lt, "ts", g_suricata_ips_job.last_ts);
    g_mutex_unlock(&g_suricata_ips_job.mu);
    json_object_set_object_member(res, "last_toggle", lt);
...
static void
_suricata_ips_drop_ctx_free(gpointer data)
{
    SuricataIpsDropCtx *ctx = data;
    if (!ctx) return;
    if (ctx->desired) g_hash_table_destroy(ctx->desired);
    if (ctx->prev)    g_hash_table_destroy(ctx->prev);
    g_free(ctx->admin);
    g_free(ctx);
}
...
static void
_suricata_ips_drop_worker(GTask *task, gpointer src, gpointer task_data,
                          GCancellable *cancellable)
{
    (void)task; (void)src; (void)cancellable;
    SuricataIpsDropCtx *ctx = task_data;
    const gchar *method = ctx->add ? "suricata.ips.drop.add" : "suricata.ips.drop.remove";

    GError *error = NULL;
                                                    
                             
    gboolean ok = pcv_suricata_ips_rules_apply(ctx->desired, ctx->admin, &error);

    if (ok) {
                                          
        GError *serr = NULL;
        if (!pcv_suricata_policy_drop_sids_commit(&serr)) {
            ok = FALSE;
            PCV_LOG_WARN("suricata_ips",
                "drop_sids 정책 저장 실패 — 파생 룰셋은 이미 갱신됨(다음 성공적 "
                "drop 변경 또는 rules.update 재생성에서 정합 복구): %s",
                serr ? serr->message : "unknown");
            if (!error) error = serr; else g_clear_error(&serr);
        }
    }
    if (!ok) {
                                                
        pcv_suricata_policy_drop_sids_rollback(ctx->prev);
    }
    ...
    pcv_audit_log(ctx->admin ? ctx->admin : "system", method, "suricata-ips-drop",
                  ok ? "ok" : "fail", ok ? 0 : (error ? error->code : -1), 0, "local");
    ...
    g_free(g_suricata_ips_job.last_result);
    g_suricata_ips_job.last_result = g_strdup(ok ? "ok" : "fail");
    g_free(g_suricata_ips_job.last_error);
    g_suricata_ips_job.last_error = ok ? NULL
        : g_strdup(error ? error->message : "unknown error");
    g_suricata_ips_job.last_ts = g_get_real_time() / G_USEC_PER_SEC;
    g_mutex_unlock(&g_suricata_ips_job.mu);
...
static void
_suricata_ips_drop_mutate(gboolean add, JsonObject *params, const gchar *rpc_id,
                          UdsServer *server, GSocketConnection *connection)
{
    const gchar *op = add ? "drop.add" : "drop.remove";

                                   
    JsonArray *sids = NULL;
    if (params && json_object_has_member(params, "sids")) {
        JsonNode *n = json_object_get_member(params, "sids");
        if (n && JSON_NODE_HOLDS_ARRAY(n))
            sids = json_node_get_array(n);
    }
    if (!sids) {
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            "Missing required parameter: sids (array of positive integers)");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    ...
    if (g_suricata_ips_job.running) {
        g_mutex_unlock(&g_suricata_ips_job.mu);
        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_BUSY,
            "suricata ips operation already in progress");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        return;
    }
    ...
    gchar *saved_op = g_suricata_ips_job.last_op;
    g_suricata_ips_job.last_op = g_strdup(op);
    g_mutex_unlock(&g_suricata_ips_job.mu);

                                                
    GHashTable *prev = pcv_suricata_policy_drop_sids_snapshot();
    GError *error = NULL;
    if (!pcv_suricata_policy_set_drop_sids_global(sids, add, &error)) {
        ...
        g_mutex_lock(&g_suricata_ips_job.mu);
        g_free(g_suricata_ips_job.last_op);
        g_suricata_ips_job.last_op = saved_op;               
        g_suricata_ips_job.running = FALSE;
        g_mutex_unlock(&g_suricata_ips_job.mu);

        gchar *err = pure_rpc_build_error_response(rpc_id, PURE_RPC_ERR_INVALID_PARAMS,
            error ? error->message : "invalid sids");
        pure_uds_server_send_response(server, connection, err);
        g_free(err);
        g_clear_error(&error);
        return;
    ...
    const gchar *admin = _dispatcher_caller_subject(params, connection);
    SuricataIpsDropCtx *ctx = g_new0(SuricataIpsDropCtx, 1);
    ctx->add     = add;
    ctx->prev    = prev;
...
static void
_handle_suricata_ips_drop_list(JsonObject *params, const gchar *rpc_id,
                               UdsServer *server, GSocketConnection *connection)
{
    (void)params;
                                                 
                                                                
    JsonObject *full = pcv_suricata_policy_get_json(NULL);
    JsonNode   *arr  = json_object_dup_member(full, "drop_sids");
    json_object_unref(full);

    JsonObject *res = json_object_new();
    if (arr)
        json_object_set_member(res, "sids", arr);
    else
        json_object_set_array_member(res, "sids", json_array_new());           
...
static void
_handle_suricata_ips_drop_add(JsonObject *params, const gchar *rpc_id,
                              UdsServer *server, GSocketConnection *connection)
{
    _suricata_ips_drop_mutate(TRUE, params, rpc_id, server, connection);
}

                                                      
                                                      
static void
_handle_suricata_ips_drop_remove(JsonObject *params, const gchar *rpc_id,
                                 UdsServer *server, GSocketConnection *connection)
{
    _suricata_ips_drop_mutate(FALSE, params, rpc_id, server, connection);
}
...
static void dispatcher_init_routes(void)
{
    ...
    static const char *_async_method_names[] = {
        ...
        "vpc.create", "vpc.delete", "vpc.egress.set",
        "vpc.subnet.create", "vpc.subnet.delete",
        "vpc.attachment.create", "vpc.attachment.delete",
        "vpc.service.publish", "vpc.service.unpublish",
        "vpc.reconcile",
        NULL
    };
    ...
    g_hash_table_insert(g_rpc_routes, "alert.ack",           (gpointer)_handle_alert_ack);              
    ...
    g_hash_table_insert(g_rpc_routes, "push.vapid.get",      (gpointer)_handle_push_vapid_get);
    g_hash_table_insert(g_rpc_routes, "push.subscribe",      (gpointer)_handle_push_subscribe);
    g_hash_table_insert(g_rpc_routes, "push.unsubscribe",    (gpointer)_handle_push_unsubscribe);
    g_hash_table_insert(g_rpc_routes, "push.mine",           (gpointer)_handle_push_mine);
    g_hash_table_insert(g_rpc_routes, "push.list",           (gpointer)_handle_push_list);
    g_hash_table_insert(g_rpc_routes, "push.vapid.rotate",   (gpointer)_handle_push_vapid_rotate);
    g_hash_table_insert(g_rpc_routes, "push.test",           (gpointer)_handle_push_test);
    ...
    g_hash_table_insert(g_rpc_routes, "auth.totp.status",              (gpointer)handle_auth_totp_status);
    g_hash_table_insert(g_rpc_routes, "auth.totp.disable",             (gpointer)handle_auth_totp_disable);
    g_hash_table_insert(g_rpc_routes, "auth.totp.reset",               (gpointer)handle_auth_totp_reset);
    g_hash_table_insert(g_rpc_routes, "auth.totp.recovery.regenerate", (gpointer)handle_auth_totp_recovery_regenerate);
    ...
    g_hash_table_insert(g_rpc_routes, "tenant_overlay.create",    (gpointer)handle_tenant_overlay_create);
    g_hash_table_insert(g_rpc_routes, "tenant_overlay.delete",    (gpointer)handle_tenant_overlay_delete);
    g_hash_table_insert(g_rpc_routes, "tenant_overlay.list",      (gpointer)handle_tenant_overlay_list);
    g_hash_table_insert(g_rpc_routes, "tenant_overlay.get",       (gpointer)handle_tenant_overlay_get);
    g_hash_table_insert(g_rpc_routes, "tenant_overlay.attach_vm", (gpointer)handle_tenant_overlay_attach_vm);
    g_hash_table_insert(g_rpc_routes, "tenant_overlay.detach_vm", (gpointer)handle_tenant_overlay_detach_vm);

                                                                          
    g_hash_table_insert(g_rpc_routes, "vpc.list",              (gpointer)handle_vpc_list);
    g_hash_table_insert(g_rpc_routes, "vpc.get",               (gpointer)handle_vpc_get);
    g_hash_table_insert(g_rpc_routes, "vpc.subnet.list",       (gpointer)handle_vpc_subnet_list);
    g_hash_table_insert(g_rpc_routes, "vpc.attachment.list",   (gpointer)handle_vpc_attachment_list);
    g_hash_table_insert(g_rpc_routes, "vpc.service.list",      (gpointer)handle_vpc_service_list);
    g_hash_table_insert(g_rpc_routes, "vpc.status",            (gpointer)handle_vpc_status);
    g_hash_table_insert(g_rpc_routes, "vpc.backend.list",      (gpointer)handle_vpc_backend_list);
    g_hash_table_insert(g_rpc_routes, "vpc.create",            (gpointer)handle_vpc_create);
    g_hash_table_insert(g_rpc_routes, "vpc.delete",            (gpointer)handle_vpc_delete);
    g_hash_table_insert(g_rpc_routes, "vpc.egress.set",        (gpointer)handle_vpc_egress_set);
    g_hash_table_insert(g_rpc_routes, "vpc.subnet.create",     (gpointer)handle_vpc_subnet_create);
    g_hash_table_insert(g_rpc_routes, "vpc.subnet.delete",     (gpointer)handle_vpc_subnet_delete);
    g_hash_table_insert(g_rpc_routes, "vpc.attachment.create", (gpointer)handle_vpc_attachment_create);
    g_hash_table_insert(g_rpc_routes, "vpc.attachment.delete", (gpointer)handle_vpc_attachment_delete);
    g_hash_table_insert(g_rpc_routes, "vpc.service.publish",   (gpointer)handle_vpc_service_publish);
    g_hash_table_insert(g_rpc_routes, "vpc.service.unpublish", (gpointer)handle_vpc_service_unpublish);
    g_hash_table_insert(g_rpc_routes, "vpc.reconcile",         (gpointer)handle_vpc_reconcile);
    ...
    g_hash_table_insert(g_rpc_routes, "qos.vm.set",           (gpointer)_handle_qos_vm_set);
    g_hash_table_insert(g_rpc_routes, "qos.vm.get",           (gpointer)_handle_qos_vm_get);
    g_hash_table_insert(g_rpc_routes, "qos.tenant.set",       (gpointer)_handle_qos_tenant_set);
    g_hash_table_insert(g_rpc_routes, "qos.tenant.get",       (gpointer)_handle_qos_tenant_get);
    g_hash_table_insert(g_rpc_routes, "qos.stats",            (gpointer)_handle_qos_stats);
    g_hash_table_insert(g_rpc_routes, "qos.chaos.start",      (gpointer)_handle_qos_chaos_start);
    g_hash_table_insert(g_rpc_routes, "qos.chaos.stop",       (gpointer)_handle_qos_chaos_stop);
    g_hash_table_insert(g_rpc_routes, "qos.chaos.status",     (gpointer)_handle_qos_chaos_status);

                                                                 
    g_hash_table_insert(g_rpc_routes, "debug.trace.start",   (gpointer)_handle_debug_trace_start);
    g_hash_table_insert(g_rpc_routes, "debug.trace.stop",    (gpointer)_handle_debug_trace_stop);
    g_hash_table_insert(g_rpc_routes, "debug.trace.status",  (gpointer)_handle_debug_trace_status);
    g_hash_table_insert(g_rpc_routes, "debug.trace.list",    (gpointer)_handle_debug_trace_list);
    g_hash_table_insert(g_rpc_routes, "debug.trace.report",  (gpointer)_handle_debug_trace_report);

                                                                   
    g_hash_table_insert(g_rpc_routes, "suricata.policy.set",   (gpointer)_handle_suricata_policy_set);
    g_hash_table_insert(g_rpc_routes, "suricata.policy.get",   (gpointer)_handle_suricata_policy_get);
    g_hash_table_insert(g_rpc_routes, "suricata.rules.update", (gpointer)_handle_suricata_rules_update);
    g_hash_table_insert(g_rpc_routes, "suricata.status",       (gpointer)_handle_suricata_status);
    g_hash_table_insert(g_rpc_routes, "suricata.ips.status",  (gpointer)_handle_suricata_ips_status);
    g_hash_table_insert(g_rpc_routes, "suricata.ips.enable",  (gpointer)_handle_suricata_ips_enable);
    g_hash_table_insert(g_rpc_routes, "suricata.ips.disable", (gpointer)_handle_suricata_ips_disable);
    g_hash_table_insert(g_rpc_routes, "suricata.ips.drop.list",   (gpointer)_handle_suricata_ips_drop_list);
    g_hash_table_insert(g_rpc_routes, "suricata.ips.drop.add",    (gpointer)_handle_suricata_ips_drop_add);
    g_hash_table_insert(g_rpc_routes, "suricata.ips.drop.remove", (gpointer)_handle_suricata_ips_drop_remove);
