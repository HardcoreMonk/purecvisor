...
WORK_MARKER=".pcv-webperf-autoinstall-workdir"
CRED_TMP=""
XORRISO_LOG=""
CHECKSUM_FILE=""
...
cleanup() {
    local status=$?
    if [[ -n "$CRED_TMP" && -e "$CRED_TMP" ]]; then
        rm -f -- "$CRED_TMP"
    fi
    if [[ "$status" -ne 0 && -n "$XORRISO_LOG" && -f "$XORRISO_LOG" ]]; then
        printf 'xorriso log: %s\n' "$XORRISO_LOG" >&2
    fi
}
trap cleanup EXIT
...
validate_inputs() {
    local normalized parent src_canonical out_canonical cred_canonical

    [[ "$WORK_DIR" == /* ]] || fail_input "work directory must be absolute"
    normalized="$(realpath -m -- "$WORK_DIR")"
    [[ "$normalized" == "$WORK_DIR" ]] || fail_input "work directory must be normalized"
    case "$WORK_DIR" in
        /|/bin|/boot|/dev|/etc|/home|/lib|/lib64|/opt|/proc|/root|/run|/sbin|/srv|/sys|/tmp|/usr|/var|/var/tmp)
            fail_input "protected work directory"
            ;;
    esac

    parent="$(dirname -- "$WORK_DIR")"
    [[ -d "$parent" && ! -L "$parent" ]] || fail_input "work directory parent must be a real directory"
    if [[ -e "$WORK_DIR" || -L "$WORK_DIR" ]]; then
        [[ -d "$WORK_DIR" && ! -L "$WORK_DIR" ]] || fail_input "work directory must not be a symlink"
                                                         
                                                           
        [[ -f "$WORK_DIR/$WORK_MARKER" && ! -L "$WORK_DIR/$WORK_MARKER" ]] \
            || fail_input "existing work directory is not owned by this tool"
    fi

    [[ "$HOSTNAME" =~ ^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?$ ]] \
        || fail_input "hostname must be a single DNS label"
    [[ "$USERNAME" =~ ^[a-z_][a-z0-9_-]{0,30}$ ]] \
        || fail_input "username is not a supported Linux account name"
    [[ -z "$HOME_SIZE" || "$HOME_SIZE" =~ ^[1-9][0-9]*([KMGTPE]i?B?|B)?$ ]] \
        || fail_input "home size must be a positive curtin size"
    [[ "$PASSWORD" != *$'\n'* && "$PASSWORD" != *$'\r'* ]] \
        || fail_input "password must not contain line breaks"

                                                   
                                                          
    [[ ! -L "$OUT_ISO" ]] || fail_input "output ISO must not be a symlink"
    [[ ! -L "$CRED_FILE" ]] || fail_input "credential file must not be a symlink"
    if ! src_canonical="$(realpath -e -- "$SRC_ISO")"; then
        fail_input "source ISO cannot be canonicalized"
    fi
    if ! out_canonical="$(realpath -m -- "$OUT_ISO")"; then
        fail_input "output ISO cannot be canonicalized"
    fi
    if ! cred_canonical="$(realpath -m -- "$CRED_FILE")"; then
        fail_input "credential file cannot be canonicalized"
    fi
    CHECKSUM_FILE="${out_canonical}.sha256"
    [[ ! -L "$CHECKSUM_FILE" ]] || fail_input "checksum file must not be a symlink"

                                                              
                                                                
                            
    [[ "$src_canonical" != "$out_canonical" ]] \
        || fail_input "source and output ISO must differ"
    [[ "$cred_canonical" != "$src_canonical" && "$cred_canonical" != "$out_canonical" ]] \
        || fail_input "credential file must not overlap an ISO"
    [[ "$CHECKSUM_FILE" != "$src_canonical" && "$CHECKSUM_FILE" != "$cred_canonical" ]] \
        || fail_input "checksum file must not overlap source or credentials"
    if [[ "$SRC_ISO" -ef "$OUT_ISO" || "$SRC_ISO" -ef "$CRED_FILE" \
          || "$SRC_ISO" -ef "$CHECKSUM_FILE" || "$OUT_ISO" -ef "$CRED_FILE" \
          || "$OUT_ISO" -ef "$CHECKSUM_FILE" || "$CRED_FILE" -ef "$CHECKSUM_FILE" ]]; then
        fail_input "source, output, checksum, and credential files must have distinct inodes"
    fi

    for normalized in "$src_canonical" "$out_canonical" "$CHECKSUM_FILE" "$cred_canonical"; do
        case "$normalized" in
            "$WORK_DIR"|"$WORK_DIR"/*)
                fail_input "source and result files must be outside the disposable work directory"
                ;;
        esac
    done

                                       
    SRC_ISO="$src_canonical"
    OUT_ISO="$out_canonical"
    CRED_FILE="$cred_canonical"
}
...
prepare_work_dir() {
    ...
    : >"$WORK_DIR/$WORK_MARKER"
    chmod 0600 "$WORK_DIR/$WORK_MARKER"
}
...
rm -f -- "$OUT_ISO"
XORRISO_LOG="$(mktemp "$WORK_DIR/.xorriso.XXXXXX.log")"
xorriso_maps=()
if [[ -f "$WORK_DIR/boot/grub/grub.cfg" ]]; then
    xorriso_maps+=(-map "$WORK_DIR/boot/grub/grub.cfg" /boot/grub/grub.cfg)
fi
if [[ -f "$WORK_DIR/boot/grub/loopback.cfg" ]]; then
    xorriso_maps+=(-map "$WORK_DIR/boot/grub/loopback.cfg" /boot/grub/loopback.cfg)
fi
xorriso \
    -indev "$SRC_ISO" \
    -outdev "$OUT_ISO" \
...
chmod 0644 "$OUT_ISO"
sha256sum "$OUT_ISO" > "$CHECKSUM_FILE"
chmod 0644 "$CHECKSUM_FILE"

cred_dir="$(dirname -- "$CRED_FILE")"
mkdir -p -- "$cred_dir"
[[ -d "$cred_dir" ]] || fail_input "credential parent is not a directory"
CRED_TMP="$(mktemp "$cred_dir/.pcv-webperf-credentials.XXXXXX")"
cat > "$CRED_TMP" <<EOF_CRED
