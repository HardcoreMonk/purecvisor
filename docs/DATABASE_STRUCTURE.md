...
| Local VPC DB | `src/modules/network/vpc/vpc_store.c` | tenant VPC, subnet, VM attachment, Service Publish desired state | `/var/lib/purecvisor/vpc.db` | `pcv_vpc_init()` 인자(테스트 주입) |
...
`ALTER TABLE ... ADD COLUMN`로 구조를 보장한다. 예외적으로 RBAC DB와 Local VPC DB는
`PRAGMA user_version`으로 모듈별 schema version을 표시한다. Local VPC는 현재 버전보다
높은 DB를 열면 스키마를 덮어쓰지 않고 기동을 거부한다.
...
| `prev_hash` | `TEXT` | nullable | 직전 레코드의 `rec_hash` (해시체인 링크, A09/2.9). 첫 레코드는 제네시스 상수 |
| `rec_hash` | `TEXT` | nullable | `SHA256(prev_hash\|ts\|username\|method\|target\|result\|error_code)`. 마이그레이션 전 기록은 두 컬럼 모두 NULL이며 검증에서 체인 밖으로 skip된다 |
