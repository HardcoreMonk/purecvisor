...
```c
/* dispatcher.c — 변경 전의 제거된 helper 호출 */
...
2. `dispatcher.c`의 rpc_done 블록은 메서드가 등록되어 있으면 현재 공통
   `pcv_audit_log()` 호출을 건너뛴다. `pcv_audit_log_rpc()`는 AUDIT-F2에서 제거된
   과거 wrapper 이름이며, 위 변경 전 예시에만 남긴다.

   ```c
   if (!pcv_dispatcher_is_async_method(method)) {
       pcv_audit_log(username, method, target, result, error_code,
                     (gint64)dur_ms, "local");
   }
   ```
