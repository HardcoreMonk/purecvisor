...
> 운영 단계: 공개 `main`·Pages와 지정 Arch·로컬 Ubuntu 노드 내장 도움말 배포·HTTPS 검증 완료
...
지정 Arch 검증 노드와 로컬 Ubuntu 노드에 도움말·서비스 워커를 적용했다. 환경별 내용은
아래 후속 배포 기록을 따른다.
...
## 로컬 Ubuntu 노드 도움말 후속 배포 — 2026-09-16

- 공개 `main`의 `09be0d2`를 입력으로 로컬 Ubuntu 26.04.1 노드에 `guide-content.md`와
  `sw.js`를 배포했다. 기존 파일은 `/var/backups/purecvisor/docs-local-20260916-09be0d2/`에
  백업하고 root:root `0644`로 원자 교체했다. 실패 시 백업본으로 복원하는 경로를 준비했다.
- 이 설치본의 일부 UI 파일은 공개 배포본과 달랐다. 도움말을 제외한 기존 캐시 입력
  15개를 보존하고 `Makefile`의 16개 `UI_CACHE_INPUTS`와 같은 해시 계산으로 설치본의
  캐시 버전 `f7863d6e`를 생성했다. 서비스 워커 실행 로직은 캐시 이름 외에 동일하다.
  공개 소스의 캐시 버전 `75dcc77b`와 설치 환경별 산출물을 혼용하지 않는다.
- 로컬은 NGINX 외부 TLS 종료 환경이다. 데몬 PKI 인증서와 구분해 실제 NGINX 인증서의
  신뢰·관리 IP 일치를 검증한 HTTPS로 도움말·SW·JS 번들·`guide.html`·`docs.html`을 조회했다.
  5개 모두 HTTP 200, 기대 MIME과 설치 산출물의 SHA-256 일치를 확인했다.
- 도움말 SHA-256은 Arch 배포와 동일한
  `146a61d81035b402c6daffa19a269e96c126c14047a9a0f19499b3b902e531d3`이다.
  로컬 `sw.js` SHA-256은
  `9b2a5f148d5bce9af4ae3a916fa259249c58a410cb83e7659632ba44b7487800`이다.
- `purecvisorsd`·NGINX는 `active/running`, health는 `ok`, 제품 버전은 `2.0.0`이다.
  두 서비스의 PID·시작 시각·재시작 횟수, 바이너리·설정·인증서와 나머지 UI 자산은
