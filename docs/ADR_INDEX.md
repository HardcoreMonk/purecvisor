...
| ADR-0047 | Verified | 공개 운영 가이드는 `docs/GUIDE.md`의 22개 장을 `/ko/<분류>/<문서>/` 정적 page로 생성한다. 기본 진입은 `/ko/getting-started/installation/`이며 landing·Header는 새 route를 사용하고 `/docs.html`은 legacy hash redirect만 유지한다. 2026-08-24 Pages run `32652592291`과 custom domain에서 검증했다. |
