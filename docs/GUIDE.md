...
>
> **문서 현행화(2026-09-07)**: 개발선 감사·UI 시정과 운영 확인 현황을 1장·13장·21장·22장에 반영했습니다. 공개 제품 버전과 공개 소스의 검증 스냅샷은 별도로 관리합니다.
>
...
**2026-09-07 검토·배포 현황:** 개발선에서는 VM 메모리/I/O 화면의 요청 대상, 늦은 응답과 대화상자 수명 문제를 시정하고 독립 리뷰 및 두 운영 노드의 UI 자산 배포 확인을 마쳤습니다. 전체 소스 감사와 지원 환경 인증은 진행 중이며 전체 감사 판정은 **FAIL(미완료)**입니다.

이번 Pages 갱신은 문서 업데이트입니다. 개발선 시정이 이 공개 저장소의 제품 소스에 모두 반영됐다는 의미는 아닙니다. 공개판은 `2.0.0`과 Single Edge 범위를 유지하며, 상세 회차·잔여는 [품질 게이트의 최신 검토 현황](/ko/development/quality-gates/)에서 확인합니다.
...
> 설계 근거: [ADR-0033](adr/0033-bridge-nic-mtu-contract.md).
...
resp=$(curl -s -X POST http://localhost:80/api/v1/auth/token \
  -d '{"username":"admin","password":"<password>"}')
PENDING=$(echo "$resp" | python3 -c "import sys,json;print(json.load(sys.stdin)['pending_token'])")

# 2. TOTP 코드로 완주
curl -s -X POST http://localhost:80/api/v1/auth/totp/verify \
...
스냅샷이다. 공개 소스의 과거 검증 기록이며 2026-09-07 개발선 감사 수치와 합산하지 않습니다. 릴리스 판단은 `git`,
...
| VM clone | `./test_runner -r /vm_clone_plan`, `scripts/check_vm_clone_cleanup.py`, [ADR-0023](adr/0023-vm-clone-beta-safety-guard.md) 실환경 기준 확인 |
...
| 4b~4z-2 | 계약 게이트 27종 (22.4의 32종 중 26종 + `check-help-counts`) | 해당 영역 변경 | 차단 |
...
공개 스냅샷의 C 1,375/1,375·audit startup 5/5와 계약 gate 38/38은 18.6의 2026-08-31 기록입니다. 이번 문서 배포에서 다시 실행한 전체 C·제품 UI 시험 수치가 아닙니다. Pages 자체는 콘텐츠 build·route·링크·공개 자산 검사와 게시 후 HTTP·브라우저 확인으로 검증합니다.
