...
# OVN NAT VPC와 첫 subnet 일괄 생성 — 기본값은 worker terminal 완료까지 기다린다.
...
# 자동화가 Job을 별도 추적할 때만 accepted-only 사용
pcvctl --format=json vpc reconcile --no-wait
```
...
릴리스 전:
  make check-all (계약 게이트 전량) + make test (1,259건) → 릴리스 허용
```
...
| 3 | 유닛 테스트 1,259건 (200건 최소) | C 변경 | 차단 |
